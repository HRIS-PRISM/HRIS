  ...
  db.query(summaryQuery, [employeeNumber, endDate, startDate, startDate], (err, summaryRows) => {
    ...
    db.query(dailyQuery, [employeeNumber, startDate, endDate], (err2, dailyRows) => {
      ...
      const stats = {
        ...
        present_days:   dailyRecords.filter(r => !r.is_absent).length,
        absent_days:    dailyRecords.filter(r => r.is_absent).length,
...
router.post("/leave", authenticateToken, requireAdmin, (req, res) => {
  const { employeeNumber, leave_code, earned_hours, period_year, period_month, entry_type = "EARNED", remarks } = req.body;

  if (!employeeNumber || !leave_code || !period_year || !period_month)
    return res.status(400).json({ error: "employeeNumber, leave_code, period_year, period_month are required" });

  const hrs = toNum(earned_hours);
  if (hrs <= 0) return res.status(400).json({ error: "earned_hours must be > 0" });
  ...
  const query = `
    ...
    VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?)
  `;
  db.query(query, [employeeNumber, leave_code, hrs, period_year, parseInt(period_month), entry_type, remarks || null, req.user?.username || null], (err, result) => {
    if (err) return res.status(500).json({ error: "Failed to create leave earning" });
    auditEarning(req, "created", "leave", result.insertId, null, "pending", { employeeNumber, leave_code, earned_hours: hrs });
    res.status(201).json({ id: result.insertId, employee_number: employeeNumber, leave_code, earned_hours: hrs, period_year, period_month: parseInt(period_month), entry_type, earn_status: "pending", remarks: remarks || null });
  });
});
    ...
    db.query(
      ...
      (err2) => {
        ...
        db.query(findQuery, findParams, (err3, laRows) => {
          ...
          if (matched) {
            db.query(
              `UPDATE leave_assignment SET total_hours = total_hours + ?, remaining_hours = remaining_hours + ?, allocated_hours = allocated_hours + ? WHERE id = ?`,
...
router.post("/sc", authenticateToken, requireAdmin, (req, res) => {
  ...
  if (earnedHrs <= 0) return res.status(400).json({ error: "earned_hours must be > 0" });

  const query = `
    INSERT INTO sc_earnings
      (employee_number, sc_type, ot_hours_regular, ot_hours_holiday, ot_hours_night_diff, total_ot_hours, earned_hours, used_hours, period_year, period_month, earn_status, remarks, emp_category_snapshot, created_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 'pending', ?, ?, ?)
  `;
  db.query(query, [
    ...
    remarks || null, emp_category_snapshot ? JSON.stringify(emp_category_snapshot) : null, req.user?.username || null
  ], (err, result) => {
...
router.post("/cto", authenticateToken, requireAdmin, (req, res) => {
  ...
  const query = `
    INSERT INTO cto_earnings
      (employee_number, ot_hours, earned_hours, used_hours, period_year, period_month, expiry_date, earn_status, remarks, emp_category_snapshot, created_by)
    VALUES (?, ?, ?, 0, ?, ?, ?, 'pending', ?, ?, ?)
  `;
  db.query(query, [
