...
async function ensureSupervisorPage(componentIdentifier) {
  const seed = SUPERVISOR_PAGE_SEEDS.find((p) => p.identifier === componentIdentifier);
  if (!seed) return null;
...
async function countSupervisorAssignments(employeeNumber) {
  const rows = await queryAsync(
    `SELECT COUNT(*) AS cnt FROM supervisor_assignment WHERE ${empMatchSql('supervisorEmployeeNumber')}`,
...
async function grantSupervisorPageAccess(employeeNumber, componentIdentifier) {
  ...
  if (!page) {
    console.warn(`[supervisorPageAccess] Could not register page: ${componentIdentifier}`);
    return;
  }
