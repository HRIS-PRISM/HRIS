...
const FacultyClearance70Days = () => {
  ...
  const printPage = async () => {
    try {
      ...
      await printFormHtml(formRef.current, { title: 'Print Faculty Clearance 70 Days' });
    } catch (err) {
  ...
  const downloadPDF = async () => {
    try {
      ...
      await downloadFormHtml(
        formRef.current,
        `Faculty-Clearance-70Days-${new Date().toISOString().split('T')[0]}.pdf`,
        { title: 'Faculty Clearance 70 Days' },
      );
