...
useEffect(() => {
  ...
  if (stored) {
    try {
      ...
      if (p && typeof p === "object") setSettings(p);
    } catch (e) {
      console.error(e);
    }
  }
...
<Box
  sx={{
    ...
    p: 2,
    borderRadius: 2,
    border: `1.5px dashed ${alpha(T.accent, 0.3)}`,
    bgcolor: T.accentFaint,
  }}
>
  ...
  <Grid container spacing={2}>
    ...
    <Grid item xs={12} sm={6}>
      <FieldInput
        fullWidth
        size="small"
        label="Override Cut-off Time (optional)"
        type="time"
  ...
  >
    Example: Official time 7:00 AM + 2 hrs grace → employees must clock in by{" "}
    <strong>9:00 AM</strong> with no deduction.
  </Typography>
</Box>
);
...
>
  <Icon
    sx={{
      fontSize: 18,
      color: active ? color : alpha(T.accent, 0.4),
      mt: 0.2,
    }}
  />
  <Box sx={{ flex: 1 }}>
    ...
    <Typography sx={{ fontSize: "0.67rem", color: T.muted, lineHeight: 1.4 }}>
      {caption}
    </Typography>
  </Box>
</Box>
...
const chips = [];
const isHrOnly =
  item.hr_only === true || item.hr_only === 1 || item.hr_only === "1";
const isFlexi =
  item.is_flexi === true || item.is_flexi === 1 || item.is_flexi === "1";
...
if (isHrOnly) {
  chips.push(
    ...
    >
      ...
      <Typography
        sx={{ fontSize: "0.65rem", fontWeight: 700, color: "#1565c0" }}
      >
        HR Only
      </Typography>
    </Box>,
...
return {
  label: "Announcement",
  icon: <AnnouncementIcon sx={{ fontSize: 10 }} />,
  bg: T.accentFaint,
  color: T.accent,
  border: `1px solid ${T.accentBorder}`,
};
...
const buildFormData = (src) => {
  ...
  if (src.is_flexi) {
    ...
    if (src.flexi_custom_time)
      fd.append("flexi_custom_time", src.flexi_custom_time);
  }
...
const handleAdd = async () => {
  ...
  if (!title || !about || !date_start || !date_end) {
    showSnackbar("Please fill in Title, About, and Date Range.");
    return;
  }
  setLoading(true);
  try {
    ...
    showSnackbar("Failed to add announcement.");
  } finally {
    setLoading(false);
  }
};
...
const handleAddSuspension = async () => {
  const { title, date_start, date_end, suspension_type, effective_time } =
    newSuspension;
  if (!title || !date_start || !date_end) {
    showSnackbar("Please fill in Title and Date Range.");
    return;
  }
  if (suspension_type === "partial_day" && !effective_time) {
    showSnackbar("Effective time is required for a partial-day suspension.");
    return;
  }
  setLoading(true);
  try {
    const fd = buildFormData({
      ...newSuspension,
      about: newSuspension.about || "",
    });
    fd.append("reason", newSuspension.reason || "");
    fd.append("personnel_scope", newSuspension.personnel_scope || "all");
    fd.append(
      "suspension_type",
      newSuspension.suspension_type || "whole_day",
    );
    fd.append(
      "effective_time",
      newSuspension.suspension_type === "partial_day"
    ...
    setNewSuspension({ ...emptySusp, ...emptySuspExtra });
    setSuccessOpen(true);
    fetchAll();
  } catch (e) {
    showSnackbar("Failed to add suspension.");
  } finally {
    setLoading(false);
  }
};
...
const handleAddHoliday = async () => {
  ...
  if (!title || !date_start || !date_end || !status) {
    showSnackbar("Please fill in Title, Date Range, and Status.");
    return;
  }
  setLoading(true);
  try {
    ...
    fd.append("about", newHoliday.about || "");
    ...
    showSnackbar("Failed to add holiday.");
  } finally {
    setLoading(false);
  }
};
...
const isEnded = (item) => {
  if (item.isHoliday && (item.status || "").toLowerCase() === "inactive")
    return true;
...
const suspensionsMapped = (suspensions || []).map((s) => ({
  ...
  flexi_custom_time: s.flexi_custom_time,
}));
...
const filteredItems = useMemo(() => {
  ...
  if (typeFilter === "holiday") f = f.filter((i) => i.isHoliday);
    ...
    f = f.filter((i) => !i.isHoliday && !i.isSuspension);
  else if (typeFilter === "suspension") f = f.filter((i) => i.isSuspension);
  else if (typeFilter === "hr_only")
    f = f.filter((i) => i.hr_only === true || i.hr_only === 1);
  else if (typeFilter === "flexi")
    f = f.filter((i) => i.is_flexi === true || i.is_flexi === 1);
  if (searchQuery.trim()) {
    const q = searchQuery.toLowerCase();
    f = f.filter(
      (i) =>
        (i.title || "").toLowerCase().includes(q) ||
        (i.about || "").toLowerCase().includes(q),
    );
  }
  return f;
...
const renderLeftPanel = () => {
  ...
  const onFlexiChange = (field, val) =>
    setSrc((p) => ({ ...p, [field]: val }));
  const onSubmit = isAnn
    ? handleAdd
    : isSusp
      ? handleAddSuspension
      : handleAddHoliday;
  const submitLabel = isAnn
    ? "Add Announcement"
    : isSusp
      ? "Add Suspension"
      : "Add Holiday";
  ...
  return (
    <Box sx={{ p: 2, display: "flex", flexDirection: "column", gap: 1.5 }}>
      ...
      <Box>
        ...
        >
          <ToggleButton value="announcement">
            <AnnouncementIcon sx={{ fontSize: 12 }} />
            &nbsp;Ann.
          </ToggleButton>
          <ToggleButton value="suspension">
      ...
      {isSusp && (
        <Box>
          <FormSectionLabel icon={Info}>Applies To</FormSectionLabel>
          <FormControl fullWidth size="small">
            <Select
              value={src.personnel_scope || 'all'}
              onChange={(e) => setSrc((p) => ({ ...p, personnel_scope: e.target.value }))}
              sx={compactSelectSx}
            >
      ...
      {isSusp && (
        <Box>
          <FormSectionLabel icon={Info}>Suspension Type</FormSectionLabel>
          <FormControl fullWidth size="small">
            <Select
              value={src.suspension_type || 'whole_day'}
              onChange={(e) =>
                setSrc((p) => ({
                  ...p,
                  suspension_type: e.target.value,
                  // whole_day never carries an effective_time
                  effective_time: e.target.value === 'whole_day' ? '' : p.effective_time,
                }))
              }
              sx={compactSelectSx}
            >
              <MenuItem value="whole_day" sx={{ fontSize: '0.82rem' }}>Whole Day</MenuItem>
              <MenuItem value="partial_day" sx={{ fontSize: '0.82rem' }}>Partial Day</MenuItem>
            </Select>
          </FormControl>
        </Box>
      )}

      {isSusp && src.suspension_type === 'partial_day' && (
        <Box>
          <FormSectionLabel icon={Info}>Effective From</FormSectionLabel>
          <FieldInput
            fullWidth size="small" type="time"
            value={src.effective_time || ''}
            onChange={(e) => setSrc((p) => ({ ...p, effective_time: e.target.value }))}
            InputLabelProps={{ shrink: true }}
          />
        </Box>
      )}
        ...
        <Box>
          ...
          <FormControl fullWidth size="small">
            <Select
              ...
              sx={compactSelectSx}
            >
              <MenuItem value="Active" sx={{ fontSize: "0.82rem" }}>
                Active
              </MenuItem>
              <MenuItem value="Inactive" sx={{ fontSize: "0.82rem" }}>
                Inactive
              </MenuItem>
            </Select>
          </FormControl>
      ...
      {/* About */}
      <Box>
        <FormSectionLabel icon={AnnouncementIcon}>
          {isAnn ? "About *" : isHol ? "About" : "Details / Description"}
        </FormSectionLabel>
        <FieldInput
          fullWidth
          size="small"
      ...
      {isAnn && (
        <Box>
          ...
          <Box sx={{ display: "flex", gap: 0.75 }}>
            {/* HR Only */}
            <Box
              onClick={() => setSrc((p) => ({ ...p, hr_only: !p.hr_only }))}
              sx={{
                flex: 1,
                display: "flex",
                flexDirection: "column",
                ...
                py: 0.85,
                borderRadius: 2,
                border: `1.5px solid ${src.hr_only ? "#1565c0" : T.accentBorder}`,
                bgcolor: src.hr_only ? alpha("#1565c0", 0.07) : "transparent",
                cursor: "pointer",
            ...
            >
              ...
              >
                <Typography
                  sx={{
                    fontSize: "0.65rem",
                    fontWeight: 700,
                    color: src.hr_only ? "#1565c0" : T.text,
                    lineHeight: 1.2,
                  }}
                >
                  HR Only
                ...
                <Switch
                  ...
                  onChange={(e) => {
                    e.stopPropagation();
                    setSrc((p) => ({ ...p, hr_only: e.target.checked }));
                  }}
                  sx={{
                    p: 0,
                    ml: 0.5,
              ...
              <Typography
                sx={{ fontSize: "0.58rem", color: T.muted, lineHeight: 1.3 }}
              >
                HR staff visibility
              </Typography>
            </Box>
            {/* Flexi */}
            <Box
              onClick={() => setSrc((p) => ({ ...p, is_flexi: !p.is_flexi }))}
              sx={{
                flex: 1,
                display: "flex",
                flexDirection: "column",
                ...
                py: 0.85,
                borderRadius: 2,
                border: `1.5px solid ${src.is_flexi ? "#2e7d32" : T.accentBorder}`,
                bgcolor: src.is_flexi
                  ? alpha("#2e7d32", 0.07)
                  : "transparent",
                cursor: "pointer",
                transition: "all 0.15s",
              ...
              >
                <Typography
                  sx={{
                    fontSize: "0.65rem",
                    fontWeight: 700,
                    color: src.is_flexi ? "#2e7d32" : T.text,
                    lineHeight: 1.2,
                  }}
                >
                  Flexi Schedule
                ...
                <Switch
                  size="small"
                  checked={!!src.is_flexi}
                  onChange={(e) => {
                    e.stopPropagation();
                    setSrc((p) => ({ ...p, is_flexi: e.target.checked }));
                  }}
                  sx={{
                    p: 0,
                    ml: 0.5,
      ...
      <Button
        ...
        sx={{
          ...
          bgcolor: T.accent,
          color: "#fff",
          boxShadow: `0 2px 10px ${alpha(T.accent, 0.3)}`,
  ...
  >
    ...
    <Box sx={{ flex: "1 1 200px", minWidth: 160 }}>
      ...
      <FieldInput
        fullWidth
        size="small"
        placeholder="Search title or about…"
        value={searchQuery}
    ...
    <Box sx={{ flex: "1 1 160px", minWidth: 140 }}>
      ...
      <FormControl fullWidth size="small">
        <Select
          ...
          sx={compactSelectSx}
        >
          <MenuItem value="all" sx={{ fontSize: "0.76rem" }}>
            All Types
          </MenuItem>
          <MenuItem value="announcement" sx={{ fontSize: "0.76rem" }}>
            Announcements
          </MenuItem>
          <MenuItem value="suspension" sx={{ fontSize: "0.76rem" }}>
            Suspensions
          </MenuItem>
          <MenuItem value="holiday" sx={{ fontSize: "0.76rem" }}>
            Holidays
          </MenuItem>
          <MenuItem value="hr_only" sx={{ fontSize: "0.76rem" }}>
            HR Only
          </MenuItem>
          <MenuItem value="flexi" sx={{ fontSize: "0.76rem" }}>
            Flexi
          </MenuItem>
        </Select>
      </FormControl>
    ...
    >
      {[
        {
          icon: <HrIcon sx={{ fontSize: 10, color: "#1565c0" }} />,
          label: "HR Only",
          desc: "HR staff only",
        },
        {
          icon: <FlexiIcon sx={{ fontSize: 10, color: "#2e7d32" }} />,
          label: "Flexi",
          desc: "Grace period",
        },
      ].map(({ icon, label, desc }) => (
        ...
        >
          {icon}
          <Typography
            sx={{ fontSize: "0.67rem", fontWeight: 700, color: T.text }}
          >
            {label}
          </Typography>
...
if (hasAccess === false)
  return (
    <AccessDenied
      title="Access Denied"
      message="You do not have permission to access Announcement Management."
...
return (
  <Fade in timeout={400}>
    <Box>
          ...
          >
            ...
            >
              ...
              <Box>
                ...
                >
                  Administrative Panel • Announcements, Suspensions &amp;
                  Holidays
                </Typography>
              </Box>
            </Box>
            ...
            >
              ...
              >
                {createFormType === "announcement" && (
                  <AnnouncementIcon sx={{ fontSize: 13, color: T.accent }} />
                )}
                {createFormType === "suspension" && (
                  <BlockIcon sx={{ fontSize: 13, color: "#b71c1c" }} />
                )}
                {createFormType === "holiday" && (
                  <EventIcon sx={{ fontSize: 13, color: "#e65100" }} />
                )}
                <Typography
                  sx={{
                    fontSize: "0.78rem",
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <Typography sx={{ fontSize: "0.72rem", color: T.muted }}>
                    {announcements.length} ann · {suspensions.length} susp ·{" "}
                    {holidays.length} hol
                  </Typography>
                </Box>
                {loading && (
              ...
              >
                ...
                <Box
                  sx={{
                    display: "grid",
                    gridTemplateColumns: "0.5fr 1fr 1.2fr 1.8fr 1.3fr 1.1fr",
                    px: 2.5,
                    py: 1.25,
                    bgcolor: T.accent,
                    gap: 1,
                ...
                >
                  {["#", "TYPE", "TITLE", "ABOUT", "DATE RANGE", "FLAGS"].map(
                    (h) => (
                      <Typography
                        key={h}
                        sx={{
                        ...
                        return (
                          <Box
                            ...
                            sx={{
                              display: "grid",
                              gridTemplateColumns:
                                "0.5fr 1fr 1.2fr 1.8fr 1.3fr 1.1fr",
                              px: 2.5,
                              py: 1.5,
                              gap: 1,
                              alignItems: "center",
                ...
                {[
                  {
                    ...
                    label: "Active = within date range",
                  },
                  {
                    icon: <HrIcon sx={{ fontSize: 13, color: "#1565c0" }} />,
                    label: "HR Only = restricted visibility",
                  },
                  {
                    icon: (
                      <FlexiIcon sx={{ fontSize: 13, color: "#2e7d32" }} />
                    ),
                    label: "Flexi = grace period for Non-Teaching",
                  },
                  {
                    icon: (
                      <BlockIcon sx={{ fontSize: 13, color: "#b71c1c" }} />
                    ),
                    label: "Suspension = work-day cancelled",
                  },
                ].map((item, i) => (
                  <Box
                    key={i}
      ...
      <LoadingOverlay
        open={loading}
        message="Processing announcement data…"
      />
      <SuccessfulOverlay
        open={successOpen}
