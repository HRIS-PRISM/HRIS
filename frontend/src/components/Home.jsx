            ...
            >
              ...
              <Box
                sx={{
                  ...
                  borderRadius: 4,
                  p: 2,
                  animation: 'adminPulse 2s ease-in-out 0.3s infinite',
                }}
              >
                <SkeletonBox
                  width={60}
...
const Home = () => {
      ...
      try {
        ...
        const grouped = Object.entries(byCode).map(([code, entries]) => {
          ...
          const prevTotal = previous.reduce(
            (s, e) => s + (parseFloat(e.total_hours) || 0) / 8,
            0,
          );
          return {
            code,
            name: leaveType?.leave_description || code,
            grandRemaining: currRemaining + prevRemaining,
            grandTotal: currTotal + prevTotal,
            currRemaining,
            currTotal,
            currAllocated,
            prevRemaining,
            period: current?.period_year
              ? `${current.period_year}${current.period_semester ? ` ${current.period_semester}` : ''}`
              : null,
          };
        });
      ...
      if (contactNotifs.length > 0) {
        try {
          ...
          contactNotifs.forEach((notif) => {
            ...
            if (ticket) {
              notifStatusMap[notif.id] = {
                status: ticket.status,
                subject: ticket.subject || ticket.title || '',
                employee_name: ticket.employee_name || ticket.name || '',
                employee_number:
                  ticket.employee_number || ticket.agencyEmployeeNum || '',
              };
            } else {
              const empTickets = ticketList.filter(
                (t) => String(t.employee_number || '').trim() === empNum,
              );
              if (empTickets.length > 0) {
                ...
                notifStatusMap[notif.id] = {
                  ...
                  subject: latest.subject || latest.title || '',
                  employee_name: latest.employee_name || latest.name || '',
                  employee_number:
                    latest.employee_number || latest.agencyEmployeeNum || '',
                };
              }
            }
          });
  ...
  useEffect(() => {
    const fetchProfilePicture = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const res = await axios.get(
          `${API_BASE_URL}/personalinfo/${employeeNumber}`,
          { headers: { Authorization: `Bearer ${token}` } },
        );
        ...
        const fullNameFromPerson =
          `${data.firstName || ''} ${data.middleName || ''} ${data.lastName || ''} ${data.nameExtension || ''}`.trim();
  ...
  const isDateInHolidayRange = useCallback(
    (dateStr) => {
      ...
      for (const raw of rawHolidays) {
        ...
        if (check >= start && check <= end) {
          return {
            name: raw.description || raw.title || '',
            status: raw.status,
          };
        }
      }
  ...
  const quickActions = [
    ...
    {
      icon: <ContactPage sx={{ fontSize: 16 }} />,
      label: 'PDS',
      link: '/pds1',
    },
    {
  ...
  const getCarouselItemForNotif = useCallback(
    (notif) => {
      ...
      if (type === 'holiday') {
        return (
          scheduledHolidaysForCarousel.find((h) => {
        ...
        <Box sx={{ pt: 4, px: 4, mx: 'auto', maxWidth: '1600px' }}>
          ...
          <Grid container spacing={2} sx={{ flex: 1, minHeight: 0 }}>
                    ...
                    {announcementsLoading ? (
                      ...
                      >
                        <Flag
                          sx={{
                            fontSize: 80,
                            color: `${settings.primaryColor}4D`,
                          }}
                        />
                        <Typography
                          variant="h5"
            ...
            <Grid
              ...
              sx={{
                minHeight: 0,
                height: { xs: '65vh', md: 'calc(100vh - 230px)' },
                display: 'flex',
                flexDirection: 'column',
                      ...
                      >
                        ...
                        <Grid container spacing={0.3}>
                          {calendarDays.map((day, index) => {
                            const currentDateStr = buildDateStr(
                              year,
                              month,
                              day,
                            );
                            const holidayData = day
                              ? isDateInHolidayRange(currentDateStr)
                              : null;
                            const dayNotes = day
                              ? getNotesForDate(currentDateStr)
                              : [];
                            const dayEvents = day
                              ? getEventsForDate(currentDateStr)
                              : [];
                            const dayAnnouncements = day
                              ? getAnnouncementsForDate(currentDateStr)
                              : [];
                            ...
                            return (
                              <Grid item xs={12 / 7} key={index}>
                                <Tooltip
                                  title={tooltipTitle}
                                  arrow
                                  disableHoverListener={!tooltipTitle || !day}
                                >
                                  <Box
                                    onClick={() => {
                                      if (day) {
                        ...
                        <Box
                          sx={{
                            ...
                            gap: 0.5,
                            borderRadius: 1,
                            bgcolor: `${settings.primaryColor}0A`,
                            p: 0.5,
                            mt: 0.75,
                          }}
                        >
                          ...
                          >
                            <CalendarMonth sx={{ fontSize: 13 }} /> Click day to
                            view / add
                          </Typography>
                          <Tooltip title="Legends">
                            <IconButton
                              size="small"
                              onClick={(e) =>
                                setCalendarLegendAnchorEl(e.currentTarget)
                              }
                              sx={{
                                color: settings.textPrimaryColor,
                                p: 0.4,
                                ...
                                sx={{
                                  ...
                                  backgroundColor:
                                    item.type === 'note'
                                      ...
                                      : '#e8f5e9',
                                  borderRadius: 1,
                                  borderLeft: `2px solid ${item.type === 'note' ? settings.primaryColor : '#4caf50'}`,
                                  cursor: 'pointer',
                                  transition: 'all 0.2s',
                                  '&:hover': {
                                    transform: 'translateX(2px)',
                                    boxShadow: `2px 0 8px ${settings.primaryColor}20`,
                                  },
                                }}
                                onClick={() => {
                        ...
                        >
                          ...
                          <FormControl size="small" sx={{ minWidth: 70 }}>
                            ...
                            <Select
                              ...
                              label="Month"
                              onChange={(e) =>
                                setPayslipMonth(Number(e.target.value))
                              }
                              sx={{
                                color: settings.textPrimaryColor,
                                fontSize: '0.7rem',
                          ...
                          {[
                            {
                              label: '1st Quinceña',
                              key: 'pay1st',
                              grad: `linear-gradient(135deg, ${settings.primaryColor} 0%, ${settings.secondaryColor} 100%)`,
                            },
                            {
                              label: '2nd Quinceña',
                              key: 'pay2nd',
                              grad: `linear-gradient(135deg, ${settings.secondaryColor} 0%, ${settings.primaryColor} 100%)`,
                            },
                          ].map(({ label, key, grad }) => (
                            <Grid item xs={6} key={key}>
                              ...
                              >
                                ...
                                >
                                  {payrollData
                                    ? (() => {
                                        const n = parseFloat(payrollData[key]);
                                        return !isNaN(n) && n !== 0
                      ...
                      >
                        ...
                        >
                          ...
                          {!leaveLoading && leaveCredits.length > 0 && (
                            <Chip
                              label={`${leaveCredits.reduce((s, g) => s + g.currRemaining + g.prevRemaining, 0).toFixed(1)} Days`}
                              size="small"
                              sx={{
                          ...
                          {leaveLoading ? (
                            ...
                            >
                              {leaveCredits.map((leave, idx) => {
                                const pct =
                                  leave.currTotal > 0
                                    ? (leave.currRemaining / leave.currTotal) *
                                      100
                                    : 0;
                                ...
                                const usedDays =
                                  leave.currAllocated - leave.currRemaining;
                                return (
                                  <Box
                                    key={idx}
                                    sx={{
                                      ...
                                      bgcolor: `${settings.primaryColor}05`,
                                    }}
                                  >
                                    <Box
                                      sx={{
                                    ...
                                    >
                                      ...
                                      >
                                        ...
                                        used / {leave.currAllocated.toFixed(1)}{' '}
                                        total
                                      </Typography>
                                    </Box>
                                    <LinearProgress
                                    ...
                                    {leave.prevRemaining > 0 && (
                                      ...
                                      >
                                        ...
                                        >
                                          +{leave.prevRemaining.toFixed(1)} days
                                          carried over
                                        </Typography>
                                      </Box>
                                    )}
            ...
            <Fade in={openModal}>
              <Box
                sx={{
                  ...
                  borderRadius: '18px',
                  boxShadow:
                    '0 48px 120px rgba(0,0,0,0.42), 0 8px 32px rgba(0,0,0,0.18)',
                  maxHeight: '90vh',
                  display: 'flex',
                  flexDirection: 'column',
                  overflow: 'hidden',
                      ...
                      <>
                        ...
                        <Box
                          sx={{
                            ...
                            gap: 1,
                            flexWrap: 'wrap',
                            borderBottom: `1px solid ${settings.primaryColor}18`,
                            flexShrink: 0,
                          }}
                        >
                          {dateChip && (
                        ...
                        <Box
                          sx={{
                            ...
                            '&::-webkit-scrollbar-thumb': {
                              bgcolor: `${settings.primaryColor}28`,
                              borderRadius: 2,
                            },
                          }}
                        >
                        ...
                        <Box
                          sx={{
                            px: { xs: 2.5, md: 3 },
                            py: 1.5,
                            borderTop: `1px solid ${settings.primaryColor}18`,
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'space-between',
                            flexShrink: 0,
                          ...
                          >
                            {[
                              ...
                              >
                                ...
                                />
                                <Typography
                                  sx={{ fontSize: '0.68rem', color: '#6B7280' }}
                                >
                                  {label}
                                </Typography>
                              </Box>
            ...
            <Fade in={notifModalOpen}>
              <Box
                sx={{
                  ...
                  boxShadow: `0 16px 48px ${settings.primaryColor}28`,
                  borderRadius: '16px',
                  overflow: 'hidden',
                }}
              >
                <Box
                  sx={{
                    px: 2,
                    py: 1.25,
                    background: `linear-gradient(135deg, ${settings.secondaryColor} 0%, ${settings.primaryColor} 55%, ${settings.secondaryColor} 100%)`,
                    flexShrink: 0,
                  }}
                >
                ...
                <Button
                  ...
                  sx={{
                    ...
                    fontWeight: 600,
                    color: settings.secondaryColor,
                    bgcolor: `${settings.primaryColor}0D`,
                    borderBottom: '1px solid #efe4e4',
                    '&:hover': { bgcolor: `${settings.primaryColor}16` },
                  }}
                >
                  Mark all as read
                </Button>
                <Box
                  sx={{
                    ...
                    '&::-webkit-scrollbar': { width: 4 },
                    '&::-webkit-scrollbar-thumb': {
                      bgcolor: `${settings.primaryColor}40`,
                      borderRadius: 4,
                    },
                  }}
                >
                        ...
                        .map((notif, idx, arr) => {
                          ...
                          const TYPE_CONFIG = {
                            ...
                            announcement: {
                              ...
                              iconBg: `${settings.primaryColor}18`,
                            },
                          };
                          ...
                          const ICON_MAP = {
                            payslip: (
                              <Receipt
                                sx={{ fontSize: 15, color: cfg.accent }}
                              />
                            ),
                            contact: (
                              <ContactPage
                            ...
                            suspension: (
                              <Event sx={{ fontSize: 15, color: cfg.accent }} />
                            ),
                            announcement: (
                              <NotificationsIcon
                                sx={{ fontSize: 15, color: cfg.accent }}
                              ...
                              <React.Fragment key={`notif-wrap-${notif.id}`}>
                                ...
                                <Box
                                  ...
                                  sx={{
                                    ...
                                    bgcolor: isRead
                                      ? '#ffffff'
                                      : `${settings.primaryColor}06`,
                                    cursor: 'pointer',
                                    transition: 'background 0.12s',
                                    '&:hover': {
                                      bgcolor: isRead
                                        ? '#fdf8f8'
                                        : `${settings.primaryColor}0E`,
                                    },
                                    '&:last-child': { borderBottom: 'none' },
                                    borderLeft: isRead
                                      ? 'none'
                                      : `3px solid ${settings.primaryColor}`,
                                  }}
                                >
                                  {isAnnouncementCard ? (
                                    <Box
                                      ...
                                      {itemImage ? (
                                        ...
                                        <Box
                                          sx={{
                                            ...
                                            height: '100%',
                                            background:
                                              'linear-gradient(135deg,#3d0c6b,#1a0a3d)',
                                          }}
                                        />
                                      )}
                                      <Box
                                      ...
                                      {!isRead && (
                                        <Box
                                          sx={{
                                            ...
                                            borderRadius: '50%',
                                            bgcolor: '#fff',
                                            outline: `2px solid ${settings.secondaryColor}`,
                                          }}
                                        />
                                      )}
                                      {itemTitle && (
                                    ...
                                    <Box
                                      sx={{
                                        ...
                                        px: 1.75,
                                        gap: 1.5,
                                        border: `1px solid ${notif.notification_type === 'holiday' ? 'rgba(237,108,2,0.3)' : 'rgba(211,47,47,0.25)'}`,
                                        bgcolor:
                                          notif.notification_type === 'holiday'
                                            ? 'rgba(237,108,2,0.12)'
                                            : 'rgba(211,47,47,0.09)',
                          ...
                          const ticketEntry = isContact
                            ? (contactTicketStatuses || {})[notif.id]
                            : null;
                          const ticketSubject = ticketEntry?.subject || '';
                          const ticketEmpName =
                            ticketEntry?.employee_name || '';
                          const ticketEmpNum =
                            ticketEntry?.employee_number ||
                      ...
                      >
                        ...
                        >
                          <NotificationsIcon
                            sx={{
                              fontSize: 24,
                              color: `${settings.primaryColor}80`,
                            }}
                          />
                        </Box>
                        <Typography
                ...
                >
                  ...
                  {[
                    ...
                    >
                      ...
                      />
                      <Typography sx={{ fontSize: '0.64rem', color: '#666' }}>
                        {item.label}
                      </Typography>
                    </Box>
                  ))}
            ...
            <Box sx={{ px: 2, py: 1.5 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                {[
                  ['#ff9800', 'Notes'],
                  ['#4caf50', 'Events'],
                  [settings.primaryColor, 'Holidays'],
                  [settings.secondaryColor, 'Today'],
                ].map(([color, label]) => (
                  <Box
                    key={label}
          ...
          <Dialog
            ...
            PaperProps={{
              sx: {
                ...
                border: `1px solid ${settings.primaryColor}26`,
                boxShadow: `0 24px 64px ${settings.primaryColor}33`,
              },
            }}
          >
            <DialogTitle
            ...
            <DialogContent sx={{ pt: 1 }}>
              ...
              {(() => {
                const dateAnnouncements = announcements.filter(
            ...
            <DialogActions sx={{ px: 3, pb: 2, gap: 0.5 }}>
              ...
              <Button
                ...
                sx={{
                  ...
                  fontWeight: 500,
                  color: settings.textPrimaryColor,
                  border: `1.5px solid ${settings.primaryColor}40`,
          ...
          >
            ...
            <DialogContent sx={{ pt: 1 }}>
              ...
              <Typography variant="caption" color="textSecondary">
                ...
                {currentNote.date &&
                  (() => {
                    const [y, m, d] = currentNote.date.split('-').map(Number);
