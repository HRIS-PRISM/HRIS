  ...
  <Fade in={visible} timeout={600} unmountOnExit>
    ...
    >
      ...
      <Box sx={{ flex: 1 }}>
        ...
        {retryIn > 0 && (
          <Typography
            sx={{ fontSize: "0.7rem", color: alpha(T.accent, 0.45), mt: 0.3 }}
          >
    ...
    >
      <Box
        sx={{
          ...
          animation: "ulBlink 2s ease-in-out infinite",
        }}
      >
        <Box
          sx={{
        ...
        {[1, 2, 3, 4, 5].map((i) => (
          <Box
            ...
            sx={{
              ...
              bgcolor: "#fff",
              p: 2.5,
              animation: `ulBlink 2s ease-in-out ${i * 0.08}s infinite`,
            }}
          >
      ...
      <Box
        sx={{
          ...
          bgcolor: "#fff",
          overflow: "hidden",
          animation: "ulBlink 2s ease-in-out 0.2s infinite",
...
const getUserRole = () => {
  try {
    ...
    return (
      JSON.parse(jsonPayload).role || JSON.parse(jsonPayload).userRole || null
    );
  } catch {
    return null;
  }
...
const hasPagePrivilege = (accessMap, pageId) =>
  !!accessMap[normalizePageId(pageId)];
    ...
    (async () => {
      try {
        ...
        if (r.data && typeof r.data === "object") {
          setSettings(r.data);
          localStorage.setItem("systemSettings", JSON.stringify(r.data));
        }
      } catch {}
    })();
  }, []);
...
const DialogAccentBar = () => (
  <Box
    sx={{
      height: 4,
      background: `linear-gradient(90deg, ${T.accent} 0%, ${T.accentMid} 60%, ${alpha(T.accent, 0.4)} 100%)`,
    }}
  />
);
...
const UsersList = () => {
  ...
  const getBranchLabel = (code) => {
    ...
    const found = BRANCHES.find((b) => b.code === Number(code));
    ...
    () =>
      styled(TextField)(() => ({
        "& .MuiOutlinedInput-root": {
          ...
          "&.Mui-focused": {
            backgroundColor: "#fff",
            boxShadow: `0 0 0 3px ${alpha(p, 0.12)}`,
          },
  ...
  const SharpTableContainer = useMemo(
    () =>
  ...
  const properUsers = useMemo(
    () =>
      users.filter(
        (u) =>
          u.fullName && u.fullName.trim() !== "" && u.fullName !== "Username",
      ),
    [users],
  );
  ...
  // ─── PW modal derived ──────────────────────────────────────────────────────
  const pwProperUsers = useMemo(
    () => pwUsers.filter((u) => u.fullName && u.fullName.trim() !== ""),
    [pwUsers],
  );
  const pwIncompleteUsers = useMemo(
    () => pwUsers.filter((u) => !u.fullName || u.fullName.trim() === ""),
    [pwUsers],
  );
  ...
  const fetchPwUsers = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setPwErrMessage(e.error || "Failed to fetch users");
        setPwUsers([]);
        return;
      }
  ...
  const handleResetPassword = async (employeeNumber) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setPwSuccessAction("reset");
        setPwSuccessOpen(true);
      } else {
        setPwErrMessage(data.error || "Failed to reset password");
      }
    } catch {
  ...
  const pwPaginatedUsers = pwFilteredUsers.slice(
    pwPage * pwRowsPerPage,
    pwPage * pwRowsPerPage + pwRowsPerPage,
  );
  ...
  const getPwRoleStyle = (role = "") => {
    switch ((role || "").toLowerCase()) {
      case "superadmin":
        return { color: T.accent, bg: T.accentFaint };
      case "administrator":
        return { color: T.accentMid, bg: "rgba(139,69,69,0.08)" };
      case "technical":
        return { color: "#1565C0", bg: "#E3F2FD" };
      case "staff":
        return { color: "#2E7D32", bg: "#E8F5E9" };
      default:
        return { color: T.muted, bg: "rgba(0,0,0,0.05)" };
    }
  };
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const remaining =
        new Date(temporaryPasswordInfo.validUntil).getTime() - Date.now();
      if (remaining <= 0) {
  ...
  useEffect(() => {
    if (moduleAuthorized && !isTechnicalUser) {
      fetchUsers();
      fetchTypeConfigs();
    }
    ...
    try {
      ...
      if (accessResponse.ok) {
        ...
        if (pagesResponse.ok) {
          ...
          const eligiblePages = pagesData.filter((page) =>
            isPageAuthorizedForRole(page, user.role),
          );
          const accessiblePages = pagesData.filter((page) =>
            hasPagePrivilege(accessMap, page.id),
          );
          ...
          const percentage =
            eligiblePages.length > 0
              ? (accessiblePages.length / eligiblePages.length) * 100
              : 0;
          let current = 0;
  ...
  const applyPageAccessChange = async (
    pageId,
    newAccess,
    pageRow,
    expiresAt = null,
  ...
  const handleBranchChange = (user, newBranchCode) => {
    const normalizedCurrent =
      user.branch === null || user.branch === undefined
        ? ""
        : Number(user.branch);
    if (normalizedCurrent === newBranchCode) return;
    setPendingBranchChange({
      user,
      oldBranch: user.branch,
  ...
  const confirmBranchChange = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/users/${pendingBranchChange.user.employeeNumber}/branch`,
        {
          method: "PUT",
          ...authHeaders,
          body: JSON.stringify({ branch: pendingBranchChange.newBranch }),
        },
      );
      if (!response.ok) {
        const e = await response.json().catch(() => ({}));
        setError(e.error || "Failed to update user branch");
        setBranchChangeDialog(false);
      ...
      setUsers((prev) =>
        prev.map((u) =>
          u.employeeNumber === pendingBranchChange.user.employeeNumber
            ? { ...u, branch: pendingBranchChange.newBranch }
            : u,
        ),
      );
      ...
      setError("Network error while updating user branch");
    } finally {
      setBranchChangeLoading(false);
    }
  };
  ...
  const handleStatusChange = (user, newStatus) => {
    ...
    setPendingStatusChange({ user, oldStatus: user.status, newStatus });
  ...
  const confirmStatusChange = async () => {
    ...
    try {
      const authHeaders = getAuthHeaders();
      const response = await fetch(
        `${API_BASE_URL}/users/${pendingStatusChange.user.employeeNumber}/status`,
        {
          method: "PUT",
          ...authHeaders,
      ...
      if (!response.ok) {
        const e = await response.json().catch(() => ({}));
        setError(e.error || "Failed to update user status");
      ...
      setUsers((prev) =>
        prev.map((u) =>
          u.employeeNumber === pendingStatusChange.user.employeeNumber
            ? { ...u, status: pendingStatusChange.newStatus }
            : u,
        ),
      );
      setFilteredUsers((prev) =>
  ...
  const toggleSelectEmployee = (n) =>
    setSelectedEmployeeNumbers((prev) =>
      prev.includes(n) ? prev.filter((x) => x !== n) : [...prev, n],
    );
  const isEmployeeSelected = (n) => selectedEmployeeNumbers.includes(n);
  const isAllCurrentPageSelected = (cur) =>
    cur?.length > 0 &&
    cur.every((u) => selectedEmployeeNumbers.includes(u.employeeNumber));
  const isSomeCurrentPageSelected = (cur) => {
    const any = cur?.some((u) =>
      selectedEmployeeNumbers.includes(u.employeeNumber),
    );
    const all = cur?.every((u) =>
      selectedEmployeeNumbers.includes(u.employeeNumber),
    );
    return any && !all;
  };
  const toggleSelectAllCurrentPage = (cur) => {
    ...
    if (isAllCurrentPageSelected(cur)) {
      const set = new Set(cur.map((u) => u.employeeNumber));
      setSelectedEmployeeNumbers((prev) => prev.filter((n) => !set.has(n)));
    } else {
      setSelectedEmployeeNumbers((prev) => {
        const set = new Set(prev);
        cur.forEach((u) => set.add(u.employeeNumber));
  ...
  const handleSaveBulkCategoryEdit = async () => {
    ...
    try {
      ...
      setError(
        err?.message || "Network error while bulk updating employment category",
      );
    } finally {
  ...
  const handleGrantRoleAccess = async (role) => {
    ...
    try {
      ...
      setError("Network error while granting role access");
    } finally {
    ...
    (user) => {
      ...
      if (
        ...
        const cfg = typeConfigs.find(
          (t) => String(t.id) === String(fallbackId),
        );
        if (cfg) {
          const label =
          ...
          <Box sx={{ p: 4 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 3 }}>
              ...
              <Box>
                ...
                <Typography
                  sx={{ fontSize: "0.78rem", color: T.muted, mt: 0.25 }}
                >
                  User Management · Restricted Module
                </Typography>
              </Box>
            </Box>
            <Box
            ...
            >
              ...
              >
                Use the <strong>current temporary password</strong> below (it
                rotates automatically). The permanent confidential password from
                System Administration also works.
              </Typography>
              <Box
                sx={{
                  p: 2,
              ...
              >
                ...
                {tempPasswordLoading && !temporaryPasswordInfo ? (
                  ...
                  <Typography sx={{ fontSize: "0.8rem", color: T.muted }}>
                    Temporary password unavailable. Enter the permanent
                    confidential password or contact superadmin.
                  </Typography>
                )}
              </Box>
            </Box>
            <FieldInput
              ...
              sx={{
                mb: 3,
                "& input": { fontFamily: "monospace", letterSpacing: "0.08em" },
              }}
        ...
        >
          ...
          >
            ...
            <Box>
              ...
              >
                Administrative Panel • Accounts · Roles · Page Permissions
              </Typography>
            </Box>
          </Box>
          <Box
          ...
          >
            <AccentButton
              variant="contained"
              startIcon={<LockResetIcon sx={{ fontSize: "15px !important" }} />}
              onClick={openPwMgmt}
              sx={{
                fontSize: "0.8rem",
                bgcolor: T.accent,
            ...
            {isTechnical && (
              <AccentButton
                variant="contained"
                startIcon={<Pages sx={{ fontSize: "15px !important" }} />}
    ...
    sx: {
      ...
      <SectionCard sx={{ overflow: "hidden" }}>
        ...
        >
          <Box>
            ...
            <Typography sx={{ fontSize: "0.72rem", color: T.faint, mt: 0.1 }}>
              ...
              categoryFilter !== "" ||
              departmentFilter !== ""
                ? `Showing ${filteredUsers.length} of ${tableTab === 0 ? properUsers.length : incompleteUsers.length}`
          ...
          >
            ...
            {["staff", "administrator", "superadmin"].map((role) => (
              <Tooltip
                key={role}
                title={`Grant default access to all ${role} users`}
              >
                <AccentButton
                  variant="outlined"
                  size="small"
                  startIcon={
                    grantingRole === role ? (
                      <CircularProgress
                        size={11}
                        sx={{ color: roleGrantColors[role].color }}
                      />
                    ) : null
                  }
                  ...
                  sx={{
                    ...
                    py: 0.35,
                    height: 26,
                    borderColor: alpha(roleGrantColors[role].color, 0.35),
                    color: roleGrantColors[role].color,
                    bgcolor: roleGrantColors[role].bgcolor,
                    "&:hover": {
                      bgcolor: alpha(roleGrantColors[role].color, 0.15),
                      borderColor: roleGrantColors[role].color,
                      transform: "none",
                    },
                  }}
                >
            ...
            {isTechnical && (
              <Tooltip title="Bulk Edit Employment Category">
                <AccentButton
                  variant="outlined"
                  size="small"
                  startIcon={<Category sx={{ fontSize: 13 }} />}
        ...
        {tableTab === 1 && incompleteUsers.length > 0 && (
          ...
          >
            <WarningAmberRounded
              sx={{ fontSize: 13, color: "#F57C00", flexShrink: 0 }}
            />
            <Typography
              sx={{
                fontSize: "0.72rem",
            ...
            >
              These accounts are missing a full name. Use the edit action to
              link them to an employee record.
            </Typography>
          </Box>
        )}
              ...
              <TableRow>
                ...
                >
                  <Checkbox
                    ...
                    onChange={() => toggleSelectAllCurrentPage(paginatedUsers)}
                    sx={{
                      color: T.faint,
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell sx={{ py: 1, px: 1.25, borderBottom: "none" }}>
                        ...
                        >
                          <Circle
                            sx={{ fontSize: 7, color: categoryInfo.color }}
                          />
                          <Typography
                            sx={{
                              fontSize: "0.72rem",
                      ...
                      <TableCell sx={{ py: 1, px: 1.25, borderBottom: "none" }}>
                        <Select
                          ...
                          onChange={(e) =>
                            handleBranchChange(user, e.target.value)
                          }
                          displayEmpty
                          size="small"
                          sx={{
                      ...
                      <TableCell sx={{ py: 1, px: 1.25, borderBottom: "none" }}>
                        ...
                        >
                          ...
                          <Typography
                            sx={{ fontSize: "0.78rem", color: T.muted }}
                          >
                      ...
                      >
                        <AccentButton
                          variant="outlined"
                          size="small"
                          startIcon={<Security sx={{ fontSize: 13 }} />}
      ...
      >
        <ModalHeader
          ...
          subtitle={`${pwUsers.length} users registered • Resets password to surname (ALL CAPS)`}
          onClose={closePwMgmt}
        />
        ...
        >
          <Box sx={{ display: "flex", gap: 1.25, flexShrink: 0 }}>
            {[
              ...
              {
                label: `${pwIncompleteUsers.length} missing`,
                color: "#F57C00",
              },
            ].map(({ label, color }) => (
              <Box
              ...
              >
                <Typography
                  sx={{ fontSize: "0.75rem", color, fontWeight: 700 }}
                >
                  {label}
                </Typography>
              </Box>
          ...
          {pwSuccessOpen && (
            <Fade in>
              <Alert
                severity="success"
                icon={<CheckCircle />}
                onClose={() => setPwSuccessOpen(false)}
                sx={{ borderRadius: 2, flexShrink: 0 }}
              >
          ...
          >
            ...
            >
              ...
              >
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.25 }}>
                  ...
                  <Typography
                    sx={{ fontSize: "0.85rem", fontWeight: 700, color: T.text }}
                  >
                    Account Records
                  </Typography>
                  {pwNavSection === "incomplete" &&
                    pwIncompleteUsers.length > 0 && (
                      ...
                      >
                        ...
                        >
                          {pwIncompleteUsers.length} missing
                        </Typography>
                      </Box>
                    )}
                </Box>
                <Typography sx={{ fontSize: "0.7rem", color: T.faint }}>
                  {pwFilteredUsers.length} of {pwSourceUsers.length} shown
                </Typography>
              </Box>
              ...
              >
                {[
                  { key: "all", label: "All", count: pwUsers.length },
                  {
                    key: "accounts",
                    label: "Complete",
                    count: pwProperUsers.length,
                  },
                  {
                    key: "incomplete",
                    label: "Incomplete",
                    count: pwIncompleteUsers.length,
                  },
                ].map((t) => (
                  <Tab
                    key={t.key}
                        ...
                        >
                          <Typography
                            sx={{
                              ...
                              fontWeight: 900,
                              color:
                                pwNavSection === t.key ? T.accent : T.faint,
                            }}
                          >
                            {t.count}
                          </Typography>
            ...
            {pwNavSection === "incomplete" && pwIncompleteUsers.length > 0 && (
              ...
              >
                ...
                >
                  These accounts are missing a full name. They can still have
                  their password reset, but should be updated in employee
                  records.
                </Typography>
              </Box>
            )}
            ...
            >
              <Table sx={{ minWidth: 600 }} stickyHeader>
                <TableHead>
                  <TableRow>
                    {["Emp. No", "Full Name", "Email", "Role", "Action"].map(
                      (h, i) => (
                        <TableCell
                          key={h}
                          sx={{
                  ...
                  {pwLoading && pwUsers.length === 0 ? (
                    Array.from({ length: 6 }).map((_, idx) => (
                      ...
                      >
                        ...
                        <TableCell
                          sx={{ px: 2.5, py: 1.75, textAlign: "center" }}
                        >
                          <Box
                        ...
                        >
                          ...
                          >
                            <AccentButton
                              ...
                              size="small"
                              disabled={
                                pwResetting[user.employeeNumber] || !user.email
                              }
                              onClick={() =>
                                handleResetPassword(user.employeeNumber)
                              }
                              startIcon={
                                pwResetting[user.employeeNumber] ? (
                                  <CircularProgress
                                    size={11}
                                    sx={{ color: "#fff" }}
                                  />
                    ...
                    <TableRow>
                      ...
                      >
                        ...
                        <Typography
                          sx={{ color: T.faint, fontSize: "0.78rem" }}
                        >
                          {pwSearchTerm
            ...
            {pwFilteredUsers.length > 0 && (
              ...
              >
                <TablePagination
                  ...
                  onRowsPerPageChange={(e) => {
                    setPwRowsPerPage(parseInt(e.target.value, 10));
                    setPwPage(0);
                  }}
      ...
      >
        ...
        >
          ...
          >
            <VerifiedUser sx={{ fontSize: 18, color: T.accent }} />
          </Box>
        ...
        <Box sx={{ px: 3, py: 2.5 }}>
          ...
          >
            ...
            {confirmRole && (
              ...
              >
                {confirmRole.charAt(0).toUpperCase() + confirmRole.slice(1)}
              </Box>
            )}{" "}
            users?
          </Typography>
        ...
        >
          ...
          <AccentButton
            variant="contained"
            onClick={() => {
              handleGrantRoleAccess(confirmRole);
              setConfirmRole(null);
            }}
            disabled={grantingRole !== null}
            sx={{
      ...
      <Dialog
        open={!!grantSuccessDialog}
        onClose={() => setGrantSuccessDialog(null)}
        maxWidth="xs"
        fullWidth
        PaperProps={{ sx: { borderRadius: 3, overflow: "hidden" } }}
      >
        ...
        <Box sx={{ px: 4, py: 4, textAlign: "center" }}>
          ...
          >
            <CheckCircle sx={{ fontSize: 26, color: T.accent }} />
          </Box>
          <Typography
            sx={{ fontSize: "1rem", fontWeight: 700, color: T.text, mb: 0.5 }}
          >
            Access Granted
          </Typography>
          <Typography sx={{ fontSize: "0.78rem", color: T.muted }}>
            Default pages successfully assigned
          </Typography>
        </Box>
        <Box sx={{ px: 3.5, pb: 3 }}>
          {[
            {
              label: "Role",
              value:
                grantSuccessDialog?.role?.charAt(0).toUpperCase() +
                grantSuccessDialog?.role?.slice(1),
              pill: true,
            },
            {
              label: "Users updated",
              value: `${grantSuccessDialog?.usersProcessed} users`,
            },
            {
              label: "Pages granted",
              value: `${grantSuccessDialog?.pagesGranted} pages`,
            },
            ...
            >
              <Typography
                sx={{ fontSize: "0.82rem", fontWeight: 600, color: T.muted }}
              >
                {label}
              </Typography>
              {pill ? (
                <Chip
                  ...
                  size="small"
                  sx={{
                    bgcolor: roleGrantColors[grantSuccessDialog?.role]?.bgcolor,
                    color: roleGrantColors[grantSuccessDialog?.role]?.color,
                    fontWeight: 700,
                    fontSize: "0.72rem",
                  }}
                />
          ...
          <AccentButton
            fullWidth
            variant="contained"
            onClick={() => setGrantSuccessDialog(null)}
            sx={{
              mt: 1.5,
              bgcolor: T.accent,
              color: "#fff",
          ...
          {selectedUser && (
            ...
            >
              ...
              >
                ...
                >
                  {!pageAccessLoading &&
                    ...
                    (() => {
                      ...
                      const categoryIcons = {
                        General: <Category sx={{ fontSize: 13 }} />,
                        "System Administration": (
                          <Settings sx={{ fontSize: 13 }} />
                        ),
                        Registration: <Assignment sx={{ fontSize: 13 }} />,
                        "Information Management": (
                          <Info sx={{ fontSize: 13 }} />
                        ),
                        "Attendance Management": (
                          <Assessment sx={{ fontSize: 13 }} />
                        ),
                        "Payroll Management": <Payment sx={{ fontSize: 13 }} />,
                        Form: <Description sx={{ fontSize: 13 }} />,
                        "Pages Management": <Pages sx={{ fontSize: 13 }} />,
                        "Personal Data Sheets": (
                          <Folder sx={{ fontSize: 13 }} />
                        ),
                        Uncategorized: <FolderSpecial sx={{ fontSize: 13 }} />,
                      };
                      return sortedDescs.map((desc) => {
                        const isActive = activeAccessCategory === desc;
                        const pagesInGroup = groupedPages[desc] || [];
                        const eligiblePages = pagesInGroup.filter((pg) =>
                          isPageAuthorizedForRole(pg, selectedUser?.role),
                        );
                        const enabledInGroup = eligiblePages.filter((pg) =>
                          hasPagePrivilege(pageAccess, pg.id),
                        ).length;
                        const allEnabled =
                          enabledInGroup === eligiblePages.length &&
                          eligiblePages.length > 0;
                        return (
                          <Box
                            key={desc}
                ...
                >
                  <Typography
                    sx={{ fontSize: "0.78rem", fontWeight: 600, color: T.text }}
                  >
                    Toggle All
                  </Typography>
                  <Switch
                    ...
                    checked={
                      ...
                      (() => {
                        const eligible = pages.filter(
                          (pg) =>
                            isPageAuthorizedForRole(pg, selectedUser?.role) &&
                            !isAssignmentManagedPage(pg),
                        );
                        return (
                          eligible.length > 0 &&
                          eligible.every(
                            (pg) =>
                              hasPagePrivilege(pageAccess, pg.id) === true,
                          )
                        );
                      })()
                    }
                    onChange={(e) => {
                      ...
                      eligible.forEach((page) => {
                        if (hasPagePrivilege(pageAccess, page.id) !== enableAll)
                          handleTogglePageAccess(page.id, !enableAll, page);
                      });
                    }}
                    sx={{
              ...
              >
                {pageAccessLoading ? (
                  ...
                  (() => {
                    ...
                    const enabledCount = eligiblePages.filter((pg) =>
                      hasPagePrivilege(pageAccess, pg.id),
                    ).length;
                    return (
                      <Fade
                          ...
                          >
                            ...
                            >
                              ...
                              <Box>
                                ...
                                <Typography
                                  sx={{ fontSize: "0.68rem", color: T.faint }}
                                >
                                  {enabledCount} of {eligiblePages.length}{" "}
                                  enabled
                                </Typography>
                              </Box>
                            </Box>
                            ...
                            >
                              ...
                              <Switch
                                size="small"
                                checked={(() => {
                                  const manual = eligiblePages.filter(
                                    (pg) => !isAssignmentManagedPage(pg),
                                  );
                                  return (
                                    manual.length > 0 &&
                                    manual.every((pg) =>
                          ...
                          >
                            ...
                            >
                              {pagesInGroup.map((page) => {
                                const assignmentManaged =
                                  isAssignmentManagedPage(page);
                                const inScope = isPageAuthorizedForRole(
                                  page,
                                  selectedUser?.role,
                                );
                                const isEnabled = hasPagePrivilege(
                                  pageAccess,
                                  page.id,
                                );
                                const isTemporary =
                                  isEnabled && !inScope && !assignmentManaged;
                                  ...
                                  >
                                    <Box sx={{ flex: 1, minWidth: 0 }}>
                                      ...
                                      {assignmentManaged && (
                                        ...
                                        >
                                          Managed via Supervisor Assignment —
                                          not editable here
                                        </Typography>
                                      )}
                                      {isTemporary && (
                                      ...
                                      {!assignmentManaged &&
                                        ...
                                        !isEnabled && (
                                          ...
                                          >
                                            Outside {selectedUser?.role} role —
                                            enable to open confirmation
                                          </Typography>
                                        )}
                                    </Box>
                                    <Box
                                      ...
                                      {accessChangeInProgress[
                                        ...
                                        <>
                                          ...
                                          <Tooltip
                                            title={
                                              assignmentManaged
                                                ? "Use Supervisor Assignment to grant or revoke this page"
                                                : inScope
                                                  ? ""
                                                  : "Opens a warning — temporary access outside role (max 1 day)"
                                            }
                                          >
                                            <span>
                                              <Switch
                                                checked={isEnabled}
                                                disabled={assignmentManaged}
                                                onChange={() =>
                                                  handleTogglePageAccess(
                                                    page.id,
                                                    isEnabled,
              ...
              <Box
                sx={{
                  ...
                  flexShrink: 0,
                  bgcolor: T.surface,
                  borderLeft: `2px dashed ${alpha("#16a34a", 0.3)}`,
                  display: "flex",
                  flexDirection: "column",
                  height: "100%",
                  overflow: "hidden",
              ...
              >
                <Box
                  sx={{
                    px: 2.5,
                    py: 2,
                    borderBottom: `1px solid ${alpha("#16a34a", 0.1)}`,
                ...
                >
                  ...
                  <Typography
                    sx={{ fontSize: "0.7rem", color: T.muted, pl: 2.25 }}
                  >
                    {!pageAccessLoading && pages.length > 0
                      ? (() => {
                          const enabled = pages.filter((pg) =>
                            hasPagePrivilege(pageAccess, pg.id),
                          ).length;
                          const temp = pages.filter(
                            (pg) =>
                              hasPagePrivilege(pageAccess, pg.id) &&
                              isOutOfRoleScopePage(pg, selectedUser?.role),
                          ).length;
                          return `${enabled} active${temp ? ` (${temp} temporary)` : ""}`;
                        })()
                      : "—"}
                  </Typography>
                </Box>
                ...
                >
                  {pages.filter((pg) => hasPagePrivilege(pageAccess, pg.id))
                    .length > 0 ? (
                    <Box
                      sx={{
                        px: 1.5,
                    ...
                    >
                      {pages
                        .filter((pg) => hasPagePrivilege(pageAccess, pg.id))
                        .map((page) => {
                          const isInActiveCategory =
                            activeAccessCategory &&
                            (page.page_description || "Uncategorized") ===
                              activeAccessCategory;
                          const isTemp = isOutOfRoleScopePage(
                            page,
                            selectedUser?.role,
                          );
                          return (
                            <Box
                              ...
                              sx={{
                                ...
                                bgcolor: isInActiveCategory
                                  ? alpha(isTemp ? "#c2410c" : "#16a34a", 0.1)
                                  : alpha(isTemp ? "#c2410c" : "#16a34a", 0.04),
                                border: `1px solid ${isInActiveCategory ? alpha(isTemp ? "#c2410c" : "#16a34a", 0.25) : alpha(isTemp ? "#c2410c" : "#16a34a", 0.1)}`,
                                transition: "all 0.15s",
                              }}
                            >
                              <CheckCircle
                              ...
                              <Box sx={{ minWidth: 0, flex: 1 }}>
                                ...
                                {isTemp && (
                                  <Typography
                                    sx={{ fontSize: "0.58rem", color: T.faint }}
                                  >
                                    {formatAccessExpiry(
                                      pageAccessExpiry[
                                        normalizePageId(page.id)
      ...
      >
        ...
        >
          ...
          <Box>
            <Typography
              sx={{ fontWeight: 700, fontSize: "0.95rem", color: T.text }}
            >
              Outside role scope
            </Typography>
            <Typography sx={{ fontSize: "0.72rem", color: T.muted, mt: 0.2 }}>
              Temporary access only (max 1 day)
            </Typography>
          </Box>
        </Box>
            ...
            <>
              ...
              >
                <Typography sx={{ fontSize: "0.82rem", lineHeight: 1.55 }}>
                  <strong>{exceptionGrantPage.page_name}</strong> is not
                  normally available for the{" "}
                  <strong>{selectedUser.role}</strong> role. Continuing grants
              ...
              >
                <TextField
                  ...
                  inputProps={{
                    min: 1,
                    max: exceptionDurationUnit === "days" ? 1 : 24,
                    step: 1,
                  }}
                  sx={{ flex: 1 }}
                />
              ...
              {exceptionExpiryPreview.expiresAt &&
                !exceptionExpiryPreview.error && (
                  <Box
                    sx={{
                      p: 2,
                      borderRadius: 2,
                      bgcolor: alpha("#c2410c", 0.06),
                      border: `1px solid ${alpha("#c2410c", 0.2)}`,
                    }}
                  >
                    <Typography
                      sx={{
              ...
              {exceptionExpiryPreview.error && (
                <Typography sx={{ fontSize: "0.75rem", color: "#b91c1c" }}>
                  {exceptionExpiryPreview.error}
                </Typography>
              )}
            </>
        ...
        >
          <AccentButton
            onClick={closeExceptionGrantDialog}
            variant="outlined"
            disabled={exceptionGrantLoading}
            sx={{
              fontSize: "0.8rem",
              borderColor: T.accentBorder,
              color: T.muted,
      ...
      <Drawer
        ...
        PaperProps={{
          sx: { width: isMobile ? "100%" : "500px", bgcolor: "#f7f8fa" },
        }}
      >
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                  ...
                  <Box>
                    ...
                    >
                      ...
                      >
                        {(selectedUserForDetails.role || "").toUpperCase()}
                      </Typography>
                    </Box>
                  </Box>
                </Box>
              ...
              {["info", "access"].map((tab) => (
                ...
                >
                  {tab === "info" ? (
                    <>
                      <Info
                        sx={{ mr: 0.75, fontSize: 15, verticalAlign: "middle" }}
                      />
                      Information
                    </>
                  ) : (
                    ...
                    <Box sx={{ p: 2.5 }}>
                      <Stack spacing={1.75}>
                        {[
                          ...
                          {
                            label: "Last Login",
                            value: formatDate(selectedUserForDetails.lastLogin),
                          },
                        ].map(({ label, value }) => (
                          <Box
                            key={label}
                        ...
                        <Box>
                          ...
                          {(() => {
                            const info = resolveCategoryDisplay(
                              selectedUserForDetails,
                            );
                            return (
                              <Box
                              ...
                              >
                                <Circle
                                  sx={{ fontSize: 7, color: info.color }}
                                />
                                <Typography
                                  sx={{
                                    fontSize: "0.72rem",
                ...
                <Stack spacing={2}>
                  <SectionCard>
                    ...
                    <Box sx={{ p: 2.5, textAlign: "center" }}>
                      ...
                      >
                        {Math.round(animatedValue)}% access
                      </Typography>
                    </Box>
                  </SectionCard>
                </Stack>
          ...
          {pendingRoleChange && (
            <>
              ...
              >
                <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                  ...
                  <Box>
                    ...
                    <Typography
                      sx={{ fontSize: "0.72rem", color: T.muted, mt: 0.2 }}
                    >
                      #{pendingRoleChange.user.employeeNumber}
                    </Typography>
                  </Box>
                </Box>
              </Box>
              <Alert
                ...
                icon={<Info />}
              >
                This role change will be logged in the audit trail.
              </Alert>
              <Box
                sx={{
                  p: 2.5,
              ...
              >
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                  ...
                  >
                    ...
                    >
                      {(pendingRoleChange.oldRole || "").toUpperCase()}
                    </Typography>
                  </Box>
                  <Typography sx={{ color: T.faint, fontWeight: 700 }}>
                    →
                  </Typography>
                  <Box
                    sx={{
                  ...
                  >
                    ...
                    >
                      {(pendingRoleChange.newRole || "").toUpperCase()}
                    </Typography>
                  </Box>
                </Box>
              </Box>
            ...
            <>
              ...
              >
                <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                  ...
                  >
                    {getInitials(pendingStatusChange.user.fullName)}
                  </Avatar>
                  <Box>
                    <Typography
                      sx={{
                    ...
                    <Typography
                      sx={{ fontSize: "0.72rem", color: T.muted, mt: 0.2 }}
                    >
                      #{pendingStatusChange.user.employeeNumber}
                    </Typography>
                  </Box>
                </Box>
              </Box>
              <Alert
                ...
                icon={<Info />}
              >
                This status change will be logged in the audit trail.
              </Alert>
              <Box
                sx={{
                  p: 2.5,
              ...
              >
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                  ...
                  >
                    ...
                    >
                      {(pendingStatusChange.oldStatus || "").toUpperCase()}
                    </Typography>
                  </Box>
                  <Typography sx={{ color: T.faint, fontWeight: 700 }}>
                    →
                  ...
                  >
                    ...
                    >
                      {(pendingStatusChange.newStatus || "").toUpperCase()}
                    </Typography>
                  </Box>
                </Box>
              </Box>
      ...
      >
        ...
        >
          ...
          <Box>
            ...
            {userToEdit && (
              <Typography sx={{ fontSize: "0.72rem", color: T.muted, mt: 0.1 }}>
                #{userToEdit.employeeNumber}
              </Typography>
            )}
          </Box>
        </Box>
              ...
              >
                ...
                <FormControl fullWidth sx={{ mt: 2 }} size="small">
                  ...
                  {typeConfigs.length === 0 ? (
                    ...
                    >
                      <Typography sx={{ fontSize: "0.78rem", color: T.muted }}>
                        No employment types configured yet.
                      </Typography>
                    </Box>
                  ) : (
                    <Select
    ...
    sx: {
      "& .MuiListSubheader-root": { position: "static" },
                      ...
                      renderValue={(val) => {
                        if (!val)
                          return (
                            <Typography
                              sx={{ color: T.faint, fontSize: "0.875rem" }}
                            >
                              Select a category…
                            </Typography>
                          );
                        const found = typeConfigs.find(
                        ...
                        return (
                          ...
                          >
                            <Circle
                              sx={{ fontSize: 8, color: found.colorHex }}
                            />
                            <Typography sx={{ fontSize: "0.875rem" }}>
                              {found.parentGroup} | {found.typeName}
                            </Typography>
                          </Box>
                        );
                      }}
                    >
                      ...
                      {(() => {
                        ...
                        typeConfigs
                          .filter((t) => t.isActive !== false)
                          .forEach((t) => {
                            if (!grouped[t.parentGroup])
                              grouped[t.parentGroup] = [];
                ...
                {editedEmploymentCategory &&
                  (() => {
                    const cfg = typeConfigs.find(
                      (t) => String(t.id) === String(editedEmploymentCategory),
                    );
                    return cfg ? (
                      <Box
                        sx={{
                          ...
                          py: 1,
                          borderRadius: 2,
                          bgcolor: alpha(cfg.colorHex, 0.06),
                          border: `1px solid ${alpha(cfg.colorHex, 0.2)}`,
                        }}
                      >
                        <Box
                          sx={{
      ...
      >
        ...
        >
          ...
          >
            <Category sx={{ fontSize: 18, color: T.accent }} />
          </Box>
          <Box sx={{ flex: 1 }}>
            <Typography sx={{ fontWeight: 700, fontSize: "0.95rem" }}>
          ...
          >
            ...
            >
              {selectedEmployeeNumbers.length} selected
            </Typography>
          </Box>
        </Box>
        <DialogContent sx={{ p: 3 }}>
          ...
          >
            <Typography sx={{ fontSize: "0.82rem", color: T.muted }}>
              This will apply the selected category to all selected employees.
            </Typography>
          </Box>
          {typeConfigs.length === 0 ? (
            <Box
            ...
            >
              <Typography sx={{ fontSize: "0.78rem", color: T.muted }}>
                No employment types configured yet. Set them up in Employment
                Category Management.
              </Typography>
            </Box>
          ) : (
    ...
    sx: {
      "& .MuiListSubheader-root": { position: "static" },
                ...
                renderValue={(val) => {
                  if (!val)
                    return (
                      <Typography sx={{ color: T.faint, fontSize: "0.875rem" }}>
                        Select a category…
                      </Typography>
                    );
                  const found = typeConfigs.find(
          ...
          {bulkEmploymentCategory &&
            (() => {
              const cfg = typeConfigs.find(
                (t) => String(t.id) === String(bulkEmploymentCategory),
              );
              return cfg ? (
                <Box
                  sx={{
      ...
      >
        ...
        >
          ...
          >
            <Business sx={{ fontSize: 18, color: T.accent }} />
          </Box>
          <Typography sx={{ fontWeight: 700, fontSize: "0.95rem" }}>
            ...
            <>
              ...
              >
                <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                  ...
                  >
                    {getInitials(pendingBranchChange.user.fullName)}
                  </Avatar>
                  <Box>
                    <Typography
                      sx={{
                    ...
                    >
                      {pendingBranchChange.user.fullName}
                    </Typography>
                    <Typography
                      sx={{ fontSize: "0.72rem", color: T.muted, mt: 0.2 }}
                    >
                      #{pendingBranchChange.user.employeeNumber}
                    </Typography>
                  </Box>
                </Box>
              </Box>
              <Alert
                ...
                icon={<Info />}
              >
                This branch change will be logged in the audit trail.
              </Alert>
              <Box
                sx={{
                  p: 2.5,
              ...
              >
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                  ...
                  >
                    ...
                    >
                      {(
                        getBranchLabel(pendingBranchChange.oldBranch) || "UNSET"
                      ).toUpperCase()}
                    </Typography>
                  </Box>
                  <Typography sx={{ color: T.faint, fontWeight: 700 }}>
                  ...
                  >
                    ...
                    >
                      {(
                        getBranchLabel(pendingBranchChange.newBranch) || ""
                      ).toUpperCase()}
                    </Typography>
                  </Box>
                </Box>
        ...
        >
          ...
          >
            {branchChangeLoading ? "Updating…" : "Confirm Change"}
          </AccentButton>
        </DialogActions>
      </Dialog>
