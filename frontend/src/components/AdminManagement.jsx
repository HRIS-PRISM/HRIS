...
const UsersList = () => {
  ...
  // Selected employees (details for the bulk modal)
  const selectedEmployeesDetailed = useMemo(() => {
    const byEmpNo = new Map(
      (users || []).map((u) => [String(u.employeeNumber), u]),
    );
    return (selectedEmployeeNumbers || [])
      .map((empNo) => byEmpNo.get(String(empNo)))
      .filter(Boolean);
  }, [selectedEmployeeNumbers, users]);

  const formatSelectedEmployeeLine = (u) => {
    const p = u?.personData || {};
    const empNo = u?.employeeNumber ?? '';

    const last = (p.lastName || u?.lastName || '').trim();
    const first = (p.firstName || u?.firstName || '').trim();
    const middle = (p.middleName || u?.middleName || '').trim();

    const middleInitial = middle ? `${middle.charAt(0).toUpperCase()}.` : '';
    const core = [last, first, middleInitial].filter(Boolean).join(', ');

    return core ? `${empNo}, ${core}` : String(empNo);
  };
  ...
  const handleToggleSelectAllOnPage = () => {
    const pageEmployeeNumbers = paginatedUsers.map((u) => u.employeeNumber);
    const allSelected = pageEmployeeNumbers.every((id) =>
      selectedEmployeeNumbers.includes(id),
    );

    setSelectedEmployeeNumbers((prev) => {
      if (allSelected) {
        // unselect all on current page
        return prev.filter((id) => !pageEmployeeNumbers.includes(id));
      }
      // select all on current page (keep existing)
      const merged = new Set([...prev, ...pageEmployeeNumbers]);
      return Array.from(merged);
    });
  };

  const handleOpenBulkCategoryDialog = () => {
    if (selectedEmployeeNumbers.length === 0) {
  ...
  const handleCloseBulkCategoryDialog = () => {
    if (!bulkCategoryLoading) setBulkCategoryDialogOpen(false);
  };

  const handleApplyBulkEmploymentCategory = async () => {
    if (selectedEmployeeNumbers.length === 0) return;

    if (bulkEmploymentCategory === '') {
      setSnackbarMessage('Please choose an employment category.');
      setSnackbarOpen(true);
      return;
    }
    ...
    try {
      const authHeaders = getAuthHeaders();
      const newCategory = parseInt(bulkEmploymentCategory);

      for (const employeeNumber of selectedEmployeeNumbers) {
      ...
      setSnackbarMessage('Failed to bulk update employment category.');
      setSnackbarOpen(true);
    } finally {
  ...
  return (
    ...
    >
      <Box sx={{ px: 6, mx: 'auto', maxWidth: '1600px' }}>
            ...
            <GlassCard>
              ...
              >
                ...
                <Box display="flex" alignItems="center" gap={2}>
                  ...
                  <Tooltip title="Bulk edit employment category for selected employees">
                    <ProfessionalButton
                      variant="outlined"
                        ...
                        <Checkbox
                          checked={
                            ...
                            paginatedUsers.every((u) =>
                              selectedEmployeeNumbers.includes(
                                u.employeeNumber,
                              ),
                            )
                          }
        ...
        {/* Bulk Employment Category Dialog */}
        <Dialog
          open={bulkCategoryDialogOpen}
          onClose={handleCloseBulkCategoryDialog}
          maxWidth="sm"
          fullWidth
          PaperProps={{
            sx: {
              borderRadius: 4,
              bgcolor: '#FFFFFF',
              border: `1px solid ${alpha('#6d2323', 0.18)}`,
        ...
        >
          <DialogTitle
            sx={{
              ...
              fontWeight: 800,
              p: 3,
              borderBottom: `1px solid ${alpha('#6d2323', 0.12)}`,
            }}
          >
            <Box
              sx={{
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                <WorkOutline sx={{ fontSize: 26, color: '#6d2323' }} />
                <Typography
                  variant="h6"
                  sx={{ fontWeight: 800, color: '#6d2323' }}
                >
              ...
              <IconButton
                onClick={handleCloseBulkCategoryDialog}
                disabled={bulkCategoryLoading}
                sx={{
                  color: '#6d2323',
                  border: `1px solid ${alpha('#6d2323', 0.2)}`,
                  '&:hover': { bgcolor: alpha('#6d2323', 0.06) },
                }}
              >
                <Close />
              </IconButton>
            ...
            >
              Selected employees: {selectedEmployeeNumbers.length}
            </Typography>
          </DialogTitle>
          ...
          <DialogContent sx={{ p: 3, bgcolor: '#FFFFFF' }}>
            <Box sx={{ mb: 2 }}>
              <FormControl fullWidth>
                <InputLabel sx={{ fontWeight: 600, color: '#6d2323' }}>
                  Employment Category
                </InputLabel>
                <Select
                  ...
                  sx={{
                    ...
                    bgcolor: '#FFFFFF',
                    '& .MuiOutlinedInput-notchedOutline': {
                      borderColor: alpha('#6d2323', 0.25),
                    },
                    '&:hover .MuiOutlinedInput-notchedOutline': {
                      borderColor: alpha('#6d2323', 0.45),
                    },
            ...
            <Box
              sx={{
                borderRadius: 3,
                border: `1px solid ${alpha('#6d2323', 0.12)}`,
                overflow: 'hidden',
              }}
            >
              <Box
                sx={{
                  ...
                  py: 1.5,
                  bgcolor: '#FFFFFF',
                  borderBottom: `1px solid ${alpha('#6d2323', 0.08)}`,
                }}
              >
                <Typography sx={{ fontWeight: 800, color: '#6d2323' }}>
                  Selected Employees
                </Typography>
              </Box>
              ...
              <List
                dense
                sx={{ maxHeight: 220, overflow: 'auto', bgcolor: '#FFFFFF' }}
              >
                {selectedEmployeesDetailed.length === 0 ? (
                  <ListItem>
                    <ListItemText
                      primary="No employees selected."
                  ...
                  selectedEmployeesDetailed.map((u, idx) => (
                    <React.Fragment key={u.employeeNumber || idx}>
                      <ListItem
                        sx={{
                          bgcolor: '#FFFFFF',
                          py: 1.2,
                      ...
                      >
                        <ListItemText
                          primary={formatSelectedEmployeeLine(u)}
                          primaryTypographyProps={{
                            sx: {
                              color: '#6d2323',
                              fontWeight: 700,
                      ...
                      {idx !== selectedEmployeesDetailed.length - 1 && (
                        <Divider sx={{ borderColor: alpha('#6d2323', 0.08) }} />
                      )}
                    </React.Fragment>
                  ))
                )}
          ...
          <DialogActions
            sx={{
              ...
              gap: 2,
              bgcolor: '#FFFFFF',
              borderTop: `1px solid ${alpha('#6d2323', 0.1)}`,
            }}
          >
            <ProfessionalButton
              onClick={handleCloseBulkCategoryDialog}
              ...
              disabled={bulkCategoryLoading}
              sx={{
                borderColor: alpha('#6d2323', 0.35),
                color: '#6d2323',
                '&:hover': {
                  borderColor: '#6d2323',
                  bgcolor: alpha('#6d2323', 0.06),
                },
              }}
            >
              Cancel
            ...
            <ProfessionalButton
              onClick={handleApplyBulkEmploymentCategory}
              variant="contained"
              disabled={bulkCategoryLoading}
              startIcon={
                bulkCategoryLoading ? (
              ...
              sx={{
                ...
                color: '#FFFFFF',
                '&:hover': {
                  bgcolor: alpha('#6d2323', 0.92),
                },
                '&:disabled': {
                  bgcolor: alpha('#6d2323', 0.35),
                },
              }}
            >
              {bulkCategoryLoading ? 'Applying...' : 'Apply Category'}
            </ProfessionalButton>
          </DialogActions>
        </Dialog>
