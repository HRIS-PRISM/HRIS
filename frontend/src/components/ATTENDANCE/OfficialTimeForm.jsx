...
const AcademicYearAutocomplete = ({
  ...
  <Autocomplete
    ...
    onChange={(_, v) => {
      const f = autoFormatAcademicYear(typeof v === "string" ? v : "");
      onChange(f || (typeof v === "string" ? v : ""));
    }}
    onBlur={(e) => {
      const f = autoFormatAcademicYear(e.target.value);
      if (f !== e.target.value) onChange(f);
      if (onBlur) onBlur(e);
    }}
        ...
        {label && (
          <Box
            ...
            sx={{
              ...
              bgcolor: disabled ? "#f5f5f5" : "#fff",
              fontSize: "0.62rem",
              color: focused ? accentColor : "rgba(0,0,0,0.55)",
              lineHeight: 1,
              pointerEvents: "none",
...
const checkTimeOverlaps = (rows) => {
  ...
  for (const row of rows) {
    ...
    const push = (inKey, outKey, label) => {
      ...
      if (s != null && e != null && s < e && !(s === 0 && e === 12 * 60))
        segs.push({ start: s, end: e, label });
    };
    ...
    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
      ...
      <Box>
        ...
        <Typography sx={{ fontSize: "0.78rem", opacity: 0.85, mt: 0.25 }}>
          Schedule data was modified outside the application.
        </Typography>
      </Box>
    </Box>
    <Button
...
const OfficialTimeForm = () => {
  ...
  const scheduleBlockRows = useMemo(() => {
    const rows = [];
    for (let i = 0; i < scheduleBlocks.length; i += 2)
      rows.push(scheduleBlocks.slice(i, i + 2));
    return rows;
  }, [scheduleBlocks]);
  ...
  const activeBlockData = useMemo(() => {
    ...
    return (
      scheduleBlocks.find((b) => b.key === activeScheduleKey) ||
      scheduleBlocks[0] ||
      null
    );
  ...
  const handleSubmitFromModal = useCallback(async () => {
    ...
    const sevenRows = DAYS_ORDER.map((day) => {
      ...
      return r ? { ...r, day } : makeDefaultRow(employeeID, day);
    });
    ...
    if (!overlapResult.valid) {
      ...
      setWarningMessage(
        `Time overlap on ${overlapResult.day}: ${a.label} (${formatMinutesToTime(a.start)} – ${formatMinutesToTime(a.end)}) overlaps with ${b.label} (${formatMinutesToTime(b.start)} – ${formatMinutesToTime(b.end)}).`,
      );
  ...
  const handleSaveEditedSchedule = useCallback(async () => {
    ...
    if (
      ...
      newEndDate < normalizeDateStr(viewScheduleInfo.startDate)
    ) {
    ...
    if (!overlapResult.valid) {
      ...
      const b = overlapResult.segmentB;
      setWarningMessage(
        `Time overlap on ${overlapResult.day}: ${a.label} (${formatMinutesToTime(a.start)} – ${formatMinutesToTime(a.end)}) overlaps with ${b.label}.`,
      );
      setShowWarningModal(true);
      return;
    }
  ...
  const paginatedAllUsers = useMemo(
    () =>
      filteredAllUsers.slice(
        allUsersPage * allUsersRowsPerPage,
        (allUsersPage + 1) * allUsersRowsPerPage,
      ),
  ...
  const handleConfirmBulkBlocks = useCallback(() => {
    ...
    if (!block) return;
    if (!block.academicYear || !block.startDate || !block.endDate) {
