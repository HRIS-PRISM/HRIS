  ...
  <>
    ...
    <Box sx={{
      ...
      <Box sx={{ mb: 2, borderRadius: '12px', overflow: 'hidden', border: '0.5px solid rgba(0,0,0,0.09)', animation: 'blink 2s ease-in-out infinite' }}>
        <Box sx={{ px: 4, py: 3, background: 'linear-gradient(135deg,#fdf5f5 0%,#f0dede 100%)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box><Bone w={220} h={18} sx={{ mb: 1 }} /><Bone w={320} h={11} /></Box>
          </Box>
          <Box sx={{ display: 'flex', gap: 1 }}>
            <Box sx={{ width: 120, height: 36, borderRadius: '6px', bgcolor: 'rgba(109,35,35,0.08)', border: '1px solid rgba(109,35,35,0.14)' }} />
            <Box sx={{ width: 120, height: 36, borderRadius: '6px', bgcolor: 'rgba(109,35,35,0.08)', border: '1px solid rgba(109,35,35,0.14)' }} />
          </Box>
        </Box>
      </Box>
      <Box sx={{ mb: 2, borderRadius: '12px', overflow: 'hidden', border: '0.5px solid rgba(0,0,0,0.09)', bgcolor: '#fff', animation: 'blink 2s ease-in-out 0.1s infinite' }}>
        <Box sx={{ px: 2.5, py: 1.25, bgcolor: T.accentFaint, borderBottom: `1px solid ${T.divider}`, display: 'flex', alignItems: 'center', gap: 1, minHeight: 42 }}>
          <Box sx={{ width: 14, height: 14, borderRadius: '50%', bgcolor: 'rgba(109,35,35,0.2)' }} />
          <Bone w={180} h={12} />
        </Box>
        <Box sx={{ px: 2.5, py: 2.5 }}>
          <Box sx={{ display: 'flex', gap: 1.5, mb: 2 }}>
            {[1,2,3,4,5].map(i => <Box key={i} sx={{ flex: 1, height: 40, borderRadius: '8px', bgcolor: T.accentFaint, border: `1px solid ${T.accentBorder}` }} />)}
          </Box>
          <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
            <Box sx={{ width: 100, height: 36, borderRadius: '8px', bgcolor: T.accentFaint, border: `1px solid ${T.accentBorder}` }} />
            <Box sx={{ width: 140, height: 36, borderRadius: '20px', bgcolor: 'rgba(109,35,35,0.15)' }} />
          </Box>
        </Box>
      </Box>
    </Box>
...
const PremiumTableCell = styled(TableCell)(({ isHeader }) => ({
  ...
  padding: '14px 16px',
  borderBottom: isHeader ? `2px solid ${alpha(T.accent, 0.2)}` : `1px solid ${T.divider}`,
  fontSize: '0.88rem',
...
const ScheduleTabBar = ({ activeTab, setTab }) => (
  <Box sx={{ display: 'flex', border: `1px solid ${T.accentBorder}`, borderRadius: 1, overflow: 'hidden', flex: 1, minWidth: 280 }}>
    {[
      ...
      <Button key={key} onClick={() => setTab(key)} disableElevation fullWidth
        sx={{
...
const AcademicYearAutocomplete = ({
  value, onChange, onBlur, sx, size = 'small',
  label = 'Academic Year', placeholder = 'e.g. 2025 - 2026',
}) => (
  <Autocomplete
    freeSolo
    ...
    renderOption={(props, option) => (
      <Box component="li" {...props} sx={{
        ...
        py: '6px !important', px: '14px !important',
        '&.Mui-focused, &:hover': { bgcolor: `${T.accentFaint} !important`, color: T.accent },
      }}>
...
const TimePickerField = ({
  value, onChange, label, size = 'small', disabled = false, accentColor = T.accent,
}) => {
  const parseVal = (v) => {
    if (!v || !String(v).trim()) return { hh: '', mm: '', ss: '', ampm: 'AM' };
    const match = String(v).trim().match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?\s*(AM|PM)$/i);
  ...
  const emitAll = useCallback((h, m, s, a) => {
    const isEmpty = !h && !m && !s;
    const out = isEmpty
      ? ''
  ...
  const onHhChange = (e) => {
    ...
    setHh(raw); emitAll(raw, mm, ss, ampm);
    if (raw.length === 2) { mmRef.current?.focus(); mmRef.current?.select(); }
  };
  const onHhBlur = (e) => {
    const v = e.target.value.replace(/\D/g, '');
    const padded = v ? v.padStart(2, '0') : '';
    setHh(padded); emitAll(padded, mm, ss, ampm);
  };
  const onHhKeyDown = (e) => {
  ...
  const onMmChange = (e) => {
    const raw = e.target.value.replace(/\D/g, '').slice(0, 2);
    setMm(raw); emitAll(hh, raw, ss, ampm);
    if (raw.length === 2) { ssRef.current?.focus(); ssRef.current?.select(); }
  };
  const onMmBlur = (e) => {
    const v = e.target.value.replace(/\D/g, '');
    const padded = v ? v.padStart(2, '0') : '';
    setMm(padded); emitAll(hh, padded, ss, ampm);
  };
  const onMmKeyDown = (e) => {
  ...
  const onSsChange = (e) => {
    const raw = e.target.value.replace(/\D/g, '').slice(0, 2);
    setSs(raw); emitAll(hh, mm, raw, ampm);
  };
  const onSsBlur = (e) => {
    const v = e.target.value.replace(/\D/g, '');
    const padded = v ? v.padStart(2, '0') : '';
    setSs(padded); emitAll(hh, mm, padded, ampm);
  };
  const onSsKeyDown = (e) => {
  ...
  const handleAmpmChange = (e) => { const a = e.target.value; setAmpm(a); emitAll(hh, mm, ss, a); };
  const handleClear = () => { setHh(''); setMm(''); setSs(''); setAmpm('AM'); lastEmittedRef.current = ''; onChange(''); };
  const handleQuickPick = (h, m, a) => { setHh(h); setMm(m); setSs('00'); setAmpm(a); emitAll(h, m, '00', a); setAnchorEl(null); };
  ...
  const _HOURS = Array.from({ length: 12 }, (_, i) => String(i + 1).padStart(2, '0'));
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.4, width: '100%' }}>
      <Box
        ...
        sx={{
          flex: 1, display: 'flex', alignItems: 'center', minWidth: 96,
          px: 1, py: 0.65,
      ...
      >
        {label && (
          <Box component="span" sx={{
            position: 'absolute', top: -8, left: 7, px: 0.4,
            bgcolor: disabled ? '#f5f5f5' : '#fff',
            fontSize: '0.62rem', color: containerFocused ? accentColor : 'rgba(0,0,0,0.55)',
        ...
        <input ref={hhRef} type="text" inputMode="numeric" maxLength={2} placeholder="HH" value={hh} disabled={disabled}
          onChange={onHhChange} onBlur={(e) => { setContainerFocused(false); onHhBlur(e); }}
          onFocus={(e) => { setContainerFocused(true); e.target.select(); }} onKeyDown={onHhKeyDown} style={segStyle} />
        ...
        <input ref={mmRef} type="text" inputMode="numeric" maxLength={2} placeholder="MM" value={mm} disabled={disabled}
          onChange={onMmChange} onBlur={(e) => { setContainerFocused(false); onMmBlur(e); }}
          onFocus={(e) => { setContainerFocused(true); e.target.select(); }} onKeyDown={onMmKeyDown} style={segStyle} />
        <span style={{ fontSize: '0.82rem', fontFamily: 'monospace', color: '#bbb', lineHeight: 1, margin: '0 1px', userSelect: 'none' }}>:</span>
        <input ref={ssRef} type="text" inputMode="numeric" maxLength={2} placeholder="SS" value={ss} disabled={disabled}
          onChange={onSsChange} onBlur={(e) => { setContainerFocused(false); onSsBlur(e); }}
          onFocus={(e) => { setContainerFocused(true); e.target.select(); }} onKeyDown={onSsKeyDown} style={segStyle} />
      </Box>

      <Select size={size} value={ampm} onChange={handleAmpmChange} disabled={disabled} renderValue={(v) => v}
        sx={{
      ...
      <Tooltip title="Quick pick time">
        <span>
          <IconButton size="small" disabled={disabled} onClick={(e) => setAnchorEl(e.currentTarget)}
      ...
      <Popover
        ...
        disablePortal={false}
        PaperProps={{ sx: { borderRadius: 2, boxShadow: '0 8px 32px rgba(0,0,0,0.22)', border: `1px solid ${alpha(accentColor, 0.18)}`, p: 0, width: 300, maxWidth: 300, maxHeight: 340, overflow: 'hidden', display: 'flex', flexDirection: 'column' } }}
      >
        <Box sx={{ bgcolor: accentColor, px: 2, py: 1, flexShrink: 0 }}>
          <Typography sx={{ color: '#fff', fontWeight: 700, fontSize: '0.78rem', letterSpacing: '0.5px', textTransform: 'uppercase' }}>Quick Pick</Typography>
        </Box>
          ...
          {['AM', 'PM'].map((ap) => (
            <Box key={ap}>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '3px', px: 1.25, py: 0.75 }}>
                {_HOURS.map((h) => _MINUTES.map((min) => (
                  <Button key={`${h}${min}${ap}`} size="small" onClick={() => handleQuickPick(h, min, ap)}
...
const autoFormatAcademicYear = (raw) => {
  ...
  if (/^\d{4}\s*-\s*\d{4}$/.test(trimmed)) return trimmed.replace(/\s*-\s*/, ' - ');
  if (/^\d{4}$/.test(trimmed)) { const y = parseInt(trimmed, 10); return `${y} - ${y + 1}`; }
  return raw;
};
...
const formatDateLong = (val) => {
  ...
  if (/^\d{4}-\d{2}-\d{2}$/.test(dateOnly)) {
    ...
    y = yy; day = dd; month = months[mm - 1] || '';
  } else {
    const d = new Date(val);
    if (Number.isNaN(d.getTime())) return s;
    y = d.getFullYear(); day = d.getDate(); month = months[d.getMonth()] || '';
  }
...
const makeDefaultRow = (employeeID, day) => ({
  employeeID, day,
  officialTimeIN: '08:00:00 AM', officialBreaktimeIN: '00:00:00 AM',
  officialBreaktimeOUT: '00:00:00 PM', officialTimeOUT: '05:00:00 PM',
  officialHonorariumTimeIN: '00:00:00 AM', officialHonorariumTimeOUT: '00:00:00 PM',
  officialServiceCreditTimeIN: '00:00:00 AM', officialServiceCreditTimeOUT: '00:00:00 AM',
  officialOverTimeIN: '00:00:00 AM', officialOverTimeOUT: '00:00:00 PM',
  breaktime: '',
});
  ...
  <Box sx={{
    ...
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
      ...
      <Box>
        ...
        <Typography sx={{ fontSize: '0.78rem', opacity: 0.85, mt: 0.25 }}>Schedule data was modified outside the application. Displaying last verified data from server.</Typography>
      </Box>
    </Box>
  ...
  return (
    <Box sx={{ display: 'flex', border: `1.5px solid ${alpha(T.accent, 0.35)}`, borderRadius: 2, overflow: 'hidden', bgcolor: alpha(T.accent, 0.04) }}>
      {options.map(({ key, label, icon }, i) => {
        ...
        return (
          <Button key={key} onClick={() => onChange(key)} startIcon={icon} disableElevation
            sx={{
              borderRadius: 0, textTransform: 'none',
...
const ScheduleTimeRows = ({ records, onChangeRecord, scheduleView, readOnly = false }) => {
  const TIME_FIELDS = {
    workDays: [
      { key: 'officialTimeIN', label: 'Time In' }, { key: 'officialBreaktimeIN', label: 'Break In' },
      { key: 'officialBreaktimeOUT', label: 'Break Out' }, { key: 'officialTimeOUT', label: 'Time Out' },
    ],
    honorarium: [
      { key: 'officialHonorariumTimeIN', label: 'Honorarium In' }, { key: 'officialHonorariumTimeOUT', label: 'Honorarium Out' },
    ],
    serviceCredits: [
      { key: 'officialServiceCreditTimeIN', label: 'Service Credit In' }, { key: 'officialServiceCreditTimeOUT', label: 'Service Credit Out' },
    ],
    overtime: [
      { key: 'officialOverTimeIN', label: 'Overtime In' }, { key: 'officialOverTimeOUT', label: 'Overtime Out' },
    ],
  };
    ...
    <>
      <TableHead>
        <TableRow sx={{ bgcolor: T.accent }}>
          <TableCell sx={{ color: '#fff', fontWeight: 700, fontSize: '0.75rem', py: 1.25, width: 100 }}>Day</TableCell>
          {fields.map((f) => (
            <TableCell key={f.key} sx={{ color: '#fff', fontWeight: 700, fontSize: '0.75rem', py: 1.25, minWidth: 200 }}>{f.label}</TableCell>
          ))}
        </TableRow>
      </TableHead>
      <TableBody>
...
const OfficialTimeForm = () => {
  ...
  useEffect(() => {
    ...
    tamperCheckIntervalRef.current = setInterval(() => {
      setRecords((current) => { runIntegrityCheck(current); return current; });
    }, 3000);
  ...
  const handleRestoreFromServer = useCallback(async () => {
    ...
    try {
      ...
      stampServerRecords(fresh); setRecords(deepClone(fresh)); setFound(res.data.length > 0);
  ...
  const showToast = useCallback((msg) => {
    setSuccessAction(msg); setSuccessOpen(true);
  ...
  const fetchAllUsers = useCallback(async () => {
    ...
    try {
      ...
      setAllUsers(response.data || []); setAllUsersPage(0);
    } catch (error) { console.error('Error fetching users:', error); showToast('Error fetching users.'); }
    finally { setLoadingUsers(false); }
  }, [showToast]);

  const handleClearModalTimes = useCallback(() => {
    const fieldsByView = {
      workDays: ['officialTimeIN','officialBreaktimeIN','officialBreaktimeOUT','officialTimeOUT'],
      honorarium: ['officialHonorariumTimeIN','officialHonorariumTimeOUT'],
      serviceCredits: ['officialServiceCreditTimeIN','officialServiceCreditTimeOUT'],
      overtime: ['officialOverTimeIN','officialOverTimeOUT'],
    };
    const fields = fieldsByView[scheduleView] || [];
    setModalRecords((prev) => prev.map((row) => { const updated = { ...row }; fields.forEach((f) => { updated[f] = ''; }); return updated; }));
  }, [scheduleView]);
  ...
  const handleSaveEditedSchedule = useCallback(async () => {
    ...
    const trimmedId = String(employeeID || '').trim();
    if (!trimmedId) { showToast('Employee ID is missing.'); return; }
    ...
    if (newEndDate && origStartDate && newEndDate < origStartDate) { showToast('End date cannot be before start date.'); return; }
    const overlapResult = checkTimeOverlaps(editViewRecords);
    if (!overlapResult.valid) {
      const a = overlapResult.segmentA; const b = overlapResult.segmentB;
      setWarningMessage(`Time overlap on ${overlapResult.day}: ${a.label} (${formatMinutesToTime(a.start)} – ${formatMinutesToTime(a.end)}) overlaps with ${b.label} (${formatMinutesToTime(b.start)} – ${formatMinutesToTime(b.end)}). Please adjust the schedule so times do not overlap.`);
      setWarningOverlap(overlapResult.overlap || null); setShowWarningModal(true); return;
    }
    setEditViewSaving(true);
    try {
      await axios.put(`${API_BASE_URL}/officialtimetable/${trimmedId}`, {
  ...
  const handleSearch = useCallback(async () => {
    const trimmedId = String(employeeID || '').trim();
    if (!trimmedId) { showToast('Please enter an Employee ID.'); return; }
    setHasSearched(true); setLoading(true); setRecords([]); setFound(false);
    try {
      const res = await axios.get(`${API_BASE_URL}/officialtimetable/${trimmedId}`, getAuthHeaders());
      const data = res.data.length > 0 ? res.data : buildDefaultRecords(trimmedId);
      stampServerRecords(data); setRecords(deepClone(data)); setFound(res.data.length > 0);
    } catch (err) { console.error('Error fetching data:', err); showToast('Error fetching records.'); }
  ...
  const handleChange = useCallback((index, field, value) => {
    setRecords((prev) => {
      ...
      if (!isBulkSchedule) {
        ...
        autoSaveTimeoutRef.current = setTimeout(() => { autoSaveRecords(updated); }, 1500);
      }
      return updated;
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const autoSaveRecords = useCallback(async (recordsToSave) => {
    ...
    setAutoSaving(true); saveInFlightRef.current = true;
    try {
      const academicYearForBackend = [draftAcademicYear, draftSemester].filter(Boolean).join(' ').trim() || null;
      await axios.post(`${API_BASE_URL}/officialtimetable`, { employeeID: trimmedId, academicYear: academicYearForBackend, startDate: draftStartDate, endDate: draftEndDate, status: draftStatus || 'active', records: recordsToSave }, getAuthHeaders());
      setLastSaved(new Date()); setFound(true); stampServerRecords(recordsToSave);
    } catch (err) { console.error('Error auto-saving records:', err); }
    finally { setAutoSaving(false); saveInFlightRef.current = false; }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const openCreateScheduleModal = useCallback(async () => {
    const trimmedId = String(employeeID || '').trim();
    if (!trimmedId) { showToast('Please enter Employee Number.'); return; }
    setIsBulkSchedule(false); setBulkTargetEmployees([]); setBulkScheduleBlocks([]); setScheduleView('workDays');
    if (hasSearched && records.length === 0) {
      ...
      setModalRecords(buildDefaultRecords(trimmedId)); setShowScheduleModal(true); return;
    }
    let existingRecords = [];
    try {
      const res = await axios.get(`${API_BASE_URL}/officialtimetable/${trimmedId}`, getAuthHeaders());
    ...
    setModalRecords(sevenRows); setShowScheduleModal(true);
  ...
  const handleModalRecordChange = useCallback((index, field, value) => {
    setModalRecords((prev) => { const updated = [...prev]; updated[index] = { ...updated[index], [field]: value }; return updated; });
  }, []);

  const handleSubmitFromModal = useCallback(async () => {
    const trimmedId = String(employeeID || '').trim();
    if (!isBulkSchedule) {
      if (!trimmedId || !draftStartDate || !draftEndDate) { showToast('Employee Number, Start date and End date are required.'); return; }
      if (new Date(draftStartDate) > new Date(draftEndDate)) { showToast('Start date must be on or before end date.'); return; }
    }
    ...
    if (!overlapResult.valid) {
      const a = overlapResult.segmentA; const b = overlapResult.segmentB;
      setWarningMessage(`Time overlap on ${overlapResult.day}: ${a.label} (${formatMinutesToTime(a.start)} – ${formatMinutesToTime(a.end)}) overlaps with ${b.label} (${formatMinutesToTime(b.start)} – ${formatMinutesToTime(b.end)}). Please adjust the schedule so Work Days, Honorarium, Service Credits, and Overtime do not overlap.`);
      setShowWarningModal(true); return;
    }
    if (isBulkSchedule && bulkTargetEmployees.length > 1) { setPendingBulkSubmit(true); setShowBulkConfirmModal(true); return; }
  ...
  const executeSave = useCallback(async (sevenRows, trimmedId) => {
    ...
    try {
      if (isBulkSchedule) {
        ...
        showToast(`Bulk schedules processed. Inserted ${Math.round((res.data.totalInserted || 0) / 7)} users.`);
        await fetchAllUsers();
      } else {
        const academicYearForBackend = [draftAcademicYear, draftSemester].filter(Boolean).join(' ').trim() || null;
        await axios.post(`${API_BASE_URL}/officialtimetable`, { employeeID: trimmedId, academicYear: academicYearForBackend, startDate: draftStartDate, endDate: draftEndDate, status: draftStatus || 'active', records: sevenRows }, { ...getAuthHeaders(), timeout: 30000 });
        setLastSaved(new Date()); showToast('Official time saved successfully.'); stampServerRecords(sevenRows);
        await handleSearch();
      }
      setShowScheduleModal(false); setIsBulkSchedule(false); setBulkScheduleBlocks([]); setBulkTargetEmployees([]);
    } catch (err) {
      console.error('Error saving data:', err);
      const msg = err.code === 'ECONNABORTED' || err.message?.includes('timeout') ? 'Request timed out. Please try again.'
        : err.response?.status === 409 ? err.response?.data?.message || 'This date range overlaps an existing schedule. Choose different dates.'
  ...
  const handleUpload = useCallback(async () => {
    ...
    setUploading(true);
    try {
      await axios.post(`${API_BASE_URL}/upload-excel-faculty-official-time/validate`, formData, { headers: { Authorization: `Bearer ${localStorage.getItem('token') || ''}` } });
      ...
      if (response.data.records && response.data.records.length > 0) {
        ...
        if (uploadedEmpId) {
          setEmployeeID(uploadedEmpId); setHasSearched(true);
          try {
            const refreshed = await axios.get(`${API_BASE_URL}/officialtimetable/${uploadedEmpId}`, getAuthHeaders());
            if (Array.isArray(refreshed.data) && refreshed.data.length > 0) { stampServerRecords(refreshed.data); setRecords(deepClone(refreshed.data)); setFound(true); }
            else { const defaults = buildDefaultRecords(uploadedEmpId); stampServerRecords(defaults); setRecords(deepClone(defaults)); setFound(false); }
          } catch (e) { console.error('Error refreshing schedules after upload:', e); }
        }
      }
      ...
      const message = error.response?.data?.message || error.response?.data?.error || (error.response?.status === 400 && 'Check file format: use .xlsx, include employeeID, day, effective_from, effective_until.') || (error.response?.status === 409 && 'Date range overlaps an existing schedule.') || error.message || 'Upload failed!';
      setWarningOverlap(error.response?.data?.overlap || null); setWarningMessage(message); setShowWarningModal(true);
    } finally { setUploading(false); }
  }, [file, uploading, buildDefaultRecords, showToast, stampServerRecords]);

  const handleUserSelect = useCallback((empNumber) => {
    setSelectedUsers((prev) => { const newSet = new Set(prev); if (newSet.has(empNumber)) newSet.delete(empNumber); else newSet.add(empNumber); return newSet; });
  }, []);
  ...
  const getFilteredUsers = useCallback(() => {
    ...
    return allUsers.filter((user) => (user.fullName || '').toLowerCase().includes(q) || (user.employeeNumber || '').toLowerCase().includes(q));
  }, [allUsers, searchQuery]);

  const filteredAllUsers  = useMemo(() => getFilteredUsers(), [getFilteredUsers]);
  const paginatedAllUsers = useMemo(() => { const start = allUsersPage * allUsersRowsPerPage; return filteredAllUsers.slice(start, start + allUsersRowsPerPage); }, [filteredAllUsers, allUsersPage, allUsersRowsPerPage]);
  ...
  const openBulkBlocksModalForSelected = useCallback(() => {
    ...
    setBulkTargetEmployees(Array.from(selectedUsers)); setShowBulkBlocksModal(true);
  ...
  const openBulkBlocksModalForAllMissing = useCallback(() => {
    ...
    setBulkScheduleBlocks([{ id: `${Date.now()}-${Math.random()}`, academicYear: '', semester: '', startDate: '', endDate: '' }]);
    setBulkTargetEmployees(usersWithoutDefault); setShowBulkBlocksModal(true);
  }, [allUsers, showToast]);

  const handleConfirmBulkBlocks = useCallback(() => {
    const block = bulkScheduleBlocks[0];
    if (!block) return;
    const cleaned = { ...block, academicYear: String(block.academicYear || '').trim(), semester: String(block.semester || '').trim(), startDate: String(block.startDate || '').trim(), endDate: String(block.endDate || '').trim() };
    if (!cleaned.academicYear || !cleaned.semester || !cleaned.startDate || !cleaned.endDate) { showToast('Please complete all fields before proceeding.'); return; }
    setBulkScheduleBlocks([cleaned]); setIsBulkSchedule(true);
    setDraftAcademicYear(cleaned.academicYear); setDraftSemester(cleaned.semester);
    setDraftStartDate(cleaned.startDate); setDraftEndDate(cleaned.endDate);
    const empId = String(bulkTargetEmployees[0] || '');
    setEmployeeID(empId); setModalRecords(DAYS_ORDER.map((day) => makeDefaultRow(empId, day)));
    setScheduleView('workDays'); setShowBulkBlocksModal(false); setShowScheduleModal(true);
  ...
  const handleEmployeeIDChange = useCallback((value) => {
    if (value === '' || /^\d+$/.test(value)) {
      setEmployeeID(value);
      if (value !== employeeID) {
        setRecords([]); setHasSearched(false); setFound(false);
        setDraftAcademicYear(''); setDraftSemester(''); setDraftStartDate(''); setDraftEndDate('');
        setLastSaved(null); serverRecordsRef.current = []; checksumRef.current = null; setTamperDetected(false);
      }
    }
  }, [employeeID]);
  ...
  const handleAnalyzeFile = async () => {
    ...
    try {
      ...
      setAnalyzeResult({ ok: false, message: data.message || error.message || 'Validation failed.', timeErrors: data.timeErrors || [], allErrors: data.allErrors || [], skippedRows: data.skippedRows || [], warnings: data.warnings || [], overlap: data.overlap || null });
    ...
    try {
      ...
      if (response.data.records?.length > 0) {
        ...
        if (uploadedEmpId) {
          ...
          } catch (e) { console.error('Error refreshing after upload:', e); }
        }
      }
      const warnings = response.data.warnings || [];
      showToast(`Upload complete! Inserted: ${response.data.inserted} rows.` + (warnings.length ? ` (${warnings.length} warning${warnings.length > 1 ? 's' : ''})` : ''));
      setFile(null);
    } catch (error) {
      const data = error.response?.data || {};
      setWarningMessage(data.message || error.message || 'Upload failed.'); setWarningOverlap(data.overlap || null); setShowWarningModal(true);
      setShowAnalyzeModal(false); setUploadAcknowledgeChecked(false);
    } finally { setConfirming(false); }
  };

  useEffect(() => {
    if (showAllUsers) { fetchAllUsers(); setSelectedUsers(new Set()); }
    setAllUsersPage(0);
  }, [showAllUsers, fetchAllUsers]);

  useEffect(() => { return () => { if (autoSaveTimeoutRef.current) clearTimeout(autoSaveTimeoutRef.current); if (tamperCheckIntervalRef.current) clearInterval(tamperCheckIntervalRef.current); }; }, []);

  useEffect(() => { if (autoSaveTimeoutRef.current) { clearTimeout(autoSaveTimeoutRef.current); autoSaveTimeoutRef.current = null; } }, [employeeID]);
  ...
  const dialogCloseBtn = (onClose, disabled) => (
    <IconButton size="small" onClick={onClose} disabled={disabled}
      sx={{ color: '#fff', ml: 1, flexShrink: 0, '&:hover': { bgcolor: 'rgba(255,255,255,0.15)' } }} aria-label="Close">
      <Close fontSize="small" />
    </IconButton>
  );
  ...
  if (hasAccess === false) {
    return (
      <AccessDenied title="Access Denied" message="You do not have permission to access Official Time Form. Contact your administrator to request access." returnPath="/admin-home" returnButtonText="Return to Home" />
    );
  }
  ...
  return (
    <>
      ...
      <LoadingOverlay
        ...
        message={checkingOverlap ? 'Checking schedule for conflicts...' : uploading ? 'Uploading...' : saving ? 'Saving...' : 'Loading...'}
      />
      ...
      <Fade in timeout={400}>
        <Box sx={{
          py: { xs: 1, md: 2 }, mt: { xs: 0, md: -2 }, mb: { xs: 1, md: 2 },
          width: '100vw', maxWidth: '100%',
          position: 'relative', left: '55%', transform: 'translateX(-53%)',
          px: { xs: 2, sm: 3, md: 6 },
            ...
            <Box sx={{
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2.5, position: 'relative', zIndex: 1 }}>
                ...
                <Box>
                  ...
                  <Typography sx={{ fontSize: '0.78rem', color: T.accentMid, fontWeight: 600 }}>Manage and update official time schedules for employees</Typography>
                </Box>
              </Box>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, position: 'relative', zIndex: 1 }}>
                <Box sx={{ px: 2, py: 0.6, borderRadius: 5, bgcolor: alpha('#4caf50', 0.12), border: '1px solid rgba(76,175,80,0.25)' }}>
                  <Typography sx={{ fontSize: '0.72rem', color: '#2e7d32', fontWeight: 700 }}>
                    {lastSaved ? `Saved ${lastSaved.toLocaleTimeString()}` : 'Official Time'}
                  </Typography>
                </Box>
          ...
          {showSingleView && (
            <Fade in timeout={400} key="single-view">
              <SectionCard sx={{ mb: 2 }}>
                ...
                {/* Step 1 */}
                <PanelHeader icon={Person} title="Step 1 — Employee & schedule period" />
                <Box sx={{ p: 2.5 }}>
                  <Grid container spacing={1.5} alignItems="center">
                    <Grid item xs={12} md={9}>
                      <Grid container spacing={1.5}>
                        <Grid item xs={12} sm={6} md={3}>
                          <ModernTextField fullWidth size="small" label="Employee Number" value={employeeID}
                            onChange={(e) => handleEmployeeIDChange(e.target.value)}
                            onKeyDown={(e) => { if (!/[0-9]/.test(e.key) && !['Backspace','Delete','ArrowLeft','ArrowRight','Tab'].includes(e.key)) e.preventDefault(); }}
                            InputProps={{ startAdornment: <InputAdornment position="start"><Person sx={{ color: T.accentMid, fontSize: 18 }} /></InputAdornment> }}
                          />
                        </Grid>
                        ...
                        <Grid item xs={12} sm={6} md={3}>
                          <Autocomplete freeSolo
                            ...
                            renderInput={(params) => <ModernTextField {...params} size="small" label="Semester" placeholder="e.g. 1st Semester" />}
                          />
                        </Grid>
                    ...
                    <Grid item xs={12} md={3}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Button variant="outlined" size="small" onClick={handleSearch} startIcon={<SearchIcon />}
                          ...
                          sx={{ borderColor: T.accent, color: T.accent, borderRadius: '8px', minWidth: 100, textTransform: 'none', fontWeight: 600, whiteSpace: 'nowrap', '&:hover': { bgcolor: T.accentFaint, borderColor: T.accent } }}>
                          Search
                        </Button>
                        <Button fullWidth variant="contained" onClick={openCreateScheduleModal}
                          disabled={!String(employeeID || '').trim() || ((!hasSearched || records.length > 0) && (!draftAcademicYear || !draftSemester || !draftStartDate || !draftEndDate))}
                          startIcon={<Schedule />}
                          sx={{ flex: 1, bgcolor: T.accent, color: '#fff', borderRadius: '20px', fontWeight: 600, textTransform: 'none', boxShadow: `0 2px 10px ${alpha(T.accent, 0.35)}`, '&:hover': { bgcolor: T.accentDark } }}>
                          Create Schedule
                        </Button>
                      </Box>
                    </Grid>
                ...
                {/* Step 2: Excel Upload */}
                <PanelHeader icon={CloudUploadIcon} title="Step 2 — Excel upload" rightContent={<Typography sx={{ fontSize: '0.7rem', color: T.faint, fontStyle: 'italic' }}>optional</Typography>} />
                <Box sx={{ p: 2.5 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
                    ...
                    <Typography variant="caption" sx={{ color: T.faint, fontSize: '0.7rem' }}>
                  ...
                  {file && !analyzing && (
                    <Box sx={{ mt: 1.25, display: 'flex', alignItems: 'center', gap: 0.75 }}>
                      <Box component="span" sx={{ width: 5, height: 5, borderRadius: '50%', bgcolor: alpha(T.accent, 0.5), display: 'inline-block', flexShrink: 0 }} />
                      <Typography sx={{ fontSize: '0.71rem', color: T.faint }}>
                        Click <strong>Validate</strong> to preview before uploading. No data will change until you confirm.
                      </Typography>
                    </Box>
                  )}
                </Box>
                ...
                {hasSearched && (
                  <>
                    <PanelHeader icon={Schedule} title="Existing schedules"
                      rightContent={(() => {
                        const byKey = new Map();
                    ...
                    <Box sx={{ p: 2.5 }}>
                      {(() => {
                        ...
                        const validSchedules = Array.from(byKey.values())
                          ...
                          .sort((a, b) => (String(b.status || 'active').toLowerCase() === 'active' ? 1 : 0) - (String(a.status || 'active').toLowerCase() === 'active' ? 1 : 0));

                        if (records.length === 0 || validSchedules.length === 0) {
                          return (
                            <Box sx={{ py: 2.5, px: 2, bgcolor: T.accentFaint, borderRadius: 1.5, border: `1px dashed ${T.accentBorder}`, textAlign: 'center' }}>
                              <Typography variant="body2" sx={{ color: T.accent, fontWeight: 500 }}>No existing record for this employee.</Typography>
                              <Typography variant="body2" sx={{ color: T.faint, mt: 0.5, fontSize: '0.82rem' }}>Use "Create new schedule" above to get started.</Typography>
                            </Box>
                          );
                        }

                        return (
                          <Box sx={{ border: `0.5px solid ${T.accentBorder}`, borderRadius: 1.5, overflow: 'hidden', bgcolor: alpha(T.accent, 0.02) }}>
                            {validSchedules.map((v, idx) => {
                              ...
                              const openView = () => {
                                ...
                                const rows = (records || []).filter((r) => normalizeDateStr(r.startDate) === normStart && normalizeDateStr(r.endDate) === normEnd);
                              ...
                              return (
                                <Box key={idx} sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 2, py: 1.25, borderBottom: idx < validSchedules.length - 1 ? `0.5px solid ${T.accentBorder}` : 'none', '&:hover': { bgcolor: T.accentFaint }, transition: 'background 0.15s' }}>
                                  <Box sx={{ width: 30, height: 30, borderRadius: 1.5, bgcolor: isActive ? T.accentFaint : alpha(T.accent, 0.03), display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                    <Schedule sx={{ fontSize: 15, color: isActive ? T.accent : T.faint }} />
                                  </Box>
                                  <Box sx={{ flex: 1, minWidth: 0 }}>
                                    <Typography sx={{ fontSize: '0.85rem', fontWeight: 600, color: T.text, lineHeight: 1.3 }}>{v.academicYear || '—'}</Typography>
                                    <Typography sx={{ fontSize: '0.75rem', color: T.faint, mt: 0.2 }}>
                                      {formatDateLong(v.startDate) || formatDateOnly(v.startDate)} – {formatDateLong(v.endDate) || formatDateOnly(v.endDate)}
                                    </Typography>
                                  </Box>
                                  <StatusBadge active={isActive} />
                                  <Tooltip title="View schedule">
                                    <IconButton size="small" onClick={openView} sx={{ width: 28, height: 28, border: `0.5px solid ${T.accentBorder}`, borderRadius: 1.5, color: T.faint, '&:hover': { borderColor: T.accent, color: T.accent, bgcolor: T.accentFaint } }}>
              ...
              <SectionCard sx={{ mb: 2 }}>
                <Box sx={{ px: 4, py: 3, background: 'linear-gradient(135deg, #fdf5f5 0%, #f0dede 100%)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', overflow: 'hidden' }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2.5, position: 'relative', zIndex: 1 }}>
                    ...
                    <Box>
                      ...
                      <Typography sx={{ fontSize: '0.78rem', color: T.accentMid, fontWeight: 600 }}>View and manage official time schedules for all users</Typography>
                    </Box>
                  </Box>
                </Box>
                ...
                <Box sx={{ p: 2.5 }}>
                  <Box sx={{ mb: 2.5, display: 'flex', gap: 1.5, alignItems: 'center', flexWrap: 'wrap' }}>
                    <ModernTextField fullWidth size="small" placeholder="Search by name or employee number..." value={searchQuery}
                      onChange={(e) => { setSearchQuery(e.target.value); setAllUsersPage(0); }}
                      InputProps={{ startAdornment: <InputAdornment position="start"><SearchIcon sx={{ color: T.accentMid }} /></InputAdornment> }}
                    />
                    <Button variant="contained" size="small" onClick={openBulkBlocksModalForSelected}
                      disabled={selectedUsers.size === 0 || settingDefault} startIcon={<CheckCircleIcon />}
                      sx={{ bgcolor: T.accent, color: '#fff', minWidth: 200, flexShrink: 0, textTransform: 'none', fontWeight: 600, '&:hover': { bgcolor: T.accentDark } }}>
                      Bulk Create (Selected: {selectedUsers.size})
                    </Button>
                    <Button variant="outlined" size="small" onClick={openBulkBlocksModalForAllMissing}
                      disabled={settingDefault || allUsers.filter((u) => !u.hasDefaultOfficialTime).length === 0}
                      sx={{ borderColor: T.accentBorder, color: T.accent, minWidth: 200, flexShrink: 0, textTransform: 'none', fontWeight: 600, '&:hover': { bgcolor: T.accentFaint, borderColor: T.accent } }}>
                  ...
                  {loadingUsers ? (
                    ...
                    <>
                          ...
                          <TableHead>
                            <TableRow sx={{ bgcolor: T.accent }}>
                              ...
                              {['Employee Number','Name','Department','Academic Year','Status','Start Date','End Date'].map((h) => (
                                <PremiumTableCell key={h} isHeader sx={{ color: '#fff', bgcolor: T.accent, fontSize: '0.7rem', letterSpacing: '0.07em', textTransform: 'uppercase' }}>{h}</PremiumTableCell>
                              ))}
                            </TableRow>
                          </TableHead>
                          <TableBody>
                            {paginatedAllUsers.map((user) => (
                              <TableRow key={user.employeeNumber} sx={{ '&:nth-of-type(even)': { bgcolor: T.rowOdd }, '&:hover': { bgcolor: T.rowHover }, transition: 'background 0.15s' }}>
                                <PremiumTableCell>
                                  <Checkbox checked={selectedUsers.has(user.employeeNumber)} onChange={() => handleUserSelect(user.employeeNumber)} sx={{ color: T.accentBorder, '&.Mui-checked': { color: T.accent } }} size="small" />
                                </PremiumTableCell>
                      ...
                      <Box sx={{ borderTop: `1px solid ${T.divider}` }}>
                        <TablePagination component="div" count={filteredAllUsers.length} page={allUsersPage}
          ...
          <Dialog open={showScheduleModal}
            ...
            PaperProps={{ sx: { borderRadius: 2, overflow: 'hidden', width: isBulkSchedule ? '90vw' : '75vw', maxWidth: isBulkSchedule ? '90vw' : '75vw' } }}
          >
                ...
                <Box sx={{ minWidth: 0 }}>
                  ...
                  <Typography sx={{ color: 'rgba(255,255,255,0.8)', fontSize: '0.78rem', mt: 0.25 }}>
                    {isBulkSchedule
                      ...
                      : (() => { const id = String(employeeID || '').trim(); const name = resolveEmployeeName(id); return id ? (name ? `${id} — ${name}` : `Employee No. ${id}`) : 'No employee selected'; })()}
                  </Typography>
                </Box>
              </Box>
            ...
            {isBulkSchedule && (
              <Box sx={{ bgcolor: '#f7f0f0', borderBottom: `1px solid ${T.accentBorder}`, px: 3, py: 2 }}>
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  <Box sx={{ flex: 1, height: 2, mx: 1.5, borderRadius: 1, bgcolor: T.accent }} />
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flex: 1 }}>
                    <Box sx={{ width: 26, height: 26, borderRadius: '50%', bgcolor: T.accent, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <Typography sx={{ color: '#fff', fontSize: '0.75rem', fontWeight: 700 }}>2</Typography>
                    </Box>
                    <Box>
                      <Typography sx={{ fontSize: '0.65rem', color: T.accent, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Current</Typography>
                      <Typography sx={{ fontSize: '0.82rem', color: T.text, fontWeight: 600 }}>Set time schedule</Typography>
                    </Box>
                  </Box>
                </Box>
              </Box>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: isBulkSchedule ? '220px 1fr' : '1fr' }}>
              {isBulkSchedule && (
                <Box sx={{ borderRight: `0.5px solid ${T.accentBorder}`, px: 1.75, py: 2, bgcolor: '#faf9f8', overflowY: 'auto', maxHeight: 560 }}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.6 }}>
                    {bulkTargetEmployees.map((empId) => {
                      ...
                      return (
                        <Box key={empId} sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 1, py: 0.75, bgcolor: '#fff', border: `0.5px solid ${T.accentBorder}`, borderRadius: '4px', minWidth: 0 }}>
                          <Box sx={{ width: 24, height: 24, borderRadius: '50%', bgcolor: T.accentFaint, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            <Typography sx={{ fontSize: '0.6rem', fontWeight: 700, color: T.accent }}>{initials}</Typography>
                          </Box>
                          <Box sx={{ minWidth: 0 }}>
                            <Typography sx={{ fontSize: '0.75rem', fontWeight: 600, color: T.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{u?.fullName || empId}</Typography>
              ...
              <Box sx={{ bgcolor: '#fff', px: 3, pt: 2.5, pb: 0, minWidth: 0, overflow: 'hidden' }}>
                {isBulkSchedule && (
                  <Box sx={{ border: `0.5px solid ${T.divider}`, borderRadius: '4px', overflow: 'hidden', mb: 2.5 }}>
                    <Box sx={{ bgcolor: '#f5f5f5', borderBottom: `0.5px solid ${T.divider}`, px: 2, py: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <Box sx={{ px: 2, py: 1.25, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5 }}>
                      {[{ label: 'Academic year', value: draftAcademicYear }, { label: 'Semester', value: draftSemester }, { label: 'Start date', value: formatDateLong(draftStartDate) || formatDateOnly(draftStartDate) }, { label: 'End date', value: formatDateLong(draftEndDate) || formatDateOnly(draftEndDate) }].map(({ label, value }) => (
                        <Box key={label}>
                ...
                {!isBulkSchedule && (
                  <Box sx={{ bgcolor: '#f7f0f0', border: `1px solid ${T.accentBorder}`, borderRadius: 1.5, p: 2, mb: 2.5 }}>
                    <Typography sx={{ fontSize: '0.72rem', fontWeight: 700, color: T.accent, textTransform: 'uppercase', letterSpacing: '0.4px', mb: 1.5 }}>Schedule Period</Typography>
                    <Grid container spacing={1.5}>
                      <Grid item xs={6} sm={3}>
                        <AcademicYearAutocomplete value={draftAcademicYear} onChange={setDraftAcademicYear} />
                      </Grid>
                      <Grid item xs={6} sm={3}>
                        <Autocomplete freeSolo options={['1st Semester','2nd Semester','Summer','Vacation','Christmas break','Midyear','Enrollment period']}
                          value={draftSemester || null}
                          ...
                          renderInput={(params) => (
                            <TextField {...params} size="small" label="Semester" placeholder="e.g. 1st Semester"
                              sx={{ bgcolor: '#fff', '& .MuiOutlinedInput-root.Mui-focused fieldset': { borderColor: T.accent }, '& label.Mui-focused': { color: T.accent } }} />
                          )}
                        />
                      </Grid>
                      {[{ label: 'Start Date', value: draftStartDate, setter: setDraftStartDate }, { label: 'End Date', value: draftEndDate, setter: setDraftEndDate }].map(({ label, value, setter }) => (
                  ...
                  <Box sx={{ display: 'flex', gap: 1, flexShrink: 0 }}>
                    <Tooltip title={`Clear all times on the "${scheduleView}" tab`}>
                      <Button size="small" variant="outlined" onClick={handleClearModalTimes} startIcon={<ClearAll fontSize="small" />}
                        sx={{ borderColor: T.accentBorder, color: T.accent, fontWeight: 600, textTransform: 'none', fontSize: '0.78rem', px: 1.5, py: 0.7, '&:hover': { bgcolor: T.accentFaint, borderColor: T.accent } }}>
                        Clear Tab
                      </Button>
                    </Tooltip>
                    <Tooltip title="Reset all times back to default (8:00 AM – 5:00 PM)">
                      <Button size="small" variant="outlined" onClick={handleResetModalToDefault}
                        sx={{ borderColor: T.accentBorder, color: '#555', fontWeight: 600, textTransform: 'none', fontSize: '0.78rem', px: 1.5, py: 0.7, '&:hover': { bgcolor: '#f5f5f5', borderColor: '#555' } }}>
                        Reset to Default
                      </Button>
                    </Tooltip>
                  </Box>
                ...
                <TableContainer sx={{ border: `1px solid ${T.divider}`, borderRadius: 1.5, overflow: 'auto', mb: 3 }}>
                  <Table size="small" sx={{ minWidth: isBulkSchedule ? 600 : 'auto' }}>
                    <ScheduleTimeRows records={modalRecords} onChangeRecord={handleModalRecordChange} scheduleView={scheduleView} readOnly={false} />
                  </Table>
                </TableContainer>
              </Box>
            </Box>
            ...
            <Box sx={{ borderTop: `1px solid ${T.divider}`, bgcolor: '#fafafa', px: 3, py: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <Box>
                {isBulkSchedule && (
                  <Button variant="outlined" onClick={() => { setShowScheduleModal(false); setShowBulkBlocksModal(true); }} startIcon={<ArrowBack fontSize="small" />}
                    sx={{ borderColor: T.accent, color: T.accent, fontWeight: 600, textTransform: 'none', '&:hover': { bgcolor: T.accentFaint } }}>
          ...
          {/* ── Bulk Confirmation ── */}
          <Dialog open={showBulkConfirmModal} onClose={() => { setShowBulkConfirmModal(false); setPendingBulkSubmit(false); }}
            maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: '16px', overflow: 'hidden' } }}>
            <Box sx={dialogHeaderSx}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Box sx={{ bgcolor: '#fff', px: 3, pt: 3, pb: 2 }}>
              <Typography sx={{ color: T.text, lineHeight: 1.7, fontSize: '0.93rem' }}>
                ...
                <Box component="span" sx={{ fontWeight: 700, color: T.accent }}>{bulkTargetEmployees.length} employee{bulkTargetEmployees.length !== 1 ? 's' : ''}</Box>{' '}
                across{' '}
                <Box component="span" sx={{ fontWeight: 700, color: T.accent }}>{bulkScheduleBlocks.length} block{bulkScheduleBlocks.length !== 1 ? 's' : ''}</Box>.
              </Typography>
            ...
            <Box sx={{ borderTop: `1px solid ${T.divider}`, bgcolor: '#fafafa', px: 3, py: 2, display: 'flex', justifyContent: 'flex-end', gap: 1.5 }}>
              <Button variant="outlined" onClick={() => { setShowBulkConfirmModal(false); setPendingBulkSubmit(false); }}
                sx={{ borderColor: '#ccc', color: '#444', fontWeight: 600, textTransform: 'none', '&:hover': { bgcolor: '#f5f5f5' } }}>Cancel</Button>
          ...
          <Dialog open={showConflictModal} onClose={() => setShowConflictModal(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: '16px', overflow: 'hidden' } }}>
            ...
            <Box sx={{ bgcolor: '#fff', px: 3, pt: 2.5, pb: 2 }}>
              ...
              <Box sx={{ mt: 1.5, mb: 2, px: 2, py: 1.25, bgcolor: T.accentFaint, border: `1px solid ${T.accentBorder}`, borderRadius: '10px', display: 'flex', alignItems: 'center', gap: 1 }}>
                <Schedule sx={{ color: T.accent, fontSize: 16, flexShrink: 0 }} />
            ...
            <Box sx={{ borderTop: `1px solid ${T.divider}`, bgcolor: '#fafafa', px: 3, py: 2, display: 'flex', justifyContent: 'flex-end' }}>
              <Button variant="contained" disableElevation onClick={() => setShowConflictModal(false)}
                sx={{ bgcolor: T.accent, color: '#fff', fontWeight: 700, textTransform: 'none', minWidth: 80, '&:hover': { bgcolor: T.accentDark } }}>Got It</Button>
            </Box>
          </Dialog>

          {/* ── Bulk Schedule Blocks (Step 1) ── */}
          <Dialog open={showBulkBlocksModal} onClose={() => setShowBulkBlocksModal(false)} maxWidth={false}
            PaperProps={{ sx: { borderRadius: '16px', overflow: 'hidden', width: 720, maxWidth: 720 } }}>
            <Box sx={dialogHeaderSx}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Box sx={{ bgcolor: '#f7f0f0', borderBottom: `1px solid ${T.accentBorder}`, px: 3, py: 2 }}>
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flex: 1 }}>
                  <Box sx={{ width: 28, height: 28, borderRadius: '50%', bgcolor: T.accent, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <Typography sx={{ color: '#fff', fontSize: '0.75rem', fontWeight: 700 }}>1</Typography>
                  </Box>
                  <Box>
                    <Typography sx={{ fontSize: '0.65rem', color: T.accent, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Current</Typography>
                    <Typography sx={{ fontSize: '0.82rem', color: T.text, fontWeight: 600 }}>Define schedule period</Typography>
                  </Box>
                </Box>
                <Box sx={{ flex: 1, height: 2, mx: 1.5, borderRadius: 1, background: `linear-gradient(90deg, ${T.accent} 0%, #e0c8c8 100%)` }} />
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flex: 1 }}>
                  <Box sx={{ width: 28, height: 28, borderRadius: '50%', bgcolor: '#fff', border: `1.5px solid ${T.accentBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <Box sx={{ px: 3, pt: 2.5, pb: 0, bgcolor: '#fff' }}>
              <Typography sx={{ fontSize: '0.70rem', fontWeight: 700, color: T.accent, textTransform: 'uppercase', letterSpacing: '0.5px', mb: 1 }}>
                Selected employees ({bulkTargetEmployees.length})
              </Typography>
              <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0.75, mb: 2.5, maxHeight: 200, overflowY: bulkTargetEmployees.length > 6 ? 'auto' : 'visible' }}>
                {bulkTargetEmployees.map((empId) => {
                  const u = allUsers.find((u) => String(u.employeeNumber) === String(empId));
                  const initials = (u?.fullName || String(empId)).split(' ').map((w) => w[0]).slice(0, 2).join('').toUpperCase();
                  return (
                    <Box key={empId} sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 1.25, py: 0.85, bgcolor: T.accentFaint, border: `0.5px solid ${T.accentBorder}`, borderRadius: '6px', minWidth: 0 }}>
                      <Box sx={{ width: 28, height: 28, borderRadius: '50%', bgcolor: alpha(T.accent, 0.12), display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <Typography sx={{ fontSize: '0.68rem', fontWeight: 700, color: T.accent }}>{initials}</Typography>
                      </Box>
                      <Box sx={{ minWidth: 0 }}>
                        <Typography sx={{ fontSize: '0.78rem', fontWeight: 600, color: T.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  ...
                  <Grid container spacing={1.5}>
                    ...
                    <Grid item xs={6}>
                      <Autocomplete freeSolo options={['1st Semester','2nd Semester','Summer','Vacation','Christmas break','Midyear','Enrollment period']}
                        ...
                        renderInput={(params) => (
                          <TextField {...params} size="small" label="Semester" placeholder="e.g. 1st Semester"
                            sx={{ '& .MuiOutlinedInput-root.Mui-focused fieldset': { borderColor: T.accent }, '& label.Mui-focused': { color: T.accent }, '& .MuiOutlinedInput-root': { borderRadius: '8px' } }} />
                        )}
                      />
                    </Grid>
          ...
          <Dialog open={showWarningModal} onClose={() => { setShowWarningModal(false); setWarningOverlap(null); }} maxWidth="sm" fullWidth
            ...
            <Box sx={{ bgcolor: '#fff', px: 3, pt: 2.5, pb: 2 }}>
              {warningOverlap ? (
                <Box>
                  <Typography sx={{ color: T.text, fontSize: '0.93rem', mb: 2, lineHeight: 1.65 }}>
                    Two time blocks overlap on <Box component="span" sx={{ fontWeight: 700, color: T.accent }}>{warningOverlap.day}</Box>
                    {warningOverlap.employeeID && warningOverlap.employeeID !== 'multiple' && (
                      <> for employee <Box component="span" sx={{ fontWeight: 700, color: T.accent }}>{warningOverlap.employeeID}</Box></>
                    )}:
                  </Typography>
                  <Box sx={{ border: `1px solid ${T.divider}`, borderRadius: '10px', overflow: 'hidden', mb: 2 }}>
                    <Box sx={{ bgcolor: T.accentFaint, px: 2, py: 1.5, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 1 }}>
                      <Box sx={{ px: 1.5, py: 0.5, borderRadius: '6px', bgcolor: T.accent, color: '#fff', fontWeight: 700, fontSize: '0.8rem' }}>{warningOverlap.segmentA?.label || 'Block A'}</Box>
                      <Typography sx={{ color: '#444', fontWeight: 600, fontSize: '0.85rem' }}>overlaps with</Typography>
                      <Box sx={{ px: 1.5, py: 0.5, borderRadius: '6px', bgcolor: T.text, color: '#fff', fontWeight: 700, fontSize: '0.8rem' }}>{warningOverlap.segmentB?.label || 'Block B'}</Box>
                    </Box>
                    <Box sx={{ px: 2, py: 1.25, bgcolor: '#fff', borderTop: `1px solid ${T.accentBorder}` }}>
                      <Typography sx={{ fontSize: '0.85rem', color: '#444' }}>
                        Overlapping period: <Box component="span" sx={{ fontWeight: 700, color: T.accent }}>{warningOverlap.overlap?.periodText || '—'}</Box>
                      </Typography>
                    </Box>
                  </Box>
                  <Typography sx={{ color: '#555', fontSize: '0.88rem', lineHeight: 1.6 }}>
            ...
            <Box sx={{ borderTop: `1px solid ${T.divider}`, bgcolor: '#fafafa', px: 3, py: 2, display: 'flex', justifyContent: 'flex-end' }}>
              <Button variant="contained" disableElevation onClick={() => { setShowWarningModal(false); setWarningOverlap(null); }}
                sx={{ bgcolor: T.accent, color: '#fff', fontWeight: 700, textTransform: 'none', minWidth: 90, '&:hover': { bgcolor: T.accentDark } }}>
                OK, Go Back
              </Button>
            </Box>
          </Dialog>
          ...
          <Dialog open={showViewScheduleModal}
            onClose={() => { setShowViewScheduleModal(false); setIsEditingViewSchedule(false); setEditViewRecords([]); setEditViewEndDate(''); }}
            maxWidth="xl" fullWidth
            PaperProps={{ sx: { borderRadius: '16px', overflow: 'hidden' } }}>
            <Box sx={dialogHeaderSx}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 0 }}>
                {isEditingViewSchedule ? <Edit sx={{ color: '#fff', fontSize: 20, flexShrink: 0 }} /> : <Visibility sx={{ color: '#fff', fontSize: 20, flexShrink: 0 }} />}
                <Box sx={{ minWidth: 0 }}>
                  ...
                  <Typography sx={{ color: 'rgba(255,255,255,0.75)', fontSize: '0.78rem', mt: 0.25 }}>
                    {(() => { const id = String(employeeID || '').trim(); const name = viewScheduleEmployeeName || resolveEmployeeName(id); return id ? (name ? `${id} — ${name}` : `Employee No. ${id}`) : 'Employee schedule'; })()}
                  </Typography>
                </Box>
              </Box>
          ...
          <Dialog open={showPreviewModal} onClose={() => setShowPreviewModal(false)} maxWidth="xl" fullWidth
                  ...
                  <TableContainer sx={{ border: `1px solid ${T.divider}`, borderRadius: '10px', overflow: 'hidden', mb: 3 }}>
                    <Table size="small">
                      <ScheduleTimeRows
                        ...
                        onChangeRecord={() => {}} scheduleView={previewViewScheduleView} readOnly />
                    </Table>
                  </TableContainer>
                </>
              )}
            ...
            <Box sx={{ borderTop: `1px solid ${T.divider}`, bgcolor: '#fafafa', px: 3, py: 2, display: 'flex', justifyContent: 'flex-end' }}>
              <Button variant="contained" disableElevation onClick={() => setShowPreviewModal(false)}
                sx={{ bgcolor: T.accent, color: '#fff', fontWeight: 700, textTransform: 'none', minWidth: 80, '&:hover': { bgcolor: T.accentDark } }}>Close</Button>
            </Box>
          </Dialog>

          {/* ── Analyze / Validate Modal ── */}
          <Dialog open={showAnalyzeModal}
            ...
            <Box sx={{ ...dialogHeaderSx, bgcolor: analyzeResult?.ok ? T.accent : '#b45309' }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                ...
                <Box>
                  ...
                  <Typography sx={{ color: 'rgba(255,255,255,0.72)', fontSize: '0.78rem', mt: 0.25 }}>
                    {analyzeResult?.ok ? 'Review the schedule details below before confirming the upload.' : 'Review the validation warnings below, fix your file, then re-upload.'}
                  </Typography>
                </Box>
              </Box>
            ...
            <Box sx={{ bgcolor: '#fff', px: 3, pt: 2.5, pb: 1, maxHeight: '65vh', overflowY: 'auto' }}>
              {analyzeResult?.ok ? (
                <>
                  <Typography sx={{ fontSize: '0.68rem', fontWeight: 700, color: T.accent, textTransform: 'uppercase', letterSpacing: '0.5px', mb: 1 }}>
            ...
            <Box sx={{ borderTop: `1px solid ${T.divider}`, bgcolor: '#fafafa', px: 3, py: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <Button variant="outlined" onClick={() => { setShowAnalyzeModal(false); setAnalyzeResult(null); setUploadAcknowledgeChecked(false); }} disabled={confirming}
                sx={{ borderColor: '#ccc', color: '#444', fontWeight: 600, textTransform: 'none', '&:hover': { bgcolor: '#f5f5f5' } }}>
