          ...
          >
            <Box
              sx={{
                ...
                width: 200,
                height: 200,
                background: `radial-gradient(circle,${alpha(ac, 0.1)} 0%,transparent 70%)`,
              }}
            />
            ...
            >
              ...
              >
                {Array.from({ length: 12 }, (_, i) => (
                  <S key={i} w={64} h={44} r={10} accent={ac} />
                ))}
              </Box>
            </Box>
            <Box
...
const VIEW_TABS = [
  ...
  },
  {
    ...
    icon: <CreditScore sx={{ fontSize: 16 }} />,
  },
  {
    key: 'overtime',
    label: 'Overtime',
...
const FloatingTotalsBar = ({
  ...
  const items = [
    {
      label: 'Morning Rendered',
      value: totals.morningRendered,
      group: 'regular',
    },
    {
      label: 'Morning Tardiness',
      value: totals.morningTardiness,
      group: 'regular',
    },
    {
      label: 'Afternoon Rendered',
      value: totals.afternoonRendered,
      group: 'regular',
    },
    {
      label: 'Afternoon Tardiness',
      value: totals.afternoonTardiness,
      group: 'regular',
    },
    {
      label: 'Overall Rendered',
      value: totals.overallRendered,
      group: 'regular',
      highlight: true,
    },
    {
      label: 'Overall Tardiness',
      value: totals.overallTardiness,
      group: 'regular',
      highlight: true,
    },
        ...
        >
            ...
            return (
              <Box
                ...
                sx={{
                  ...
                  transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                  '&:hover': {
                    boxShadow: `0 2px 8px ${alpha(accentColor, 0.22)}`,
                  },
                }}
              >
                {isActive && (
                ...
                <Typography
                  sx={{
                    fontSize: isActive ? '0.97rem' : '0.92rem',
                    fontWeight: 800,
                    color:
                      isActive || highlight ? accentColor : textPrimaryColor,
          ...
          >
            {saving ? (
              <CircularProgress
                size={18}
                sx={{ color: primaryColor, mb: 0.3 }}
              />
            ) : (
              <SaveAs sx={{ color: primaryColor, fontSize: 18, mb: 0.3 }} />
            )}
            <Typography
              sx={{
                fontSize: '0.72rem',
  ...
  return (
    <Box
      ...
      sx={{
        ...
        overflowY: 'hidden',
        height: 16,
        bgcolor: 'rgba(254,249,225,0.9)',
        borderTop: '1px solid rgba(109,35,35,0.15)',
        '&::-webkit-scrollbar': { height: 12 },
        '&::-webkit-scrollbar-track': {
          background: 'rgba(254,249,225,0.6)',
          borderRadius: 4,
        },
        '&::-webkit-scrollbar-thumb': {
          background: 'rgba(109,35,35,0.45)',
          borderRadius: 4,
          '&:hover': { background: 'rgba(109,35,35,0.7)' },
        },
      }}
    >
...
const AttendanceModuleNonTeachingStaff = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const processedData = rawRows.map((row) => {
        ...
        const am = calcSegment(
          timeIN,
          breaktimeIN,
          officialTimeIN,
          officialBreaktimeIN,
        );
        const pm = calcSegment(
          breaktimeOUT,
          timeOUT,
          officialBreaktimeOUT,
          officialTimeOUT,
        );
        const hn = calcSegment(
          timeIN,
          timeOUT,
          officialHonorariumTimeIN,
          officialHonorariumTimeOUT,
        );
        const sc = calcSegment(
          timeIN,
          timeOUT,
          officialServiceCreditTimeIN,
          officialServiceCreditTimeOUT,
        );
        const ot = calcSegment(
          timeIN,
          timeOUT,
          officialOverTimeIN,
          officialOverTimeOUT,
  ...
  if (hasAccess === false)
    return (
      <AccessDenied
        title="Access Denied"
        message="You do not have permission to access Attendance Module for Non-Teaching Staff."
      ...
      >
                ...
                >
                  <Box display="flex" alignItems="center">
                    ...
                    <Box>
                      ...
                      >
                        Generate and review all attendance records of
                    ...
                    <FormControl sx={{ minWidth: 140 }}>
                      ...
                      <Select
                        ...
                        onChange={(e) => {
                          ...
                          setSelectedMonth(null);
                          showSnackbar(
                            'Year changed — please click a month.',
                            'info',
                          );
                        }}
                        sx={{
        ...
        {attendanceData.length > 0 && (
          <Fade in={!loading} timeout={500}>
            <GlassCard
              ref={resultsRef}
              sx={{ mb: 17, border: `1px solid ${alpha(accentColor, 0.1)}` }}
            >
              {/* Banner */}
              <Box
                sx={{
                  ...
                  {VIEW_TABS.map(({ key, label, icon }, i, arr) => (
                    <Button
                      ...
                      sx={{
                        ...
                        py: 1.1,
                        bgcolor:
                          activeTab === key ? accentColor : 'transparent',
                  ...
                  >
                    ...
                    >
                      ...
                      <TableBody>
                        ...
                        >
                          {columns.map(
                            ({ key, accent: ac2, accentDark: adk2 }, ci) => {
                              if (ci === 0)
                                return (
                                  <PremiumTableCell
                                    key={key}
