          ...
          {items.map(({ label, value, group, highlight }) => {
            ...
            return (
              <Box key={label} sx={{
                ...
                minWidth: 88, px: 1, py: 1, borderRadius: '8px',
                bgcolor: isActive
                  ? (isTard ? 'rgba(153,27,27,0.12)' : T.accentFaint)
                  : (highlight ? alpha(T.accent, 0.08) : (isTard ? 'rgba(153,27,27,0.05)' : T.accentFaint)),
                border: `1px solid ${isActive ? T.accent : highlight ? alpha(T.accent, 0.3) : T.accentBorder}`,
...
const AttendanceModuleFacultyDesignated = () => {
  ...
  // ─── Column group spans for the two-row header ────────────────────────────
  const buildGroupSpans = (cols) => {
    ...
    cols.forEach((col) => {
      ...
      if (last && last.group === col.group && !col.dividerBefore) {
        ...
        groups.push({ label: col.group, span: 1, group: col.group, dividerBefore: col.dividerBefore });
      }
    });
  ...
  // totals row colspan for non-calc cols
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2.5, position: 'relative', zIndex: 1 }}>
              ...
              <Box>
                ...
                <Typography sx={{ fontSize: '0.78rem', color: T.accentMid, fontWeight: 600 }}>
                  40hrs · Designated Faculty · Generate and review attendance records
                </Typography>
              </Box>
            </Box>
                  ...
                  >
                    <Table sx={{ minWidth: columns.reduce((s, c) => s + (c.minWidth || 120), 0), borderCollapse: 'collapse' }}>
                      ...
                      <TableBody>
                        ...
                        {/* Overall row — only on Regular tab */}
                        {activeTab === 'regular' && (
                          <TableRow sx={{ bgcolor: alpha(T.accent, 0.05), borderTop: `1px solid ${T.accentBorder}` }}>
                            ...
                            <TableCell sx={{ fontFamily: 'monospace', fontWeight: 900, fontSize: '0.95rem', color: '#166534', textAlign: 'center', py: 1.25, borderBottom: 'none', borderLeft: `2px solid ${T.accentBorder}`, bgcolor: 'rgba(21,128,61,0.10)' }}>
                              {totals.overallRendered || '00:00:00'}
                            </TableCell>
                            <TableCell sx={{ fontFamily: 'monospace', fontWeight: 900, fontSize: '0.95rem', color: '#991b1b', textAlign: 'center', py: 1.25, borderBottom: 'none', bgcolor: 'rgba(153,27,27,0.10)' }}>
