...
const AttendanceDesignatedWireframe = () => (
  <>
    ...
    <Box sx={{ py: { xs: 1, md: 2 }, mt: { xs: 0, md: -2 }, mb: { xs: 1, md: 2 }, width: '100vw', maxWidth: '100%', position: 'relative', left: '53%', transform: 'translateX(-51%)', px: { xs: 2, sm: 3, md: 6 } }}>
...
const NativeInput = ({ value, onChange, type = 'text', placeholder, disabled, icon }) => (
  <Box sx={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
    ...
    <input
      ...
      style={{ width: '100%', padding: icon ? '9px 13px 9px 34px' : '9px 13px', borderRadius: '8px', border: `1px solid ${T.accentBorder}`, fontSize: '0.875rem', outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box', transition: 'border-color 0.18s', background: disabled ? '#f5f5f5' : '#fff', color: T.text, cursor: disabled ? 'not-allowed' : 'text' }}
...
const TAB_COLUMNS = {
  regular: [
    ...
    { label: 'Official Time IN',       key: 'officialTimeIN',        minWidth: 130, group: 'official', dividerBefore: true },
    { label: 'Breaktime IN',           key: 'breaktimeIN',           minWidth: 140, group: 'actual' },
    { label: 'Official Breaktime IN',  key: 'officialBreaktimeIN',   minWidth: 140, group: 'official' },
    { label: 'Morning Rendered',       key: '_morningRendered',      minWidth: 130, group: 'calc',     dividerBefore: true },
    { label: 'Morning Tardiness',      key: '_morningTardiness',     minWidth: 130, group: 'tard' },
    { label: 'Breaktime OUT',          key: 'breaktimeOUT',          minWidth: 130, group: 'actual',   dividerBefore: true },
    { label: 'Official Breaktime OUT', key: 'officialBreaktimeOUT',  minWidth: 140, group: 'official' },
    { label: 'Time OUT',               key: 'timeOUT',               minWidth: 120, group: 'actual' },
    { label: 'Official Time OUT',      key: 'officialTimeOUT',       minWidth: 130, group: 'official' },
    { label: 'Afternoon Rendered',     key: '_afternoonRendered',    minWidth: 130, group: 'calc',     dividerBefore: true },
    { label: 'Afternoon Tardiness',    key: '_afternoonTardiness',   minWidth: 130, group: 'tard' },
  ],
  honorarium: [
    { label: 'Date',                        key: 'date',                       minWidth: 130, group: 'meta' },
    { label: 'Day',                         key: 'day',                        minWidth: 100, group: 'meta' },
...
const FloatingTotalsBar = ({ totals, visible, onSave, saving, activeTab, startDate, endDate }) => {
  ...
  return (
    <Paper elevation={12} sx={{ borderRadius: '12px', overflow: 'hidden', minWidth: 340, width: 'fit-content', mx: 'auto', boxShadow: `0 8px 40px ${alpha(T.accent, 0.35)}, 0 2px 10px ${alpha(T.accent, 0.12)}`, border: `1.5px solid ${T.accentBorder}`, bgcolor: '#fff', backdropFilter: 'blur(20px)', transition: 'all 0.25s ease', mt: 2, mb: 2 }}>
      <Box onClick={() => setExpanded(p => !p)} sx={{ px: 2.5, py: 1.25, background: `linear-gradient(135deg, ${T.accent} 0%, ${T.accentDark} 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', userSelect: 'none' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
          <WorkHistory sx={{ color: '#fff', fontSize: 14 }} />
          <Typography sx={{ color: '#fff', fontWeight: 700, fontSize: '0.74rem', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Attendance Summary</Typography>
          {startDate && endDate && (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.6, px: 1, py: 0.2, borderRadius: '5px', bgcolor: 'rgba(255,255,255,0.18)' }}>
              <Typography sx={{ color: '#fff', fontWeight: 700, fontSize: '0.67rem', fontFamily: 'monospace' }}>{startDate} – {endDate}</Typography>
            </Box>
          )}
        </Box>
      ...
      <Collapse in={expanded}>
        <Box sx={{ px: 2, py: 1.75, display: 'flex', flexWrap: 'nowrap', gap: 1, alignItems: 'stretch', overflowX: 'auto', '&::-webkit-scrollbar': { height: 4 }, '&::-webkit-scrollbar-thumb': { background: T.accentBorder, borderRadius: 2 } }}>
          {items.map(({ label, value, group, highlight }) => {
            const isActive = group === activeTab;
            const isTard = label.includes('Tardiness');
            return (
              <Box key={label} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 88, px: 1, py: 1, borderRadius: '8px', bgcolor: isActive ? (isTard ? 'rgba(153,27,27,0.12)' : T.accentFaint) : (highlight ? alpha(T.accent, 0.08) : (isTard ? 'rgba(153,27,27,0.05)' : T.accentFaint)), border: `1px solid ${isActive ? T.accent : highlight ? alpha(T.accent, 0.3) : T.accentBorder}`, boxShadow: isActive ? `0 0 10px ${alpha(T.accent, 0.22)}` : 'none', transform: isActive ? 'translateY(-2px) scale(1.03)' : 'none', transition: 'all 0.2s ease' }}>
                <Typography sx={{ fontSize: '0.6rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', mb: 0.25, textAlign: 'center', lineHeight: 1.2, color: isActive ? T.accent : T.faint }}>{label}</Typography>
                <Typography sx={{ fontSize: isActive ? '0.95rem' : '0.88rem', fontWeight: 800, color: isTard ? '#991b1b' : '#166534', fontFamily: 'monospace', letterSpacing: '0.04em', transition: 'all 0.2s' }}>{value || '00:00:00'}</Typography>
              </Box>
            );
          })}
          <Box onClick={!saving ? onSave : undefined} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minWidth: 120, px: 1.5, py: 1, borderRadius: '8px', bgcolor: T.accent, border: `1px solid ${T.accent}`, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1, boxShadow: `0 4px 14px ${alpha(T.accent, 0.35)}`, flexShrink: 0, transition: 'all 0.18s ease', '&:hover': !saving ? { boxShadow: `0 6px 20px ${alpha(T.accent, 0.5)}`, transform: 'translateY(-1px)' } : {} }}>
            {saving ? <CircularProgress size={16} sx={{ color: '#fff', mb: 0.3 }} /> : <SaveAs sx={{ color: '#fff', fontSize: 16, mb: 0.3 }} />}
            <Typography sx={{ fontSize: '0.7rem', color: '#fff', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em', textAlign: 'center', lineHeight: 1.2 }}>{saving ? 'Saving…' : 'Save Record'}</Typography>
          </Box>
        </Box>
      </Collapse>
    </Paper>
...
const AttendanceModuleFacultyDesignated = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Step 1: Pre-flight — check raw device records (no officialtime join required)
      ...
      // Step 2: Fetch with official time join
      ...
      const processedData = rawRows.map((row) => {
        ...
        const effectiveBreaktimeIN  = officialBreaktimeIN  && officialBreaktimeIN  !== '00:00:00 AM' ? officialBreaktimeIN  : breaktimeIN;
        const effectiveBreaktimeOUT = officialBreaktimeOUT && officialBreaktimeOUT !== '00:00:00 AM' ? officialBreaktimeOUT : breaktimeOUT;
        ...
        const formattedFacultyRenderedTimeAM    = [Math.floor(diffMsAM/3600000), Math.floor((diffMsAM%3600000)/60000), Math.floor((diffMsAM%60000)/1000)].map(x => String(x).padStart(2,'0')).join(':');
        const diffMsAMMax = endOfficialTimeFacultyAM - startOfficialTimeFacultyAM;
        const formattedFacultyMaxRenderedTimeAM = [Math.floor(diffMsAMMax/3600000), Math.floor((diffMsAMMax%3600000)/60000), Math.floor((diffMsAMMax%60000)/1000)].map(x => String(x).padStart(2,'0')).join(':');
        const tardAM = new Date(`01/01/2000 ${formattedFacultyMaxRenderedTimeAM}`) - new Date(`01/01/2000 ${formattedFacultyRenderedTimeAM}`);
        const formattedfinalcalcFacultyAM = [Math.floor(tardAM/3600000), Math.floor((tardAM%3600000)/60000), Math.floor((tardAM%60000)/1000)].map(x => String(x).padStart(2,'0')).join(':');
        ...
        const formattedFacultyRenderedTimePM    = [Math.floor(diffMsPM/3600000), Math.floor((diffMsPM%3600000)/60000), Math.floor((diffMsPM%60000)/1000)].map(x => String(x).padStart(2,'0')).join(':');
        const diffMsPMMax = endOfficialTimeFacultyPM - startOfficialTimeFacultyPM;
        const formattedFacultyMaxRenderedTimePM = [Math.floor(diffMsPMMax/3600000), Math.floor((diffMsPMMax%3600000)/60000), Math.floor((diffMsPMMax%60000)/1000)].map(x => String(x).padStart(2,'0')).join(':');
        const tardPM = new Date(`01/01/2000 ${formattedFacultyMaxRenderedTimePM}`) - new Date(`01/01/2000 ${formattedFacultyRenderedTimePM}`);
        const formattedfinalcalcFacultyPM = [Math.floor(tardPM/3600000), Math.floor((tardPM%3600000)/60000), Math.floor((tardPM%60000)/1000)].map(x => String(x).padStart(2,'0')).join(':');
        ...
        return {
          ...row,
          breaktimeIN: effectiveBreaktimeIN, breaktimeOUT: effectiveBreaktimeOUT,
  ...
  if (hasAccess === false) return (
    <AccessDenied title="Access Denied" message="You do not have permission to access Attendance Module for Faculty (40 hours/Designated). Contact your administrator to request access." returnPath="/admin-home" returnButtonText="Return to Home" />
  );
      ...
      <TableHead>
        ...
        <TableRow>
          {cols.map(({ label, minWidth, group, dividerBefore }) => (
            <TableCell key={label} sx={{ minWidth: minWidth || 120, textAlign: 'center', position: 'sticky', top: 32, zIndex: 2, fontSize: '0.65rem', fontWeight: 700, py: 0.85, px: 1.75, whiteSpace: 'nowrap', letterSpacing: '0.06em', textTransform: 'uppercase', borderBottom: `2px solid ${alpha(T.accent, 0.25)}`, borderLeft: dividerBefore ? `2px solid ${alpha(T.accent, 0.3)}` : 'none', bgcolor: hBg2(group), color: '#fff', background: group === 'actual' ? T.accent : group === 'calc' ? '#166534' : group === 'tard' ? '#991b1b' : group === 'official' ? T.accentMid : T.accentDark }}>{label}</TableCell>
          ))}
        </TableRow>
      </TableHead>
    );
  ...
  const buildCell = (content, group, isEven, dividerBefore = false) => (
    <TableCell sx={{ fontSize: '0.8rem', fontFamily: 'monospace', borderBottom: `1px solid ${T.divider}`, borderLeft: group === 'actual' ? `3px solid ${alpha(T.accent, 0.4)}` : dividerBefore ? `2px solid ${T.accentBorder}` : 'none', px: 1.75, py: 1, whiteSpace: 'nowrap', textAlign: 'center', fontWeight: group === 'calc' || group === 'tard' || group === 'actual' ? 700 : 400, color: group === 'calc' ? '#166534' : group === 'tard' ? '#991b1b' : group === 'official' ? '#374151' : '#111827', bgcolor: group === 'actual' ? (isEven ? alpha(T.accent,0.07) : alpha(T.accent,0.12)) : group === 'calc' ? (isEven ? 'rgba(21,128,61,0.05)' : 'rgba(21,128,61,0.09)') : group === 'tard' ? (isEven ? 'rgba(153,27,27,0.04)' : 'rgba(153,27,27,0.08)') : isEven ? '#fff' : T.rowOdd, transition: 'background-color 0.12s', 'tr:hover &': { bgcolor: T.rowHover + ' !important' } }}>{content}</TableCell>
  );

  const calcColCount  = columns.filter(c => c.group === 'calc' || c.group === 'tard').length;
  const nonCalcCount  = columns.length - calcColCount;

  return (
    <Fade in timeout={400}>
      <Box sx={{ py: { xs: 1, md: 2 }, mt: { xs: 0, md: -2 }, mb: 0, width: '100vw', maxWidth: '100%', position: 'relative', left: '53%', transform: 'translateX(-51%)', px: { xs: 2, sm: 3, md: 6 } }}>
        <style>{shimmerKf}</style>
        ...
        <SectionCard sx={{ mb: 2 }}>
          ...
          <Box sx={{ px: 2.5, pt: 2, pb: 2.5 }}>
            <Box sx={{ display: 'flex', gap: 1.5, mb: 2.5, flexWrap: 'wrap' }}>
              {[
                { label: 'Employee Number', value: employeeNumber, onChange: (e) => setEmployeeNumber(e.target.value), placeholder: 'Employee number', icon: <Person sx={{ fontSize: 16 }} />, type: 'text' },
                { label: 'Start Date', value: startDate, onChange: (e) => setStartDate(e.target.value), icon: <CalendarToday sx={{ fontSize: 15 }} />, type: 'date' },
                { label: 'End Date', value: endDate, onChange: (e) => setEndDate(e.target.value), icon: <CalendarToday sx={{ fontSize: 15 }} />, type: 'date' },
              ].map(({ label, value, onChange, placeholder, icon, type }) => (
                <Box key={label} sx={{ flex: 1, minWidth: 160 }}>
                  ...
                  <NativeInput type={type} value={value} onChange={onChange} placeholder={placeholder} icon={icon} />
                </Box>
              ))}
            </Box>
            ...
            <Box sx={{ p: 2.5, borderRadius: 2, border: `2px dashed ${T.accentBorder}`, bgcolor: T.accentFaint }}>
              <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: { xs: 'flex-start', sm: 'center' }, justifyContent: 'space-between', gap: 2, mb: 2 }}>
                ...
                <FormControl sx={{ minWidth: 130 }} size="small">
                  ...
                  <Select value={selectedYear} label="Year" onChange={(e) => { setSelectedYear(e.target.value); setSelectedMonth(null); showSnackbar('Year changed — please click a month to load records.', 'info'); }} sx={{ bgcolor: '#fff', borderRadius: 2, fontWeight: 600, fontSize: '0.85rem', '& .MuiOutlinedInput-notchedOutline': { borderColor: T.accentBorder } }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 2, flexWrap: 'wrap', gap: 1 }}>
              ...
              <RowBtn icon={loading ? <CircularProgress size={12} sx={{ color: T.accent }} /> : <Search sx={{ fontSize: 13 }} />} label={loading ? 'Loading…' : 'Search Records'} color={T.accent} hoverBg={T.accentFaint} disabled={!employeeNumber || !startDate || !endDate || loading} onClick={handleSubmit} />
            </Box>
          </Box>
        </SectionCard>
        ...
        {attendanceData.length > 0 && (
          <Fade in={!loading} timeout={400}>
            <SectionCard ref={resultsRef} sx={{ mb: 2 }}>
              <PanelHeader icon={Assignment} title={`Records for ${employeeNumber}`}
                rightContent={
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    ...
                    <Box sx={{ px: 1.5, py: 0.3, borderRadius: 5, bgcolor: alpha(T.accent, 0.1), border: `1px solid ${alpha(T.accent, 0.18)}` }}>
                      <Typography sx={{ fontSize: '0.72rem', color: T.accent, fontWeight: 700 }}>{attendanceData.length} {attendanceData.length === 1 ? 'record' : 'records'}</Typography>
                    </Box>
                  </Box>
                }
              />
              ...
              <Box sx={{ px: 2.5, pb: 2.5 }}>
                <Box sx={{ position: 'relative', borderRadius: '8px', border: `1px solid ${T.accentBorder}`, overflow: 'hidden' }}>
                  <Box ref={tableBodyRef} sx={{ overflowX: 'auto', overflowY: 'auto', maxHeight: 500, scrollbarWidth: 'thin', '&::-webkit-scrollbar': { height: 6, width: 6 }, '&::-webkit-scrollbar-track': { background: T.accentFaint, borderRadius: 4 }, '&::-webkit-scrollbar-thumb': { background: T.accentMid, borderRadius: 4 } }}>
                    <Table sx={{ minWidth: columns.reduce((s, c) => s + (c.minWidth || 120), 0), borderCollapse: 'collapse' }}>
                      ...
                      <TableBody>
                        {attendanceData.map((row, ri) => {
                          ...
                          const isEven = ri % 2 === 0;
                          return (
                            <TableRow key={ri} sx={{ '&:hover td': { bgcolor: `${T.rowHover} !important` } }}>
                              {columns.map(({ key, group, dividerBefore: db }) => {
                                ...
                                if (key === 'day') {
                                  return <TableCell key={key} sx={{ fontSize: '0.8rem', color: T.muted, bgcolor: isEven ? '#fff' : T.rowOdd, borderBottom: `1px solid ${T.divider}`, px: 1.75, py: 1, textAlign: 'center', transition: 'background-color 0.12s' }}>{row.day}</TableCell>;
                                }
                                return <React.Fragment key={key}>{buildCell(getCellValue(row, key, isFurlough), group, isEven, db)}</React.Fragment>;
                              })}
                            </TableRow>
                          );
                        })}
                        ...
                        <TableRow sx={{ bgcolor: T.accentFaint, borderTop: `2px solid ${T.accentBorder}` }}>
                          <TableCell colSpan={nonCalcCount} sx={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: T.faint, textAlign: 'right', pr: 2.5, py: 1.25, borderBottom: 'none' }}>
                            Overall Rendered Time ({startDate} – {endDate})
                          </TableCell>
                          {tabTotals.map((val, ti) => {
                            const isRendered = ti % 2 === 0;
                            return (
                              <TableCell key={ti} sx={{ fontFamily: 'monospace', fontWeight: 800, fontSize: '0.88rem', color: isRendered ? '#166534' : '#991b1b', textAlign: 'center', py: 1.25, borderBottom: 'none', borderLeft: ti === 0 ? `2px solid ${T.accentBorder}` : 'none', bgcolor: isRendered ? 'rgba(21,128,61,0.07)' : 'rgba(153,27,27,0.07)' }}>
                                {val || '00:00:00'}
                              </TableCell>
                            );
                          })}
                        ...
                        {activeTab === 'regular' && (
                          <TableRow sx={{ bgcolor: alpha(T.accent, 0.05), borderTop: `1px solid ${T.accentBorder}` }}>
                            <TableCell colSpan={nonCalcCount + calcColCount - 2} sx={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: T.faint, textAlign: 'right', pr: 2.5, py: 1.25, borderBottom: 'none' }}>
                              Overall (AM + PM) — {startDate} → {endDate}
                            </TableCell>
                            <TableCell sx={{ fontFamily: 'monospace', fontWeight: 900, fontSize: '0.95rem', color: '#166534', textAlign: 'center', py: 1.25, borderBottom: 'none', borderLeft: `2px solid ${T.accentBorder}`, bgcolor: 'rgba(21,128,61,0.10)' }}>{totals.overallRendered || '00:00:00'}</TableCell>
                            <TableCell sx={{ fontFamily: 'monospace', fontWeight: 900, fontSize: '0.95rem', color: '#991b1b', textAlign: 'center', py: 1.25, borderBottom: 'none', bgcolor: 'rgba(153,27,27,0.10)' }}>{totals.overallTardiness || '00:00:00'}</TableCell>
                          </TableRow>
                        )}
                      </TableBody>
                    </Table>
        ...
        <FloatingTotalsBar totals={totals} visible={attendanceData.length > 0} onSave={saveOverallAttendance} saving={saving} activeTab={activeTab} startDate={startDate} endDate={endDate} />
