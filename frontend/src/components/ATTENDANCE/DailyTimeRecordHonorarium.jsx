  ...
  return (
    <Box sx={{
      ...
      <Box sx={{ p: '12px 10px 8px', borderBottom: border, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '5px' }}>
        ...
        {/* Title — slightly wider to reflect "DAILY TIME RECORD - HONORARIUM" */}
        <S w="78%" h={16} r={4} accent={accent} sx={{ my: '3px' }} />
...
const DTRHonorariumWireframeLoading = ({
  accentColor    = '#6d2323',
  primaryColor   = '#FEF9E1',
    ...
    <>
      ...
      <Container maxWidth="xl" sx={{ py: 4, mt: -5 }}>
        <Box sx={{ px: { xs: 2, sm: 4, md: 6 } }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {/* Wider title for "Daily Time Record - Honorarium" */}
                    <S w={310} h={26} r={6} accent={ac} />
                    <S w={270} h={13} r={4} accent={ac} />
                  </Box>
                </Box>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  <S w={100} h={26} r={13} accent={ac} />
          ...
          {/* 4. Bottom action buttons (Print + Download PDF) */}
          <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, mt: 2, mb: 4, animation: 'dtrPulse 2.2s ease-in-out 0.25s infinite' }}>
            <Placeholder w={130} h={52} r={12} color={alpha(ac, 0.2)} />
            <Placeholder w={160} h={52} r={12} color={alpha(ac, 0.2)} />
          </Box>
...
const DailyTimeRecordHonorarium = () => {
  ...
  const getOfficialTimesForDate = (fullDate) => {
    ...
    return officialTimes[dayMap[new Date(fullDate).getDay()]] || {};
  };
  ...
  const getDateIndicator = (dateString) => {
    ...
    if (susp) return { label: 'SUSPENSION', bgColor: 'rgba(211,47,47,0.2)', textColor: '#000000', borderColor: '#d32f2f' };
    const hol = holidays.find((h) => isDateInRange(date, h.date_start || h.date, h.date_end || h.date));
    if (hol) return { label: 'HOLIDAY', bgColor: 'rgba(237,108,2,0.25)', textColor: '#000000', borderColor: '#ed6c02' };
    return null;
  };

  // ── Access / loading guards ────────────────────────────────────────────────
  if (pageLoading || accessLoading) return (
    <DTRHonorariumWireframeLoading
  ...
  const cellStyle = {
    ...
    fontFamily: 'Arial,serif', fontSize: '10px', height: '16px', whiteSpace: 'nowrap',
  };
  ...
  const renderTableRows = () =>
    Array.from({ length: 31 }, (_, i) => {
      ...
      const bg  = ind ? ind.bgColor : 'transparent';

      return (
        <tr key={i}>
          <td style={{ ...cellStyle, backgroundColor: bg }}>{day}</td>
          {/* AM Arrival */}
          <td style={{ ...cellStyle, backgroundColor: bg, position: 'relative' }}>
            {ind && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: bg, zIndex: 0, opacity: 0.3 }} />}
          ...
          {/* Undertime / indicator label */}
          <td style={{ ...cellStyle, backgroundColor: bg, position: 'relative' }}>
            {ind && (
              <>
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: bg, zIndex: 0, opacity: 0.3 }} />
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '7px', fontWeight: 'bold', color: ind.textColor, backgroundColor: bg, zIndex: 1, pointerEvents: 'none', opacity: 0.9 }}>
                  {ind.label}
                </div>
              </>
            )}
    ...
    <Fade in timeout={500}>
      <Container maxWidth="xl" sx={{ py: 4, mt: -5 }}>
        ...
        <Box sx={{ px: { xs: 2, sm: 4, md: 6 } }}>
                ...
                <Box sx={{ p: 5, background: `linear-gradient(135deg,${primaryColor} 0%,${secondaryColor} 100%)`, color: textPrimaryColor, position: 'relative', overflow: 'hidden' }}>
                  ...
                  <Box display="flex" alignItems="center" justifyContent="space-between" position="relative" zIndex={1}>
                    <Box display="flex" alignItems="center">
                      ...
                      <Box>
                        <Typography variant="h4" component="h1" sx={{ fontWeight: 700, mb: 1, lineHeight: 1.2, color: textPrimaryColor }}>Daily Time Record - Honorarium</Typography>
                        <Typography variant="body1" sx={{ opacity: 0.8, fontWeight: 400, color: textPrimaryColor }}>Filter your honorarium DTR records by date</Typography>
                      </Box>
                    </Box>
          ...
          <Box className="no-print" sx={{ display: 'flex', justifyContent: 'center', gap: 2, mt: 2, mb: 4 }}>
            <ProfessionalButton variant="contained" onClick={printPage} startIcon={<PrintIcon />}
              sx={{ backgroundColor: accentColor, color: textSecondaryColor, '&:hover':{ backgroundColor: accentDark }, py: 1.5, px: 4 }}>
              Print
            </ProfessionalButton>
            <ProfessionalButton variant="contained" onClick={downloadPDF} startIcon={<PictureAsPdfIcon />}
              sx={{ backgroundColor: accentColor, color: textSecondaryColor, '&:hover':{ backgroundColor: accentDark }, py: 1.5, px: 4 }}>
              Download PDF
            </ProfessionalButton>
          </Box>
