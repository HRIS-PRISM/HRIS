...
// This is the core fix for slowness: instead of firing 100+ HTTP requests at once,
// we run them in controlled batches of 8.
...
const ViewAttendanceRecord = () => {
  ...
  // ── fetchRecords — with debug logging to diagnose "no records in DTR" ──────
  const fetchRecords = useCallback(
    async (showLoading = true) => {
      ...
      try {
        ...
        // ── DEBUG: log to help diagnose why DailyTimeRecord shows "no records" ──
        // Remove these console.logs once the DTR field mapping is confirmed.
        if (recs.length > 0) {
          console.group('[ViewAttendanceRecord] fetchRecords debug');
          console.log('personID sent (type):', personID, typeof personID);
          console.log('startDate / endDate:', startDate, endDate);
          console.log('Total raw records returned:', recs.length);
          console.log('Sample record keys (first row):', Object.keys(recs[0]));
          console.log('Sample record values (first row):', recs[0]);
          // Key diagnostic: check what field names the backend actually returns.
          // DailyTimeRecord expects: r.timeIN, r.breaktimeIN, r.breaktimeOUT, r.timeOUT
          // If the backend returns Time1/Time2/Time3/Time4 instead, that is the bug.
          console.log(
            'Time fields in first record → Time1:',
            recs[0].Time1,
            '| Time2:',
            recs[0].Time2,
            '| Time3:',
          ...
          console.warn(
            '[ViewAttendanceRecord] fetchRecords: 0 records returned for personID:',
            personID,
          );
        }
        // ── END DEBUG ──
  ...
  // ── fetchAllUsersDTR — FIXED: concurrency-limited to 8 simultaneous requests ─
  // Previously this fired ALL requests at once (e.g. 200 parallel HTTP calls),
  // flooding the connection pool and making the UI hang.
  const fetchAllUsersDTR = useCallback(async () => {
    ...
    try {
      ...
      // Build task functions — each returns a user result object
      const tasks = users.map((user) => async () => {
        ...
        try {
          ...
          // Update progress bar incrementally as each request completes
          setProgressDone((p) => p + 1);
        }
      });

      // Run with max 8 concurrent requests — replaces the original Promise.all
      // which fired every request simultaneously regardless of count.
      const all = await runWithConcurrency(tasks, 8);
      ...
      showSnackbar(
        `Loaded ${all.length} employees (${withRecs} with records, ${totalRecs} total records auto-saved)`,
        'success',
      );

      if (totalRecs > 0) {
        setModalMessage(
          `Successfully auto-saved ${totalRecs} attendance records for ${withRecs} employees to the database. You can now view or print their DTR.`,
        );
  ...
  // Keep refs fresh for socket handler
  useEffect(() => {
    fetchRecordsRef.current = fetchRecords;
    fetchAllUsersDTRRef.current = fetchAllUsersDTR;
  });

  // ── Socket live-update handler ─────────────────────────────────────────────
  useEffect(() => {
  ...
  // Auto-fetch when dates change in single mode
  useEffect(() => {
    ...
    <Fade in timeout={400}>
      ...
      >
        ...
        >
          ...
          >
            <Typography
              sx={{ fontWeight: 800, fontSize: '1.1rem', color: T.text }}
            >
              Loading All Users
            </Typography>
            <CircularProgress
              size={72}
              thickness={4}
              sx={{ color: '#2e7d32', my: 0.5 }}
            />
            <Typography
              sx={{ fontWeight: 900, fontSize: '1.4rem', color: '#2e7d32' }}
            >
              {pct}%
            </Typography>
            <Box sx={{ width: '100%' }}>
            ...
            <Typography sx={{ fontSize: '0.72rem', color: T.faint }}>
              Fetching in batches of 8 — please wait…
            </Typography>
          </Box>
        </Dialog>
          ...
          <Box sx={{ px: 2.5, pt: 2, pb: 2.5 }}>
            ...
            <Box sx={{ display: 'flex', gap: 1.5, mb: 2.5, flexWrap: 'wrap' }}>
              {viewMode === 'single' && (
                <Box sx={{ flex: 1, minWidth: 160 }}>
                  ...
                  <EmployeeSearchField
                    value={personID}
                    onSelectEmployeeNumber={setPersonID}
                  />
                </Box>
              )}
              <Box sx={{ flex: 1, minWidth: 160 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 2.5 }}>
              {[
                ...
                {
                  label: 'Yesterday',
                  icon: <ArrowBackIos sx={{ fontSize: 11 }} />,
                  fn: () => {
        ...
        {(loading || loadingAllUsers) && (
          ...
          >
            ...
            <Typography sx={{ fontSize: '0.78rem', color: T.muted }}>
              {loadingAllUsers
                ? `Fetching all users (batch mode, 8 at a time)…`
              ...
              <Box sx={{ px: 2.5, pt: 2, pb: 2.5 }}>
                {allUsersDTR.length > 0 ? (
                  ...
                  <Box sx={{ py: 6, textAlign: 'center' }}>
                    ...
                    >
                      <People
                        sx={{ fontSize: 24, color: alpha(T.accent, 0.3) }}
                      />
                    </Box>
                    <Typography
                      sx={{
                      ...
                      records.map((record, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <Typography
                              sx={{ fontSize: '0.78rem', color: T.text }}
                            >
                  ...
                  {[
                    {
                      ...
                      label: 'Auto-saved records from biometric device',
                    },
                    {
                      icon: <Info sx={{ fontSize: 13, color: T.accent }} />,
                      label: 'Times displayed in 12-hour format',
                    },
                  ].map((item, i) => (
                    <Box
                      key={i}
