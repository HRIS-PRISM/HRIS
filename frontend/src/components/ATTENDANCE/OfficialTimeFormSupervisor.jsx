// ─────────────────────────────────────────────────────────────────────────────
// OfficialTimeFormSupervisor — Search-First Layout
// Layout: Left panel (search + control) | Right panel (schedule detail/create)
// Employee autocomplete mirrors DailyTimeRecordSupervisor.jsx pattern:
// department-scoped list loaded once via /api/supervisor-dtr/employees/me,
// filtered client-side — NOT a generic all-employee search.
// ─────────────────────────────────────────────────────────────────────────────
//
// [FIX] This revision restores four useMemo derivations (scheduleBlocks,
// scheduleBlockRows, activeBlockData, activeBlockRecords) that were
// accidentally dropped when the "Past Periods / changed employees" feature
// was merged in — the main Single Employee panel depends on them and the
// component would previously throw ReferenceError on render. Also removes
// a dangling `historyView` / `handleExitHistoryView` branch that was never
// backed by state, and widens the Single Employee grid to 3 columns so the
// new "Past Periods" side panel doesn't wrap onto a new row.
// ─────────────────────────────────────────────────────────────────────────────
...
// [CHANGE K] Department-scoped, mirrors DailyTimeRecordSupervisor.jsx: the
// full employee list for the supervisor's assigned department(s) is fetched
// ONCE by the parent (via /api/supervisor-dtr/employees/me) and passed in as
// `options`. This component no longer performs its own debounced/paged
// axios search against a generic, unscoped employee-search endpoint — it
// only filters the already department-scoped `options` list client-side by
...
const EmployeeSearchField = ({
  ...
  // [CHANGE K] Client-side filter over the department-scoped options list —
  // no network call per keystroke, since the list is already fully loaded
  // and already restricted to the supervisor's department(s).
  const results = useMemo(() => {
    ...
    return options.filter(
      (emp) =>
        ...
        String(emp.employeeNumber || "").toLowerCase().includes(q),
    );
  }, [options, query]);
        ...
        >
          {optionsLoading ? (
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 1, py: 2.5 }}>
              ...
              <Typography sx={{ fontSize: "0.8rem", color: T.muted }}>Loading your department's employees…</Typography>
            </Box>
          ) : results.length > 0 ? (
            ...
            <Box sx={{ py: 2.5, textAlign: "center" }}>
              <Typography sx={{ fontSize: "0.8rem", color: T.faint, fontStyle: "italic" }}>
                {query.length > 0
                  ? `No results for "${query}" in your department(s)`
                  : "No employees found in your assigned department(s)."}
              </Typography>
            </Box>
          )}
        </Paper>
...
// ─────────────────────────────────────────────────────────────────────────────
// DEPARTMENT EMPLOYEE CHECKLIST
// Replaces the autocomplete EmployeeSearchField for Step 1. Shows the full
// department-scoped employee list (already loaded via /api/supervisor-dtr/
// employees/me) as a checkable list. Since the right-hand panel can only
// display ONE employee's schedule at a time, checking a row behaves like a
// single-select — checking a new employee unchecks the previous one and
// immediately loads their schedule.
// ─────────────────────────────────────────────────────────────────────────────
const AVATAR_COLORS = ["#c0392b", "#d35400", "#8e44ad", "#2980b9", "#16a085", "#c2185b", "#6d4c41"];
...
const initialsFor = (name = "") => {
  // Expects "LASTNAME, Firstname" (matches fetchDeptEmployees' name format)
...
const DepartmentEmployeeChecklist = ({
  options = [],
  optionsLoading = false,
  selectedEmployee,
  onSelect,
  ...
  const allChecked = !!selectedEmployee;

  const handleSelectAllToggle = (e) => {
    if (e.target.checked) {
      if (options.length > 0) onSelect(options[0]);
    } else {
      onClear();
    }
  };
  ...
  return (
    <Box sx={{ border: `1px solid ${T.accentBorder}`, borderRadius: "10px", overflow: "hidden", bgcolor: "#fff" }}>
      <Box sx={{ px: 1.75, py: 1, borderBottom: `1px solid ${T.divider}`, bgcolor: T.accentFaint }}>
        <Typography sx={{ fontSize: "0.68rem", fontWeight: 800, color: T.accent, textTransform: "uppercase", letterSpacing: "0.06em" }}>
          Department Employees {options.length > 0 && `(${options.length})`}
        </Typography>
      </Box>
{/* 
      <Box sx={{ px: 1.75, py: 0.9, borderBottom: `1px solid ${T.divider}` }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1, cursor: disabled ? "default" : "pointer" }}>
          <Checkbox
            size="small"
            checked={allChecked}
          ...
          />
          <Typography sx={{ fontSize: "0.8rem", fontWeight: 600, color: T.text }}>Select all</Typography>
        </Box>
      </Box> */}

      <Box sx={{ maxHeight: 180, overflowY: "auto", "&::-webkit-scrollbar": { width: "5px" }, "&::-webkit-scrollbar-thumb": { background: "#d0b8b8", borderRadius: "4px" } }}>
        {optionsLoading ? (
          ...
          <Typography sx={{ py: 3, textAlign: "center", fontSize: "0.8rem", color: T.faint, fontStyle: "italic" }}>
            No employees found in your assigned department(s).
          </Typography>
        ) : (
          <List dense disablePadding>
            {options.map((emp) => {
              const isChecked = String(selectedEmployee?.employeeNumber) === String(emp.employeeNumber);
              return (
                <ListItem
                  key={emp.employeeNumber}
                  button
                  disabled={disabled}
                  onClick={() => (isChecked ? onClear() : onSelect(emp))}
                  sx={{
                    py: 0.9, px: 1.5,
                    borderBottom: `1px solid ${T.divider}`,
                    bgcolor: isChecked ? T.accentFaint : "transparent",
                    "&:hover": { bgcolor: T.accentFaint },
                    "&:last-child": { borderBottom: "none" },
                  }}
                >
                  <Checkbox
                    ...
                    onChange={() => (isChecked ? onClear() : onSelect(emp))}
                    sx={{ p: 0.4, mr: 1, color: T.accentBorder, "&.Mui-checked": { color: T.accent } }}
                  />
                  <Avatar sx={{ width: 30, height: 30, fontSize: "0.68rem", bgcolor: avatarColorFor(emp.name || String(emp.employeeNumber)), color: "#fff", fontWeight: 700, mr: 1.25 }}>
                    {initialsFor(emp.name)}
                  </Avatar>
                  <Box sx={{ minWidth: 0 }}>
                    <Typography sx={{ fontSize: "0.83rem", fontWeight: 700, color: T.text, lineHeight: 1.2 }} noWrap>
                      {emp.name}
                    </Typography>
...
// [CHANGE L] Mirrors DailyTimeRecordSupervisor.jsx's getUserRole — reads the
// role claim straight from the JWT so the tech-admin bypass in hasPermission
// below doesn't depend on any async page-access call.
const getUserRole = () => {
  try {
...
// ─────────────────────────────────────────────────────────────────────────────
const ScheduleTimeRows = ({ records, onChangeRecord, scheduleView, readOnly = false, changedCells = null }) => {
  ...
  {records.map((record, index) => {
    ...
    return (
      ...
      >
        ...
        {fields.map((f) => {
          const isChanged = changedCells?.has(`${record.day}|${f.key}`);
          return (
            <TableCell
              key={f.key}
              sx={{
                py: 0.6,
                minWidth: readOnly ? 130 : 170,
                verticalAlign: "middle",
                bgcolor: isChanged ? "rgba(237,108,2,0.10)" : undefined,
                borderLeft: isChanged ? "2px solid #ed6c02" : undefined,
              }}
            >
              {readOnly ? (
                <Typography
...
// MAIN COMPONENT
// ─────────────────────────────────────────────────────────────────────────────
const OfficialTimeFormSupervisor = ({
  embedded = false,
  initialContext = null,
  onClose,
} = {}) => {
  const { settings } = useSystemSettings();
  const { hasAccess, loading: accessLoading } = usePageAccess("official-time-supervisor");
  // [CHANGE L] Role read directly from the JWT, same as DailyTimeRecordSupervisor,
  ...
  const [showChangedEmployeesModal, setShowChangedEmployeesModal] = useState(false);
  const [selectedPastPeriod, setSelectedPastPeriod] = useState(null);
  const [changedEmployees, setChangedEmployees] = useState([]);
  const [changedEmployeesLoading, setChangedEmployeesLoading] = useState(false);

    // Merged dialog: left = changed-employee list, right = that employee's changed
  // schedule for the selected past period. Both panes live inside ONE dialog now.
  const [selectedChangedEmployee, setSelectedChangedEmployee] = useState(null);
  const [changedEmployeeDetail, setChangedEmployeeDetail] = useState(null); // { records, changedCells } — from the audit "changes" endpoint
  const [changedEmployeeDetailLoading, setChangedEmployeeDetailLoading] = useState(false);
  const [pastPeriodScheduleView, setPastPeriodScheduleView] = useState("workDays");
  // Full official-time record set for the selected changed employee, so the
  // right pane can show the SAME "Existing schedules" block-selector as the
  // main view — just read-only. changedCells only lights up when the block
  // being viewed is the one the audit endpoint actually flagged changes on.
  const [changedEmployeeAllRecords, setChangedEmployeeAllRecords] = useState([]);
  const [changedEmployeeActiveBlockKey, setChangedEmployeeActiveBlockKey] = useState(null);
  ...
  const [supervisorDeptCtx, setSupervisorDeptCtx] = useState(null);

  const fetchSupervisorDeptContext = useCallback(async () => {
    ...
    setSupervisorDeptCtx(data);
  }, []);

  useEffect(() => { fetchSupervisorDeptContext(); }, [fetchSupervisorDeptContext]);

  // ── Restored: active-employee schedule block derivations (used by the
  // main Single Employee panel — these were accidentally dropped when the
  // "Past Periods" changed-employee feature was merged in). ──
  ...
  // [CHANGE K] Department-scoped employee list for the Single Employee search
  // box — mirrors DailyTimeRecordSupervisor.jsx's loadEmployeesDtr employee
  // fetch: pulls from /api/supervisor-dtr/employees/me, which is already
  // restricted server-side to the logged-in supervisor's department(s) via
  // supervisor_assignment → department_assignment. Loaded once; the search
  // field filters this list client-side rather than hitting a generic,
  // unscoped employee-search endpoint per keystroke.
  const [deptEmployees, setDeptEmployees] = useState([]);
  const [deptEmployeesLoading, setDeptEmployeesLoading] = useState(false);

  const fetchDeptEmployees = useCallback(async () => {
    setDeptEmployeesLoading(true);
    try {
      const r = await axios.get(`${API_BASE_URL}/api/supervisor-dtr/employees/me`, getAuthHeaders());
      const list = (r.data || []).map((emp) => {
        const name =
          emp.firstName || emp.lastName
            ? `${emp.lastName || ""}${emp.lastName && emp.firstName ? ", " : ""}${emp.firstName || ""}${emp.middleName ? ` ${emp.middleName}` : ""}`.trim()
            : emp.devicePersonName || String(emp.personID);
        return {
          employeeNumber: emp.personID,
          name,
  ...
  useEffect(() => { fetchDeptEmployees(); }, [fetchDeptEmployees]);
  ...
  // [CHANGE L] Loading flag for the live supervisor_assignment-window check,
  ...
  try {
    const res = await axios.get(`${API_BASE_URL}/officialtime/past-periods`, getAuthHeaders());
    setPastPeriods(res.data || []);
  } catch (err) {
    console.error("Error fetching past periods:", err);
    setPastPeriods([]);
  } finally {
    setPastPeriodsLoading(false);
  }
}, []);

useEffect(() => {
  if (showSingleView) fetchPastPeriods();
}, [showSingleView, fetchPastPeriods]);

// Fetches one employee's changed records for a given past period and updates
// the RIGHT pane of the merged dialog — selecting an employee no longer
// closes the dialog or navigates away, it just swaps the right pane.
const fetchEmployeeChangeDetail = useCallback(async (period, emp) => {
  setSelectedChangedEmployee(emp);
  setChangedEmployeeDetailLoading(true);
  setChangedEmployeeDetail(null);
  ...
  try {
    ...
    const matchKey = changedRecords.length
      ? `${normalizeDateStr(changedRecords[0].startDate)}|${normalizeDateStr(changedRecords[0].endDate)}`
      : null;
    setChangedEmployeeActiveBlockKey(matchKey);
  } catch (err) {
    console.error("Error fetching employee change data:", err);
    showToast("Error loading this employee's changed records.");
    setChangedEmployeeDetail({ records: [], changedCells: new Set() });
  } finally {
    setChangedEmployeeDetailLoading(false);
  }
}, [showToast]);

const fetchChangedEmployeesForPeriod = useCallback(async (period) => {
  setChangedEmployeesLoading(true);
  setChangedEmployees([]);
  setSelectedChangedEmployee(null);
  setChangedEmployeeDetail(null);
  setChangedEmployeeAllRecords([]);
  setChangedEmployeeActiveBlockKey(null);
  setPastPeriodScheduleView("workDays");
  try {
    const res = await axios.get(
      `${API_BASE_URL}/officialtime/past-periods/${period.id}/changed-employees`,
      getAuthHeaders(),
    );
    const list = res.data || [];
    setChangedEmployees(list);
    // Auto-select the first employee so the right pane isn't empty on open.
    if (list.length > 0) fetchEmployeeChangeDetail(period, list[0]);
  } catch (err) {
    console.error("Error fetching changed employees:", err);
    setChangedEmployees([]);
  } finally {
    setChangedEmployeesLoading(false);
  }
}, [fetchEmployeeChangeDetail]);

const handleOpenPastPeriod = useCallback((period) => {
  setSelectedPastPeriod(period);
  setShowChangedEmployeesModal(true);
  fetchChangedEmployeesForPeriod(period);
}, [fetchChangedEmployeesForPeriod]);

const handleSelectChangedEmployee = useCallback((emp) => {
  if (!selectedPastPeriod) return;
  fetchEmployeeChangeDetail(selectedPastPeriod, emp);
}, [selectedPastPeriod, fetchEmployeeChangeDetail]);
  ...
  const fetchSupervisorStatus = useCallback(async () => {
    setSupervisorStatusLoading(true);
    try {
      ...
      setSupervisorStatusLoading(false);
    }
  }, []);
  ...
  const changedEmployeeScheduleBlocks = useMemo(() => {
    const byKey = new Map();
    for (const r of changedEmployeeAllRecords) {
      const key = `${normalizeDateStr(r.startDate)}|${normalizeDateStr(r.endDate)}`;
      if (!byKey.has(key)) byKey.set(key, { academicYear: r.academicYear, startDate: r.startDate, endDate: r.endDate, status: r.status, key });
    }
    return Array.from(byKey.values())
      .filter((v) => normalizeDateStr(v.startDate) || normalizeDateStr(v.endDate))
      .sort((a, b) => (String(b.status).toLowerCase() === "active" ? 1 : 0) - (String(a.status).toLowerCase() === "active" ? 1 : 0));
  }, [changedEmployeeAllRecords]);

  const changedEmployeeScheduleBlockRows = useMemo(() => {
    const rows = [];
    for (let i = 0; i < changedEmployeeScheduleBlocks.length; i += 2) rows.push(changedEmployeeScheduleBlocks.slice(i, i + 2));
    return rows;
  }, [changedEmployeeScheduleBlocks]);

  const changedEmployeeActiveBlockData = useMemo(() => {
    if (!changedEmployeeActiveBlockKey) return changedEmployeeScheduleBlocks[0] || null;
    return changedEmployeeScheduleBlocks.find((b) => b.key === changedEmployeeActiveBlockKey) || changedEmployeeScheduleBlocks[0] || null;
  }, [changedEmployeeActiveBlockKey, changedEmployeeScheduleBlocks]);

  const changedEmployeeActiveBlockRecords = useMemo(() => {
    if (!changedEmployeeActiveBlockData) return [];
    const normStart = normalizeDateStr(changedEmployeeActiveBlockData.startDate);
    const normEnd = normalizeDateStr(changedEmployeeActiveBlockData.endDate);
    return [...changedEmployeeAllRecords.filter((r) => normalizeDateStr(r.startDate) === normStart && normalizeDateStr(r.endDate) === normEnd)]
      .sort((a, b) => DAYS_ORDER.indexOf(a.day) - DAYS_ORDER.indexOf(b.day));
  }, [changedEmployeeActiveBlockData, changedEmployeeAllRecords]);

  // The orange "changed" highlight only makes sense on the block the audit
  // endpoint actually reported on — if the user picks a different past
  // block from the selector, there's no change data for it, so no highlight.
  const changedEmployeeActiveChangedCells = useMemo(() => {
    if (!changedEmployeeDetail || !changedEmployeeActiveBlockData) return new Set();
    const detailKey = changedEmployeeDetail.records.length
      ? `${normalizeDateStr(changedEmployeeDetail.records[0].startDate)}|${normalizeDateStr(changedEmployeeDetail.records[0].endDate)}`
      : null;
    return detailKey && detailKey === changedEmployeeActiveBlockData.key
      ? changedEmployeeDetail.changedCells
      : new Set();
  }, [changedEmployeeDetail, changedEmployeeActiveBlockData]);
  ...
  // [CHANGE L] Consistent access gating — mirrors DailyTimeRecordSupervisor /
  // SupervisorLeaveRequest: the live supervisor_assignment window is
  // authoritative, page_access is a secondary/cached grant, and technical
  // admins always get through. Both async checks (page access + supervisor
  // status) must resolve before deciding, so a supervisor whose assignment
  // just started (but whose page_access grant hasn't synced yet) isn't
  ...
  if (!hasPermission)
    return (
      <AccessDenied
        title="Access Denied"
        message="You do not have permission to access Official Time Form. You must be assigned as a supervisor in Supervisor Assignment."
        returnPath="/home"
  ...
  return (
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: embedded ? 1.5 : 2.5, position: "relative", zIndex: 1, minWidth: 0 }}>
                ...
                <Box sx={{ minWidth: 0 }}>
                  ...
                  <Typography sx={{ fontSize: embedded ? "0.72rem" : "0.78rem", color: T.accentMid, fontWeight: 600 }}>
                    {!embedded && supervisorDeptCtx?.departments?.length > 0 && (
                      <>{supervisorDeptCtx.departments.map((d) => d.description || d.code).join(" · ")} · </>
                    )}
                    {embedded
                      ? "Manage schedules without leaving Daily Time Record"
                      : "Search an employee in your department to view and manage their official time schedules"}
                  </Typography>
                </Box>
              </Box>
            ...
            <Fade in timeout={embedded ? 0 : 400} key="single-view">
              ...
              >
                ...
                <SectionCard sx={{ position: embedded ? "relative" : { lg: "sticky" }, top: embedded ? undefined : { lg: 16 }, minHeight: embedded ? 0 : { lg: "calc(100vh - 295px)" }, height: "100%", display: "flex", flexDirection: "column", overflow: embedded ? "auto" : undefined }}>
                  <PanelHeader icon={Person} title="Step 1 — Search employee (your department)" />
                  <Box sx={{ p: 2.5 }}>
                    <DepartmentEmployeeChecklist
                      onSelect={handleEmployeeSelect}
                      selectedEmployee={selectedEmployee}
                      onClear={handleEmployeeClear}
                      options={deptEmployees}
                      optionsLoading={deptEmployeesLoading}
                    />
                    {selectedEmployee ? (
                  ...
                  {!selectedEmployee ? (
                    <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: { xs: 400, lg: "calc(100vh - 405px)" }, flex: 1, gap: 2, p: 4, textAlign: "center" }}>
                      ...
                      <Box>
                        ...
                        <Typography sx={{ fontSize: "0.82rem", color: T.faint }}>Search for an employee in your department on the left to view their official time schedule.</Typography>
                      </Box>
                    </Box>
                  ) : loading ? (
    ...
    <Box sx={{ flex: 1, minHeight: 0, overflowY: "auto", overscrollBehaviorY: "contain" }}>
      {pastPeriodsLoading ? (
        ...
        <Typography sx={{ px: 2.5, py: 3, fontSize: "0.8rem", color: T.faint, fontStyle: "italic" }}>No past periods found.</Typography>
      ) : (
        <List dense disablePadding>
          {pastPeriods.map((p) => (
            <ListItem key={p.id} button onClick={() => handleOpenPastPeriod(p)}
              sx={{ py: 1.25, px: 2, borderBottom: `1px solid ${T.divider}`, "&:hover": { bgcolor: T.accentFaint } }}>
              <Box sx={{ width: "100%" }}>
                <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 1 }}>
                  <Typography sx={{ fontSize: "0.85rem", fontWeight: 700, color: T.text }}>{p.department}</Typography>
                  <Box
                    component="span"
                    sx={{
                      flexShrink: 0,
                      fontSize: "0.65rem", fontWeight: 700,
                      px: 0.9, py: 0.2, borderRadius: "9px",
                      border: "0.5px solid",
                      ...(p.active
                        ? { bgcolor: "rgba(46,125,50,0.08)", color: "#2e7d32", borderColor: "rgba(46,125,50,0.3)" }
                        : { bgcolor: "rgba(237,108,2,0.08)", color: "#b45309", borderColor: "rgba(237,108,2,0.3)" }),
                  ...
                  >
                    {p.active ? "Currently Active" : "Expired"}
                  </Box>
                </Box>
                <Typography sx={{ fontSize: "0.75rem", color: T.muted, mt: 0.25 }}>
                  {formatDateTimeLong(p.startDate)} – {formatDateTimeLong(p.endDate)}
                </Typography>
              </Box>
            </ListItem>
          ))}
                ...
                <>
                  <Box sx={{ display: "flex", flexWrap: "wrap", gap: 3, mb: 2.5, p: 2, bgcolor: T.accentFaint, border: `1px solid ${T.accentBorder}`, borderRadius: "12px", alignItems: "flex-end" }}>
                    ...
                    <Box>
                      ...
                      <Box sx={{ mt: 0.5 }}><StatusBadge active={String(viewScheduleInfo.status).toLowerCase() === "active"} /></Box>
                    </Box>
                  </Box>
                  <Box sx={{ mb: 2 }}>
                    <ScheduleTabBar
            ...
            <Box sx={{ borderTop: `1px solid ${T.divider}`, bgcolor: "#fafafa", px: 3, py: 2, display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 1.5 }}>
              {isEditingViewSchedule ? (
                ...
                <>
                  ...
                  {viewScheduleInfo && String(viewScheduleInfo.status).toLowerCase() === "active" && (
          ...
          <Dialog open={showConflictModal} onClose={() => setShowConflictModal(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: "16px", overflow: "hidden" } }}>
            ...
            <Box sx={{ borderTop: `1px solid ${T.divider}`, bgcolor: "#fafafa", px: 3, py: 2, display: "flex", justifyContent: "flex-end" }}>
              <Button variant="contained" disableElevation onClick={() => setShowConflictModal(false)} sx={{ bgcolor: T.accent, color: "#fff", fontWeight: 700, textTransform: "none", "&:hover": { bgcolor: T.accentDark } }}>Got It</Button>
                    ...
                    {/* ── Past Period Changes — MERGED: left = changed-employee list,
               right = that employee's changed schedule, in one dialog.
               Selecting an employee swaps the right pane only; the dialog
               stays open and the main page is untouched. ── */}
          <Dialog
            open={showChangedEmployeesModal}
            onClose={() => {
              setShowChangedEmployeesModal(false);
            ...
            maxWidth="lg"
            fullWidth
            PaperProps={{ sx: { borderRadius: "16px", overflow: "hidden", height: "78vh", maxHeight: 670, display: "flex", flexDirection: "column" } }}
          >
            <Box sx={dialogHeaderSx}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, minWidth: 0 }}>
                <Schedule sx={{ color: "#fff", fontSize: 20, flexShrink: 0 }} />
                <Box sx={{ minWidth: 0 }}>
                  <Typography sx={{ fontWeight: 700, color: "#fff", fontSize: "0.975rem" }}>
                    Employees Changed in This Period
                  </Typography>
                  {selectedPastPeriod && (
            ...
            <Box sx={{ flex: 1, minHeight: 0, display: "grid", gridTemplateColumns: { xs: "1fr", sm: "260px 1fr" }, bgcolor: "#fff" }}>
              {/* LEFT — changed-employee list */}
              <Box sx={{ borderRight: { sm: `1px solid ${T.divider}` }, borderBottom: { xs: `1px solid ${T.divider}`, sm: "none" }, display: "flex", flexDirection: "column", minHeight: 0, maxHeight: { xs: 220, sm: "none" } }}>
                <Box sx={{ px: 2, py: 1.25, borderBottom: `1px solid ${T.divider}`, bgcolor: T.accentFaint, flexShrink: 0 }}>
                  <Typography sx={{ fontSize: "0.7rem", fontWeight: 700, color: T.accent, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                    Employees {changedEmployees.length > 0 && `(${changedEmployees.length})`}
                  </Typography>
                </Box>
                <Box sx={{ flex: 1, overflowY: "auto" }}>
                  {changedEmployeesLoading ? (
                    <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", py: 4 }}>
                      <CircularProgress size={16} sx={{ color: T.accent }} />
                    </Box>
                  ) : changedEmployees.length === 0 ? (
                    <Typography sx={{ py: 3, px: 2, textAlign: "center", fontSize: "0.8rem", color: T.faint, fontStyle: "italic" }}>
                      No employee record changes found for this period.
                    </Typography>
                  ) : (
                    <List dense disablePadding>
                      {changedEmployees.map((emp) => {
                        const isSelected = String(selectedChangedEmployee?.employeeNumber) === String(emp.employeeNumber);
                        return (
                          <ListItem
                            key={emp.employeeNumber}
                            button
                            selected={isSelected}
                            disabled={changedEmployeeDetailLoading}
                            onClick={() => handleSelectChangedEmployee(emp)}
                            sx={{
                              py: 1, px: 1.5,
                              borderBottom: `1px solid ${T.divider}`,
                              bgcolor: isSelected ? alpha(T.accent, 0.08) : "transparent",
                              "&:hover": { bgcolor: isSelected ? alpha(T.accent, 0.1) : T.accentFaint },
                          ...
                          >
                            <Box sx={{ display: "flex", alignItems: "center", gap: 1.25, width: "100%" }}>
                              ...
                              <Box sx={{ flex: 1, minWidth: 0 }}>
                                <Typography sx={{ fontSize: "0.82rem", fontWeight: isSelected ? 700 : 600, color: T.text }} noWrap>{emp.name}</Typography>
                                <Typography sx={{ fontSize: "0.68rem", color: T.muted }}>
                                  #{emp.employeeNumber}{emp.changeCount ? ` · ${emp.changeCount} change${emp.changeCount === 1 ? "" : "s"}` : ""}
                                </Typography>
                              </Box>
                            </Box>
                          </ListItem>
                            ...
                            {/* RIGHT — selected employee's schedules for this past period.
                   Shows the SAME "Existing schedules" block-selector as the
                   main view, entirely read-only (no Edit affordances). The
                   orange changed-cell highlight only appears on the block
                   the audit log actually flagged. */}
              <Box sx={{ display: "flex", flexDirection: "column", minHeight: 0 }}>
                {changedEmployeeDetailLoading ? (
                  <Box sx={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 1.5 }}>
                    <CircularProgress size={20} sx={{ color: T.accent }} />
                    <Typography sx={{ fontSize: "0.85rem", color: T.muted }}>Loading changes…</Typography>
                  </Box>
                ) : !selectedChangedEmployee || !changedEmployeeDetail ? (
                  <Box sx={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", p: 3, textAlign: "center" }}>
                    <Typography sx={{ fontSize: "0.82rem", color: T.faint, fontStyle: "italic" }}>
                      Select an employee on the left to view their changed schedule.
                    </Typography>
                  </Box>
                ) : (
                  <>
                    <Box sx={{ px: 2.5, py: 1.5, borderBottom: `1px solid ${T.divider}`, bgcolor: T.accentFaint, flexShrink: 0 }}>
                      <Typography sx={{ fontSize: "0.85rem", fontWeight: 700, color: T.text }}>
                        {selectedChangedEmployee.name}
                      </Typography>
                      <Typography sx={{ fontSize: "0.72rem", color: T.muted }}>
                        #{selectedChangedEmployee.employeeNumber}{selectedPastPeriod ? ` · ${selectedPastPeriod.department}` : ""}
                      </Typography>
                    </Box>

                    <Box sx={{ flex: 1, overflow: "hidden", px: 2.5, py: 1.75 }}>
                      {changedEmployeeScheduleBlocks.length === 0 ? (
                        <Typography sx={{ fontSize: "0.8rem", color: T.faint, fontStyle: "italic" }}>
                          No official time schedules found for this employee.
                        </Typography>
                      ) : (
                        <>
                          {/* Existing schedules — read-only block selector */}
                          <Box sx={{ mb: 1.75, border: `1px solid ${T.accentBorder}`, borderRadius: 1.5, overflow: "hidden", bgcolor: alpha(T.accent, 0.02) }}>
                            <Box sx={{ px: 1.5, py: 0.75, display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${T.accentBorder}`, bgcolor: "#fff" }}>
                              <Typography sx={{ fontSize: "0.72rem", color: T.accent, fontWeight: 700, letterSpacing: "0.03em", textTransform: "uppercase" }}>Existing schedules</Typography>
                              <Box component="span" sx={{ fontSize: "0.65rem", fontWeight: 700, bgcolor: alpha("#2e7d32", 0.1), color: "#2e7d32", border: "0.5px solid rgba(46,125,50,0.3)", borderRadius: "9px", px: 1, py: 0.2 }}>
                                {changedEmployeeScheduleBlocks.length}
                              </Box>
                            </Box>
                            <Box sx={{ maxHeight: 140, overflowY: "auto", "&::-webkit-scrollbar": { width: "6px" }, "&::-webkit-scrollbar-thumb": { background: "#d0b8b8", borderRadius: "4px" } }}>
                              {changedEmployeeScheduleBlockRows.map((row, rowIndex) => (
                                <Box key={`ce-schedule-row-${rowIndex}`} sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, borderBottom: rowIndex < changedEmployeeScheduleBlockRows.length - 1 ? `0.5px solid ${T.accentBorder}` : "none" }}>
                                  {row.map((v, colIndex) => {
                                    const isActive = String(v.status || "active").toLowerCase() === "active";
                                    const isSelected = changedEmployeeActiveBlockData?.key === v.key;
                                    return (
                                      <Box key={v.key} onClick={() => setChangedEmployeeActiveBlockKey(v.key)}
                                        sx={{ display: "flex", alignItems: "center", gap: 1, px: 1.25, py: 0.6, borderLeft: isActive ? "3px solid #2e7d32" : "3px solid transparent", borderRight: { md: colIndex === 0 ? `0.5px solid ${T.accentBorder}` : "none" }, cursor: "pointer", bgcolor: isSelected ? alpha(T.accent, 0.06) : "transparent", transition: "all 0.15s", "&:hover": { bgcolor: T.accentFaint } }}
                                      >
                                        <Box sx={{ width: 7, height: 7, borderRadius: "50%", bgcolor: isActive ? "#2e7d32" : "#b7b7b7", flexShrink: 0 }} />
                                        <Box sx={{ flex: 1, minWidth: 0 }}>
                          ...
                          <Box sx={{ mb: 2 }}>
                            <ScheduleTabBar activeTab={pastPeriodScheduleView} setTab={setPastPeriodScheduleView} />
                          </Box>

                          <PremiumTableContainer sx={{ maxHeight: "none" }}>
                            <Table size="small" stickyHeader>
                              <ScheduleTimeRows
                                records={changedEmployeeActiveBlockRecords}
                                onChangeRecord={() => {}}
                                scheduleView={pastPeriodScheduleView}
                                readOnly
                                changedCells={changedEmployeeActiveChangedCells}
                              />
                            </Table>
                          </PremiumTableContainer>
                        </>
            ...
            <Box sx={{ borderTop: `1px solid ${T.divider}`, bgcolor: "#fafafa", px: 3, py: 2, display: "flex", justifyContent: "flex-end", flexShrink: 0 }}>
              <Button
                variant="outlined"
                onClick={() => {
                  setShowChangedEmployeesModal(false);
...
export default OfficialTimeFormSupervisor;
