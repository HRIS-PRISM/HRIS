...
// ─── Theme tokens (unified with PersonTable) ──────────────────
const T = {
  accent: '#6d2323',
  accentDark: '#5a1d1d',
  accentMid: '#8B4545',
...
const DailyTimeRecordFaculty = () => {
  ...
  const fetchAllUsersDTR = async () => {
    ...
    try {
      ...
      allRecords.forEach((record) => {
        ...
        userRecordsMap.get(empNum).records.push(record);
      });
        ...
        if (Array.isArray(deptAssignRes.data)) {
          ...
          allUserData.forEach((user) => {
            ...
            if (code) {
              ...
              user.rawUser.departmentCode = code;
            }
          });
        }
      ...
      const allUserDataFiltered = allUserData.map((user) => {
        const filtered = filterByDtrType(user.records, dtrType);
        return { ...user, records: filtered, hasRecords: filtered.length > 0 };
      });
      ...
      allUserDataFiltered.sort((a, b) => {
        ...
        if (!la && !lb) return a.fullName.localeCompare(b.fullName);
  ...
  // ─── FIXED: Filter with proper search ────────────────────
  const getFilteredUsers = () => {
    ...
    if (departmentFilter) {
      filtered = filtered.filter((u) => {
        const code = u.departmentCode || u.rawUser?.departmentCode || '';
    ...
    <div style={{ display: 'flex', gap: '2%', width: '8.7in', minWidth: '8.5in', margin: '0 auto', backgroundColor: 'white', position: 'relative', zIndex: 1 }} className="table-side-by-side">
      {[0, 1].map((tIdx) => (
        <table key={tIdx} style={{ position: 'relative', border: '1px solid black', borderCollapse: 'collapse', width: '49%', tableLayout: 'fixed' }} className="print-visble">
          ...
          <tbody>
            {renderDTRRows(sourceRecords, dtrType)}
            {renderDTRFooter()}
          </tbody>
        </table>
      ))}
            ...
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              {[
                ...
                <AccentButton
                  ...
                  variant={dtrType === val ? 'contained' : 'outlined'}
                  size="small"
                  sx={{
