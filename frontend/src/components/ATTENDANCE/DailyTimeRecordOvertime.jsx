...
const DailyTimeRecordOvertime = () => {
  ...
  const loadingOverlayMessage = (() => {
    ...
    if (pageLoading) return 'Loading Overtime DTR…';
    if (singlePrintLoading) return singlePrintStatus || 'Preparing DTR…';
    if (monthLoading)
      return selectedMonth !== null
        ? `Overtime DTR — ${monthsShort[selectedMonth]}…`
        : 'Overtime DTR — Fetching records…';
    return 'Processing…';
  })();
    ...
    Array.from({ length: daysInSelectedMonth }, (_, i) => {
      ...
      return (
        <tr key={i}>
          ...
          {renderDtrAmPmWatermarkCell(record?.specialTimeIN, formatTime(record?.specialTimeIN || ''), rowTint, indicator, `o-${i}-0`)}
          {renderDtrAmPmWatermarkCell(null, '', rowTint, indicator, `o-${i}-1`)}
          {renderDtrAmPmWatermarkCell(null, '', rowTint, indicator, `o-${i}-2`)}
          {renderDtrAmPmWatermarkCell(record?.specialTimeOUT, formatTime(record?.specialTimeOUT || ''), rowTint, indicator, `o-${i}-3`)}
                ...
                >
                  ...
                  >
                    ...
                    <Box>
                      ...
                      >
                        Employee Portal — view and download your overtime DTR records
                      </Typography>
                    </Box>
                  </Box>
                  <Box
                      ...
                      >
                        ...
                        >
                          {selectedMonth !== null
                            ...
                            : 'Select a month to load your overtime DTR.'}
                        </Typography>
                      </Box>
                    </Box>
                  </SectionCard>
                    ...
                    {selectedMonth !== null && (
                      ...
                      >
                        ...
                        <Typography sx={{ fontSize: '0.7rem', color: T.faint }}>
                          Download generates a verified PDF of your Overtime DTR for{' '}
