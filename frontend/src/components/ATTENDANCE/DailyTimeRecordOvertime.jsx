  ...
  const restoreDOMFromOriginal = useCallback(() => {
    ...
    tbodies.forEach((tbody) => {
      ...
      rows.forEach((row) => {
        ...
        // Overtime uses specialTimeIN (cell 1) and specialTimeOUT (cell 4)
        const timeValues = [
  ...
  const fetchRecords = async () => {
    try {
      ...
      const overtimeRecords = data.filter(
        (record) => record.specialType === 'OVERTIME' && (record.specialTimeIN || record.specialTimeOUT),
      );
    ...
    Array.from({ length: 31 }, (_, i) => {
      ...
      return (
        <tr key={i}>
          ...
          {/* AM Departure — empty for overtime */}
          <td style={tdRel}>
            {cellOverlay('')}
            <span style={{ position: 'relative', zIndex: 1 }}></span>
          </td>

          {/* PM Arrival — empty for overtime */}
          <td style={tdRel}>
            {cellOverlay('')}
            <span style={{ position: 'relative', zIndex: 1 }}></span>
          </td>
                  ...
                  <Box display="flex" alignItems="center">
                    ...
                    <Box>
                      ...
                      <Typography variant="body1" sx={{ opacity: 0.8, fontWeight: 400, color: textPrimaryColor }}>
                        Filter your overtime DTR records by date
                      </Typography>
                    </Box>
                  </Box>
