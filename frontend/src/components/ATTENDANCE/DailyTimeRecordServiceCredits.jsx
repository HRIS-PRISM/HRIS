  ...
  const restoreDOMFromOriginal = useCallback(() => {
    ...
    tbodies.forEach((tbody) => {
      ...
      rows.forEach((row) => {
        ...
        // Service credits uses specialTimeIN (cell 1) and specialTimeOUT (cell 4)
        const timeValues = [
  ...
  const fetchRecords = async () => {
    try {
      ...
      const serviceRecords = data.filter(
        (record) => record.specialType === 'SERVICE' && (record.specialTimeIN || record.specialTimeOUT),
      );
    ...
    Array.from({ length: 31 }, (_, i) => {
      ...
      return (
        <tr key={i}>
          ...
          {/* AM Departure — empty for service credits */}
          <td style={tdRel}>
            {cellOverlay('')}
            <span style={{ position: 'relative', zIndex: 1 }}></span>
          </td>

          {/* PM Arrival — empty for service credits */}
          <td style={tdRel}>
            {cellOverlay('')}
            <span style={{ position: 'relative', zIndex: 1 }}></span>
          </td>
