...
/**
 * Lightweight djb2-style hash — converts any serialisable value into a short
 * hex string.  Used to fingerprint the API response so we can detect mutation.
 */
...
const DailyTimeRecord = () => {
  ...
  const [integrityStatus, setIntegrityStatus] = useState('none'); // 'none'|'ok'|'warn'
  const [snackbar, setSnackbar] = useState({ open: false, message: '', severity: 'info' });

  // ── Anti-tamper refs ───────────────────────────────────────────────────────
  // MutationObserver instance
  const observerRef         = useRef(null);
  // Debounce timer for the restore action
  const restoreTimerRef     = useRef(null);
  // Mirror of originalRecords visible inside observer callback (no stale closure)
  const originalRecordsRef  = useRef([]);
  // Mute flag: skip observer reactions while WE are writing to the DOM
  const isRestoringRef      = useRef(false);
  // Mirror of formatTime that the observer can call without a stale closure
  ...
  // Keep formatTimeRef in sync so the MutationObserver can always call the
  // latest version without a stale closure
  useEffect(() => {
  ...
  /**
   * Walks every tbody row in the DTR table(s) inside dtrRef and rewrites the
   * four time <span> elements to match originalRecordsRef.
  ...
  const restoreDOMFromOriginal = useCallback(() => {
    ...
    // Mute the observer so our own DOM writes don't trigger another restore loop
    isRestoringRef.current = true;

    const fmt = formatTimeRef.current || ((s) => s || '');

    // Each table has its own tbody — both show the same data so we restore both
    ...
    tbodies.forEach((tbody) => {
      ...
      rows.forEach((row) => {
        ...
        const dayText = dayCell.textContent.trim(); // "01" … "31"
        if (!/^\d{2}$/.test(dayText)) return;       // skip footer rows

        // Find the matching original record for this day number
        const record = original.find((r) => {
          const d = (r.date || '').split('T')[0].split('-')[2];
          return d === dayText;
        });

        // The four time spans are the first <span> inside cells 2-5
        const cells = row.querySelectorAll('td');
        if (cells.length < 5) return;

        const timeValues = [
          fmt(record?.timeIN     || ''),  // AM Arrival   — cell index 1
          fmt(record?.breaktimeIN  || ''),  // AM Departure — cell index 2
          fmt(record?.breaktimeOUT || ''),  // PM Arrival   — cell index 3
          fmt(record?.timeOUT    || ''),  // PM Departure — cell index 4
        ];
    ...
    // Re-enable observer after a short tick so React's own renders don't
    // immediately re-trigger the tamper logic
    setTimeout(() => {
  ...
  /**
   * Starts watching the DTR DOM for any external mutations (DevTools edits).
   * On mutation → debounce 300 ms → restore → show snackbar warning.
   */
  const startObserver = useCallback(() => {
    ...
    observerRef.current = new MutationObserver((mutations) => {
      // Ignore mutations caused by our own restore writes
      if (isRestoringRef.current) return;

      // Check if any of the mutations touch a time <span> value
      const isTimeTamper = mutations.some((m) => {
        if (m.type === 'characterData') {
          // Walk up to see if the changed node lives inside a time span
          const span = m.target.parentElement;
          return span && span.tagName === 'SPAN';
        }
        if (m.type === 'childList') {
          // Added / removed nodes inside a td
      ...
      // Debounce: clear any pending restore and schedule a fresh one
...
restoreTimerRef.current = setTimeout(() => {
  ...
  /** Stop the observer (called on cleanup / when records are cleared) */
  ...
  // ── Start / stop observer whenever records change ─────────────────────────
  useEffect(() => {
    if (originalRecords.length > 0 && dtrRef.current) {
      // Give React a tick to finish rendering the new rows before we attach
  ...
  const fetchRecords = async () => {
    try {
      ...
      if (data.length > 0) {
        // Stop old observer before state update
        stopObserver();
        ...
        originalRecordsRef.current = immutable; // keep ref in sync immediately
  ...
  // ── Stale-data + hash guard (used by print/download) ──────────────────────
  const verifyIntegrity = () => {
    ...
    if (ageMs > 30 * 60 * 1000) {
      ...
      setSnackbar({ open: true, message: '⚠️ DTR data is older than 30 minutes. Please search again to get fresh data.', severity: 'warning' });
      return false;
    }
    const currentHash = generateHash(records);
    if (currentHash !== recordsHash) {
      setIntegrityStatus('warn');
      setSnackbar({ open: true, message: '⚠️ Data integrity check failed. The records may have been modified. Please reload.', severity: 'error' });
      return false;
    }
    setIntegrityStatus('ok');
    return true;
  ...
  const printPage = async () => {
    ...
    // Restore DOM to original before capturing, just in case
    restoreDOMFromOriginal();
    ...
    return (
      <thead style={{ textAlign: 'center' }}>
        ...
        <tr><td colSpan="7" style={{ textAlign: 'center', padding: '2px 5px', lineHeight: '1.2' }}>
          <h4 style={{ fontFamily: 'Times New Roman,serif', textAlign: 'center', margin: '2px 0', fontWeight: 'bold', fontSize: '16px' }}>DAILY TIME RECORD</h4>
        </td></tr>
        <tr>
