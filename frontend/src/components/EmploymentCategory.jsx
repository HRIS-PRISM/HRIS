...
const ManageTypesTab = ({ typeConfigs, onRefresh, showSnackbar }) => {
  ...
  // Group unique parent groups from existing configs
...
const EmploymentCategoryManagement = () => {
  ...
  const [activeTab, setActiveTab] = useState(0); // 0=Assign, 1=Manage Types
  ...
  const filteredData = useMemo(() => {
    ...
    return employmentCategories.filter(r =>
                    ...
                    {pagedData.length === 0 ? (
                      <Box sx={{ py: 10, textAlign: "center" }}>
                        ...
                        <Typography sx={{ fontSize: "0.78rem", color: T.faint }}>
                          {employmentCategories.length === 0 ? "Use the form on the left to add a category." : "Try a different search term."}
                        </Typography>
                      </Box>
                    ) : viewMode === "grid" ? (
