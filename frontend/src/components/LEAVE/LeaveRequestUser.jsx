...
import {
  Box, Typography, CircularProgress, Alert, Button, Avatar, Chip, Paper,
  Fade, Grid, FormControl, Tooltip, IconButton, Backdrop, Snackbar, styled, alpha,
} from "@mui/material";
import {
  EventNote, FilterList as FilterIcon, EventAvailable as ReorderIcon,
  Person as PersonIcon, CalendarMonth as CalendarIcon, Refresh, AccessTime,
  CheckCircle, Block, Cancel as CancelIcon, Schedule as ScheduleIcon,
  Add as AddIcon, WarningAmber as WarningIcon,
...
const ProfessionalButton = styled(Button)(() => ({
  ...
  transition: 'all 0.3s ease', textTransform: 'none', fontSize: '0.95rem',
}));
const ModernTextField = styled(TextField)(() => ({
  '& .MuiOutlinedInput-root': {
    borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.9)', fontSize: '0.95rem',
...
const ModernSelect = styled(Select)(() => ({
  borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.9)', fontSize: '0.9rem',
}));
const RecordCard = styled(Card)(() => ({
  ...
  '&:hover': { boxShadow: '0 8px 24px rgba(109,35,35,0.12)', borderColor: 'rgba(109,35,35,0.15)' },
}));
...
const LeaveRequestUser = () => {
  ...
  const isSickLeave = () => {
    ...
    if (!t) return false;
    return (t.leave_code || '').toLowerCase().includes('sl') ||
  ...
  const getAllocatedRemainingDays = () => {
    ...
    const allocated = assignments
      .filter(a =>
        ...
        (a.carried_forward_hours === null || Number(a.carried_forward_hours) === 0)
      )
      .sort((a, b) => {
  ...
  useEffect(() => {
    if (personID) { fetchLeaveRequests(); fetchLeaveTypes(); fetchAssignments(); }
  }, [personID]);
  ...
  const getLeaveTypeInfo = (leaveCode) =>
    leaveTypes.find(t => t.leave_code === leaveCode) || { leave_description: leaveCode };
  ...
  const afterSubmit     = remainingDays !== null ? remainingDays - selectedDates.length : null;
    ...
    <Box sx={{ py: 4, mt: -5, maxWidth: '1800px', mx: 'auto', overflow: 'hidden', px: { xs: 2, sm: 3, md: 6 } }}>
      ...
      >
        <Alert
          ...
          sx={{
            width: '100%', maxWidth: 520, borderRadius: 3,
            ...
            <Box sx={{ p: 5, background: `linear-gradient(135deg, ${primaryColor} 0%, ${secondaryColor} 100%)`, position: 'relative', overflow: 'hidden' }}>
              ...
              <Box display="flex" alignItems="center" justifyContent="space-between" position="relative" zIndex={1}>
                <Box display="flex" alignItems="center">
                  ...
                  <Box>
                    <Typography variant="h4" sx={{ fontWeight: 700, color: textPrimaryColor }}>Employee Leave Request</Typography>
                    <Typography variant="body1" sx={{ opacity: 0.8, color: accentDark }}>Submit and view your leave requests</Typography>
                  </Box>
                </Box>
        ...
        <Grid item xs={12} lg={8}>
              ...
              <Box sx={{ p: 4 }}>
                <Grid container spacing={3}>
                  ...
                  <Grid item xs={12} sm={6}>
                    ...
                    <ModernTextField value={personID} disabled fullWidth size="small"
                  ...
                  {newLeaveRequest.leave_code && remainingDays !== null && (
                    <Grid item xs={12}>
                      <Box sx={{
                        ...
                        bgcolor: isOverBalance ? 'rgba(198,40,40,0.06)' : 'rgba(46,125,50,0.05)',
                        ...
                        <Typography variant="body2" sx={{ fontWeight: 600, color: isOverBalance ? '#C62828' : '#2E7D32', flex: 1 }}>
                          {isOverBalance
                        ...
                        {selectedDates.length > 0 && !isOverBalance && afterSubmit !== null && (
                  ...
                  <Grid item xs={12} sm={6}>
                    ...
                    <LeaveDatePickerModal
                      open={dateModalOpen}
                      onClose={() => {
                        setNewLeaveRequest(prev => ({ ...prev, leave_date: selectedDates.join(',') }));
                  ...
                  <Grid item xs={12} sm={6}>
                    ...
                    <Box sx={{
                      minHeight: 44, display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 0.5,
                      p: 1.5, border: `1px solid ${alpha(isOverBalance ? '#C62828' : accentColor, 0.2)}`,
                      borderRadius: 2, backgroundColor: 'rgba(255,255,255,0.5)',
                    }}>
                      {selectedDates.length > 0 ? (
                        selectedDates.map(date => (
                          <Chip key={date} label={date} size="small"
                            onDelete={() => setSelectedDates(prev => prev.filter(d => d !== date))}
                            sx={{ bgcolor: alpha(isOverBalance ? '#C62828' : accentColor, 0.1), color: isOverBalance ? '#C62828' : accentColor, fontSize: '0.75rem', height: 28 }}
                          />
                        ))
                      ) : (
                ...
                <ProfessionalButton
                  ...
                  sx={{
                    mt: 3, py: 1.5, fontSize: '1rem',
                    backgroundColor: isOverBalance ? '#ccc' : accentColor,
                    color: isOverBalance ? '#666' : primaryColor,
                    '&:hover': { backgroundColor: isOverBalance ? '#ccc' : accentDark },
                    '&:disabled': { backgroundColor: '#ccc !important', color: '#666 !important' },
                  }}
                >
                  {isOverBalance ? 'Insufficient Balance — Cannot Submit' : 'Submit Leave Request'}
                </ProfessionalButton>
              </Box>
            </GlassCard>
          </Fade>
          ...
          <Fade in timeout={900}>
            <GlassCard sx={{ border: `1px solid ${alpha(accentColor, 0.1)}` }}>
              ...
              <Box sx={{ p: 4, maxHeight: 500, overflowY: 'auto', '&::-webkit-scrollbar': { width: 6 }, '&::-webkit-scrollbar-track': { background: '#f1f1f1', borderRadius: 3 }, '&::-webkit-scrollbar-thumb': { background: accentColor, borderRadius: 3 } }}>
                {filteredLeaveRequests.length === 0 ? (
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    {filteredLeaveRequests.map(leaveRequest => {
                      const typeInfo   = getLeaveTypeInfo(leaveRequest.leave_code);
                            ...
                            <Grid container spacing={3} alignItems="center">
                              <Grid item xs={12} md={3}>
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                  ...
                                  <Typography variant="body1" sx={{ fontWeight: 600, color: accentColor }}>{typeInfo.leave_code || leaveRequest.leave_code}</Typography>
                                </Box>
                                <Typography variant="caption" sx={{ color: '#666' }}>{typeInfo.leave_description}</Typography>
                              </Grid>
                              <Grid item xs={6} md={2}>
                                <Typography variant="caption" sx={{ color: '#666', display: 'block', mb: 1 }}>Leave Date</Typography>
                                <Typography variant="body1" sx={{ fontWeight: 600 }}>{formatDate(leaveRequest.leave_date)}</Typography>
                              </Grid>
                              <Grid item xs={6} md={3}>
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                  ...
                                  <Typography variant="body2" sx={{ fontWeight: 500 }}>{formatDateTime(leaveRequest.createdAt || leaveRequest.dateSubmitted)}</Typography>
                                </Box>
                              </Grid>
                              <Grid item xs={6} md={2}>
                                ...
                                <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 1, px: 2, py: 0.75, borderRadius: 2, bgcolor: statusInfo.bg, border: `1px solid ${alpha(statusInfo.color, 0.2)}` }}>
                                  ...
                                  <Typography variant="body2" sx={{ fontWeight: 600, color: statusInfo.color }}>{statusInfo.label}</Typography>
                                </Box>
                                {statusInfo.sublabel && (
                                  <Typography variant="caption" sx={{ color: '#888', display: 'block', mt: 0.5 }}>{statusInfo.sublabel}</Typography>
                                )}
                              </Grid>
                              <Grid item xs={6} md={2}>
                                {canCancel && (
                                  <ProfessionalButton size="small" variant="outlined" onClick={() => handleCancelRequest(leaveRequest.id)}
                                    sx={{ color: '#C62828', borderColor: alpha('#C62828', 0.3), '&:hover': { backgroundColor: alpha('#C62828', 0.08), borderColor: '#C62828' } }}>
                                    Cancel Request
                                  </ProfessionalButton>
                                )}
                              </Grid>
        ...
        <Grid item xs={12} lg={4}>
          <Fade in timeout={800}>
            <Box sx={{ position: 'sticky', top: 20 }}>
              <LeaveCredits personID={personID} accentColor={accentColor} accentDark={accentDark} primaryColor={primaryColor} secondaryColor={secondaryColor} />
            </Box>
          </Fade>
        </Grid>
      </Grid>
