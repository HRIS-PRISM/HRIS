...
const groupByLeaveCode = rawAssignments => {
  ...
  rawAssignments.forEach(a => {
    if (!byCode[a.code]) byCode[a.code] = { code: a.code, name: a.name, entries: [] };
  ...
  return Object.values(byCode).map(group => {
    ...
    const prevRemainingDays = previousEntries.reduce((s, e) => s + e.remaining, 0);
    ...
    return {
      ...
      current,
      previousEntries,
      currRemaining, currTotal, currUsed, currAllocated,
      prevRemainingDays, prevTotalDays, prevUsedDays,
      grandTotal, grandRemaining, grandUsed,
    };
  });
};
      ...
      <Box sx={{ p: 2 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1.25 }}>
          <Box sx={{ flex: 1, minWidth: 0 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, flexWrap: 'wrap' }}>
              <Typography sx={{ fontWeight: 800, color: '#2a2a2a', fontSize: '0.85rem' }}>
                {group.code}
              </Typography>
        ...
        <Box sx={{
          ...
          {hasPrevious && (
            <>
              <Box sx={{ p: 1.25, bgcolor: alpha(prevColor, 0.04) }}>
                ...
                <Typography sx={{ fontWeight: 800, color: prevColor, fontSize: '1.05rem', lineHeight: 1 }}>
          ...
          <Box sx={{ p: 1.25, bgcolor: alpha(currColor, 0.03) }}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 0.5 }}>
              ...
              {group.current?.period_year && (
                <Chip
                  ...
                  size="small"
                  sx={{
                    height: 15, fontSize: '0.57rem', fontWeight: 700,
            ...
            <Typography sx={{ fontWeight: 800, color: currColor, fontSize: '1.05rem', lineHeight: 1 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            {hasPrevious && (
              ...
              >
                ...
                <Typography variant="caption" sx={{ fontSize: '0.67rem', fontWeight: 700 }}>
                  History
                </Typography>
      ...
      {hasPrevious && (
        <Collapse in={expanded}>
          ...
          <Box sx={{ px: 2, py: 1.5, bgcolor: alpha(accentColor, 0.015) }}>
            <Typography variant="caption" sx={{
              fontWeight: 700, color: '#999', fontSize: '0.63rem',
              letterSpacing: 0.5, textTransform: 'uppercase', display: 'block', mb: 1,
            }}>
              Previous Periods
            </Typography>
              ...
              {group.previousEntries.map((entry, i) => {
                ...
                return (
                  <Box key={i} sx={{
                    ...
                    <Typography variant="caption" sx={{ fontWeight: 700, color: '#666', fontSize: '0.63rem', display: 'block' }}>
                      {periodLabel(entry)}
                    </Typography>
                    <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 0.25 }}>
                      <Typography sx={{ fontWeight: 800, color: c, fontSize: '0.88rem' }}>
                        {entry.remaining.toFixed(1)}
                      </Typography>
                      <Typography variant="caption" sx={{ color: '#ccc', fontSize: '0.63rem' }}>
                        /{entry.total.toFixed(1)}d
                      </Typography>
                    </Box>
                    <Typography variant="caption" sx={{ color: '#bbb', fontSize: '0.6rem' }}>
...
const CompactView = ({ rawCredits, loading, accentColor }) => {
  if (loading) return <CircularProgress size={16} sx={{ color: accentColor }} />;
...
const FullView = ({ rawCredits, loading, error, onRetry, accentColor, accentDark }) => {
  ...
  return (
    <>
      ...
      {/* Leave type cards */}
      <Paper elevation={0} sx={{ borderRadius: 3, border: `1px solid ${alpha(accentColor, 0.08)}`, overflow: 'hidden' }}>
        <Box sx={{
          px: 2.5, py: 1.75,
          ...
          <Typography variant="subtitle2" sx={{ fontWeight: 700, color: accentColor, fontSize: '0.82rem' }}>
            Leave Type Balances
          </Typography>
          <Typography variant="caption" sx={{ color: '#bbb', fontSize: '0.7rem' }}>
            {grouped.length} type{grouped.length !== 1 ? 's' : ''} · click History to see periods
          </Typography>
        </Box>
...
const LeaveCredits = ({
  ...
  const fetchCredits = async () => {
    ...
    try {
      ...
      const data = userAssignments.map(a => {
        const leaveType = typesRes.data.find(lt => lt.leave_code === a.leave_code);
        return {
          code:            a.leave_code,
          ...
          allocated:       (parseFloat(a.allocated_hours) || parseFloat(a.total_hours) || 0) / 8,
  ...
  useEffect(() => { fetchCredits(); }, [personID]);
    ...
    <Paper elevation={0} sx={{
      ...
      <Box sx={{
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>Your Leave Balance</Typography>
            <Typography variant="caption" sx={{ opacity: 0.85 }}>Available leave days remaining</Typography>
          </Box>
        </Box>
      </Box>
