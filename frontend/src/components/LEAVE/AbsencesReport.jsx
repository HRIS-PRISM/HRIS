...
const AbsencesReport = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Also try to get AWOL data from attendance records
      try {
        const attendRes = await axios.get(`${API_BASE_URL}/attendance/api/overall_attendance_record`, getAuthHeaders());
        const attendRows = Array.isArray(attendRes.data?.data) ? attendRes.data.data : [];
        attendRows.forEach(row => {
          // If there's absent data with dates, add AWOL entries
          if (row._absentTotalDays && row.personID) {
            const dateMatch = String(row._absentTotalDays).match(/\d{4}-\d{2}-\d{2}/g) || [];
            dateMatch.forEach(date => {
              // Only add if not already covered by a leave request
              const alreadyCovered = absenceRecords.some(
                a => a.employeeNumber === String(row.personID) && a.date === date
              );
              if (!alreadyCovered) {
                absenceRecords.push({
                  id:             `awol-${row.personID}-${date}`,
                  requestId:      null,
                  employeeNumber: String(row.personID),
                  department:     row.code || '—',
                  date,
                  absenceType:    'AWOL',
                  status:         'Pending',
      ...
      } catch { /* attendance data optional */ }
