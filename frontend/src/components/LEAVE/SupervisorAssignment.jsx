/**
 * SupervisorAssignment.jsx
 * Admin/HR panel — assign employees as Deans, Department Heads, or Supervisors
 * and map them to department codes.
 */
...
import {
  ...
  Fade, Avatar, Tooltip, Button, TextField, Chip, List, ListItem, Paper,
...
import {
  ...
  Refresh, SupervisorAccount as SupervisorIcon,
  ArrowBack as ArrowBackIcon, ManageAccounts as ManageAccountsIcon,
  WorkspacePremium as PremiumIcon, Badge as BadgeIcon,
    ...
    >
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} lg={4}>
          <Box sx={{ borderRadius: 2, border: `1px solid ${T.accentBorder}`, bgcolor: '#fff', overflow: 'hidden', animation: 'blink 2s ease-in-out 0s infinite' }}>
            ...
            <Box sx={{ px: 3, py: 2.5, display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Box>
                ...
                <Box sx={{ height: 24, borderRadius: 6, bgcolor: 'rgba(109,35,35,0.06)', border: `1px solid ${T.accentBorder}`, width: 100 }} />
              </Box>
              {/* Department field */}
              <Box>
              ...
              {/* Assign button */}
              <Box sx={{ height: 38, borderRadius: 2, bgcolor: 'rgba(109,35,35,0.15)', mt: 1 }} />
              {/* Role info */}
              <Box sx={{ p: 1.5, borderRadius: 2, border: `1px solid ${T.divider}`, bgcolor: '#fafafa' }}>
                <Bone w={40} h={9} sx={{ mb: 1 }} />
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <Box sx={{ width: 80, height: 22, borderRadius: 6, bgcolor: 'rgba(109,35,35,0.06)', border: `1px solid ${T.accentBorder}` }} />
                  <Bone w={180} h={10} />
                </Box>
              </Box>
            </Box>
        ...
        <Grid item xs={12} lg={8}>
          <Box sx={{ borderRadius: 2, border: `1px solid ${T.accentBorder}`, bgcolor: '#fff', overflow: 'hidden', animation: 'blink 2s ease-in-out 0.1s infinite', height: 'calc(100vh - 280px)', display: 'flex', flexDirection: 'column' }}>
            <Box sx={{ px: 3.5, py: 2, borderBottom: `1px solid ${T.divider}`, bgcolor: T.accentFaint }}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5 }}>
                ...
                <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                  <Box sx={{ width: 80, height: 24, borderRadius: 6, bgcolor: 'rgba(109,35,35,0.08)', border: `1px solid ${T.accentBorder}` }} />
                  <Box sx={{ width: 60, height: 28, borderRadius: 2, border: `1px solid ${T.accentBorder}`, bgcolor: '#fff' }} />
                </Box>
              </Box>
              <Box sx={{ height: 40, borderRadius: 2, border: `1px solid ${T.accentBorder}`, bgcolor: '#fff' }} />
            </Box>
            <Box sx={{ flexGrow: 1, p: 2, display: 'flex', flexDirection: 'column', gap: 1 }}>
              {/* Fake table header */}
              <Box sx={{ px: 2, py: 1, display: 'grid', gridTemplateColumns: '1fr 1fr 140px 100px', gap: 1, borderRadius: 1.5, bgcolor: 'rgba(109,35,35,0.04)' }}>
                {[90, 70, 60, 50].map((w, i) => <Bone key={i} w={w} h={9} />)}
              </Box>
              {/* Fake rows */}
              {[...Array(7)].map((_, i) => (
                <Box key={i} sx={{ px: 2, py: 1.25, display: 'grid', gridTemplateColumns: '1fr 1fr 140px 100px', gap: 1, alignItems: 'center', borderRadius: 1.5, bgcolor: i % 2 === 0 ? '#fff' : 'rgba(109,35,35,0.025)', animation: 'blink 2s ease-in-out infinite', animationDelay: `${i * 0.08}s` }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25 }}>
                    <Box sx={{ width: 26, height: 26, borderRadius: '50%', bgcolor: 'rgba(109,35,35,0.1)', flexShrink: 0 }} />
                    <Box sx={{ flex: 1 }}><Bone w="70%" h={11} sx={{ mb: 0.5 }} /><Bone w="45%" h={9} /></Box>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    <Box sx={{ width: 13, height: 13, borderRadius: '50%', bgcolor: 'rgba(109,35,35,0.1)', flexShrink: 0 }} />
                    <Bone w="60%" h={11} />
                  </Box>
                  <Box sx={{ width: 90, height: 22, borderRadius: 6, bgcolor: 'rgba(109,35,35,0.06)', border: `1px solid ${T.accentBorder}` }} />
                  <Box sx={{ display: 'flex', gap: 0.75 }}>
                    <Box sx={{ width: 28, height: 28, borderRadius: 1, bgcolor: 'rgba(109,35,35,0.06)', border: `1px solid ${T.accentBorder}` }} />
                    <Box sx={{ width: 28, height: 28, borderRadius: 1, bgcolor: 'rgba(198,40,40,0.06)', border: '1px solid rgba(198,40,40,0.15)' }} />
                  </Box>
                </Box>
              ))}
            </Box>
...
const sortByLastName = (arr) => [...arr].sort((a, b) => getSurname(a.supervisorName || a.name || '').localeCompare(getSurname(b.supervisorName || b.name || '')));
    ...
    <Box sx={{ position: 'relative', width: '100%' }} ref={ref}>
      <FieldInput value={query}
        ...
        InputProps={{
          ...
          endAdornment: (
            <IconButton size="small" sx={{ color: T.muted }} onClick={() => setOpen((p) => !p)} disabled={disabled}>
          ...
          {filtered.length > 0 ? (
            ...
            <Box sx={{ p: 2, textAlign: 'center' }}>
              <Typography sx={{ fontSize: '0.8rem', color: T.faint, fontStyle: 'italic' }}>
                {query ? `No match for "${query}"` : 'No department codes found'}
              </Typography>
            </Box>
          )}
        </Paper>
...
const EmployeeAutocomplete = ({ value, onChange, selectedEmployee, onEmployeeSelect, placeholder = 'Search employee…', disabled = false }) => {
    ...
    <Box sx={{ position: 'relative', width: '100%' }} ref={ref}>
      <FieldInput value={query}
        ...
        InputProps={{
          ...
          endAdornment: (
            <IconButton size="small" sx={{ color: T.muted }} onClick={() => { if (!open) { setOpen(true); if (!employees.length && !loading) fetchAll(); } else setOpen(false); }}>
              {open ? <ExpandLessIcon sx={{ fontSize: 15 }} /> : <ExpandMoreIcon sx={{ fontSize: 15 }} />}
            </IconButton>
          ),
        }}
      ...
      {open && (
        <Paper elevation={4} sx={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 1400, maxHeight: 260, overflow: 'auto', mt: 0.75, borderRadius: 2, border: `1px solid ${T.accentBorder}`, ...scrollbarSx }}>
          {loading ? (
            ...
            <Box sx={{ p: 2, textAlign: 'center' }}>
              <Typography sx={{ fontSize: '0.8rem', color: T.faint, fontStyle: 'italic' }}>
                {query.length >= 2 ? `No employees found for "${query}"` : 'Type to search or browse'}
              </Typography>
            </Box>
          )}
        </Paper>
...
const ModalHeader = ({ title, subtitle, chips = [], onBack, onClose }) => (
  <Box sx={{ px: 3.5, py: 2.5, background: T.headerGrad, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
    ...
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, position: 'relative', zIndex: 1 }}>
      <Avatar sx={{ width: 44, height: 44, bgcolor: 'rgba(255,255,255,0.15)', color: '#fff' }}><SupervisorIcon sx={{ fontSize: 22 }} /></Avatar>
      <Box>
...
const SupervisorAssignment = () => {
  const { hasAccess, loading: accessLoading } = usePageAccess('supervisor-assignment');
  ...
  const fetchAssignments = async () => {
    try {
      const r = await axios.get(`${API_BASE_URL}/api/supervisor-assignment`, getAuthHeaders());
      setAssignments(Array.isArray(r.data) ? r.data : []);
    } catch { setSnackMsg('Failed to load supervisor assignments.'); }
  };
  ...
  const handleDelete = async (id) => {
    setLoading(true);
    try {
      await axios.delete(`${API_BASE_URL}/api/supervisor-assignment/${id}`, getAuthHeaders());
      setSuccessAction('delete'); setSuccessOpen(true);
      setDeleteConfirmId(null);
      setModalOpen(false);
      setSelectedAssignment(null);
      fetchAssignments();
    } catch { setSnackMsg('Failed to remove assignment.'); }
    finally { setLoading(false); }
  };
  ...
  if (hasPermission === false) return <AccessDenied title="Access Denied" message="You do not have permission to access Supervisor Assignment. Contact your HR administrator if you believe this is an error." returnPath="/admin-home" returnButtonText="Return to Home" />;

  const filtered = assignments.filter((a) => {
    const term = searchTerm.toLowerCase();
    if (!term) return true;
    return (a.supervisorName || '').toLowerCase().includes(term) ||
    ...
    <Fade in timeout={400}>
      <Box sx={{ py: { xs: 1, md: 2 }, mt: { xs: 0, md: -2 }, mb: { xs: 1, md: 2 }, width: '100vw', maxWidth: '100%', position: 'relative', left: '63%', transform: 'translateX(-61%)', px: { xs: 2, sm: 3, md: 6 } }}>
        ...
        <SectionCard sx={{ mb: 2, overflow: 'hidden' }}>
          <Box sx={{ px: 4, py: 3, background: 'linear-gradient(135deg,#fdf5f5 0%,#f0dede 100%)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', overflow: 'hidden' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, position: 'relative', zIndex: 1 }}>
              ...
              <Box>
                <Typography sx={{ fontSize: '1.25rem', fontWeight: 900, color: T.accent, lineHeight: 1.2, mb: 0.3 }}>Supervisor Assignment Management</Typography>
                <Typography sx={{ fontSize: '0.82rem', color: T.accentMid, fontWeight: 700, opacity: 0.9 }}>Administrative Panel • Assign Deans, Department Heads, and Supervisors to departments</Typography>
              </Box>
            </Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, position: 'relative', zIndex: 1 }}>
              <Box sx={{ px: 2.5, py: 0.75, borderRadius: 6, bgcolor: alpha(T.accent, 0.1), border: `1px solid ${alpha(T.accent, 0.2)}` }}>
                <Typography sx={{ fontSize: '0.8rem', color: T.accent, fontWeight: 700 }}>{assignments.length} {assignments.length === 1 ? 'assignment' : 'assignments'}</Typography>
              </Box>
          ...
          <Grid item xs={12} lg={4}>
            <SectionCard sx={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <Box sx={{ px: 3, py: 2.5, display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                {/* Department */}
                <Box>
                  <Typography sx={{ fontSize: '0.75rem', fontWeight: 600, color: T.accent, mb: 0.75 }}>Department <Box component="span" sx={{ color: '#c62828' }}>*</Box></Typography>
                  <DeptCodeAutocomplete value={formDepartmentCode} onChange={setFormDepartmentCode} departmentList={departmentList} />
                  {deptObj?.description && <Typography sx={{ fontSize: '0.72rem', color: T.muted, mt: 0.5, ml: 0.5 }}>{deptObj.description}</Typography>}
                </Box>

                {/* Employee */}
                <Box>
                  <Typography sx={{ fontSize: '0.75rem', fontWeight: 600, color: T.accent, mb: 0.75 }}>Employee <Box component="span" sx={{ color: '#c62828' }}>*</Box></Typography>
                  <EmployeeAutocomplete value={formEmployeeNum} onChange={setFormEmployeeNum} selectedEmployee={formEmployee} onEmployeeSelect={setFormEmployee} placeholder="Search employee to assign…" />
                  {formEmployee && (
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mt: 1, p: '10px 12px', borderRadius: 2, border: `1px solid ${alpha(T.accent, 0.25)}`, bgcolor: T.accentFaint }}>
                      <Avatar sx={{ width: 32, height: 32, bgcolor: T.accent, color: '#fff', fontSize: '0.75rem', fontWeight: 700 }}>{formEmployee.name?.charAt(0)?.toUpperCase() || '?'}</Avatar>
                      <Box sx={{ flex: 1, minWidth: 0 }}>
                        <Typography noWrap sx={{ fontSize: '0.83rem', fontWeight: 700, color: T.text }}>{formEmployee.name}</Typography>
                        <Typography sx={{ fontSize: '0.7rem', color: T.muted }}>#{formEmployee.employeeNumber}</Typography>
                      </Box>
                      <IconButton size="small" onClick={() => { setFormEmployee(null); setFormEmployeeNum(''); }} sx={{ color: '#c62828', width: 22, height: 22 }}>
                ...
                {snackMsg && (
                  <Box sx={{ px: 2, py: 1.25, borderRadius: 2, bgcolor: '#FFEBEE', border: '1px solid rgba(198,40,40,0.25)' }}>
                    <Typography sx={{ fontSize: '0.78rem', color: '#C62828' }}>{snackMsg}</Typography>
                  </Box>
                )}
                ...
                <AccentButton onClick={handleAssign} variant="contained"
                  ...
                  fullWidth disabled={loading || !formEmployee || !formDepartmentCode}
              ...
              <Box sx={{ px: 3.5, py: 2, borderBottom: `1px solid ${T.divider}`, bgcolor: T.accentFaint }}>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    ...
                    <Typography sx={{ fontSize: '0.88rem', fontWeight: 700, color: T.text }}>Supervisor Records</Typography>
                  </Box>
                  <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                    <Box sx={{ px: 1.5, py: 0.4, borderRadius: 6, bgcolor: alpha(T.accent, 0.08), border: `1px solid ${alpha(T.accent, 0.15)}` }}>
                      <Typography sx={{ fontSize: '0.72rem', color: T.accent, fontWeight: 700 }}>{filtered.length} record{filtered.length !== 1 ? 's' : ''}</Typography>
                    </Box>
                ...
                {filtered.length === 0 ? (
                  <Box sx={{ py: 10, textAlign: 'center' }}>
                    <Box sx={{ width: 72, height: 72, borderRadius: '50%', bgcolor: T.accentFaint, display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', mb: 2 }}>
                      <SupervisorIcon sx={{ fontSize: 32, color: alpha(T.accent, 0.3) }} />
                    </Box>
                    <Typography sx={{ fontSize: '0.9rem', fontWeight: 600, color: T.muted, mb: 0.5 }}>
                      {assignments.length === 0 ? 'No supervisors assigned yet' : 'No records match your search'}
                    </Typography>
                    <Typography sx={{ fontSize: '0.78rem', color: T.faint }}>
                      {assignments.length === 0 ? 'Use the form on the left to assign.' : 'Try a different search term.'}
                    </Typography>
                  </Box>
                ) : viewMode === 'grid' ? (
                        ...
                        <Grid item xs={6} sm={4} md={3} key={a.id}>
                          <Box onClick={() => handleOpenModal(a)}
                            ...
                            <Box>
                              ...
                              <Typography sx={{ fontSize: '0.7rem', color: T.muted }}>#{a.supervisorEmployeeNumber}</Typography>
                            </Box>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, px: 1, py: 0.35, borderRadius: 1, bgcolor: T.accentFaint, border: `0.5px solid ${T.accentBorder}` }}>
                              <DomainIcon sx={{ fontSize: 11, color: T.accent }} />
                              <Typography sx={{ fontSize: '0.7rem', fontWeight: 700, color: T.accent }}>{a.departmentCode}</Typography>
                  ...
                  <>
                    <Box sx={{ px: 2, py: 1, display: 'grid', gridTemplateColumns: '1fr 1fr 140px 100px', gap: 1, alignItems: 'center', bgcolor: alpha(T.accent, 0.04), borderRadius: 1.5, mb: 1 }}>
                      {['Supervisor', 'Department', 'Role', 'Actions'].map((col) => (
                    ...
                    {filtered.map((a, i) => (
                      <Box key={a.id} sx={{ px: 2, py: 1.25, display: 'grid', gridTemplateColumns: '1fr 1fr 140px 100px', gap: 1, alignItems: 'center', borderRadius: 1.5, bgcolor: i % 2 === 0 ? T.rowEven : T.rowOdd, border: '1px solid transparent', '&:hover': { bgcolor: T.rowHover } }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, minWidth: 0 }}>
                          <Avatar sx={{ width: 26, height: 26, bgcolor: alpha(T.accent, 0.12), color: T.accent, fontSize: '0.68rem', fontWeight: 700, flexShrink: 0 }}>{(a.supervisorName || '?').charAt(0).toUpperCase()}</Avatar>
                          <Box sx={{ minWidth: 0 }}>
                            <Typography sx={{ fontSize: '0.82rem', fontWeight: 600, color: T.text }} noWrap>{a.supervisorName || `Employee #${a.supervisorEmployeeNumber}`}</Typography>
                            <Typography sx={{ fontSize: '0.7rem', color: T.muted }}>#{a.supervisorEmployeeNumber}</Typography>
                          </Box>
                        </Box>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, minWidth: 0 }}>
                          <DomainIcon sx={{ fontSize: 13, color: T.muted, flexShrink: 0 }} />
                          <Box sx={{ minWidth: 0 }}>
                            <Typography sx={{ fontSize: '0.78rem', fontWeight: 600, color: T.text }} noWrap>{a.departmentCode}</Typography>
                            {a.departmentDescription && <Typography sx={{ fontSize: '0.68rem', color: T.muted }} noWrap>{a.departmentDescription}</Typography>}
                          </Box>
                        </Box>
                        <TitleBadge title={a.role} />
                        <Box sx={{ display: 'flex', gap: 0.75 }}>
                          ...
                          <Tooltip title="Remove">
                            <IconButton size="small" onClick={() => setDeleteConfirmId(a.id)} sx={{ width: 28, height: 28, color: '#c62828', border: '1px solid rgba(198,40,40,0.2)', '&:hover': { bgcolor: 'rgba(198,40,40,0.06)' } }}>
        ...
        <Modal open={modalOpen} onClose={() => setModalOpen(false)} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', p: 2 }}>
          <Fade in={modalOpen}>
            <Box sx={{ width: '95%', maxWidth: 520, borderRadius: 3, overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,0.22)', outline: 'none', bgcolor: T.surface, display: 'flex', flexDirection: 'column' }}>
              {selectedAssignment && (
                <>
                  <ModalHeader
                    title={selectedAssignment.supervisorName || `Employee #${selectedAssignment.supervisorEmployeeNumber}`}
                  ...
                  <Box sx={{ p: 3, display: 'flex', flexDirection: 'column', gap: 2 }}>
                    <Box sx={{ display: 'flex', gap: 2 }}>
                      <Box sx={{ flex: 1, p: 1.5, borderRadius: 2, bgcolor: T.accentFaint, border: `1px solid ${T.accentBorder}` }}>
                        <Typography sx={{ fontSize: '0.7rem', color: T.muted, mb: 0.5 }}>Employee</Typography>
                        <Typography sx={{ fontSize: '0.88rem', fontWeight: 700, color: T.text }}>{selectedAssignment.supervisorName || '—'}</Typography>
                        <Typography sx={{ fontSize: '0.72rem', color: T.muted }}>#{selectedAssignment.supervisorEmployeeNumber}</Typography>
                      </Box>
                      <Box sx={{ flex: 1, p: 1.5, borderRadius: 2, bgcolor: T.accentFaint, border: `1px solid ${T.accentBorder}` }}>
                        <Typography sx={{ fontSize: '0.7rem', color: T.muted, mb: 0.5 }}>Department</Typography>
                        <Typography sx={{ fontSize: '0.88rem', fontWeight: 700, color: T.text }}>{selectedAssignment.departmentCode}</Typography>
                        {selectedAssignment.departmentDescription && <Typography sx={{ fontSize: '0.72rem', color: T.muted }}>{selectedAssignment.departmentDescription}</Typography>}
                      </Box>
                    </Box>
                  ...
                  <Box sx={{ px: 3, py: 2, borderTop: `1px solid ${T.divider}`, display: 'flex', justifyContent: 'space-between', gap: 1 }}>
                    <AccentButton onClick={() => setDeleteConfirmId(selectedAssignment.id)} variant="outlined"
                    ...
                    <AccentButton onClick={() => setModalOpen(false)} variant="contained"
                      startIcon={<CancelIcon sx={{ fontSize: '14px !important' }} />}
                      sx={{ fontSize: '0.8rem', bgcolor: T.muted, color: '#fff', '&:hover': { bgcolor: '#555' } }}>
                      Close
                    </AccentButton>
                  </Box>
                </>
        ...
        {/* Delete Confirm */}
        <Modal open={!!deleteConfirmId} onClose={() => setDeleteConfirmId(null)} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', p: 2 }}>
          <Fade in={!!deleteConfirmId}>
            <Box sx={{ width: '100%', maxWidth: 400, borderRadius: 3, overflow: 'hidden', bgcolor: T.surface, boxShadow: '0 24px 64px rgba(0,0,0,0.22)' }}>
              <Box sx={{ px: 3, py: 2.5, background: 'linear-gradient(180deg,#c62828 0%,#d32f2f 100%)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Typography sx={{ fontWeight: 700, color: '#fff', fontSize: '0.95rem' }}>Confirm Removal</Typography>
                <IconButton size="small" onClick={() => setDeleteConfirmId(null)} sx={{ color: 'rgba(255,255,255,0.75)' }}><Close sx={{ fontSize: 16 }} /></IconButton>
              </Box>
              <Box sx={{ px: 3, py: 3 }}>
                <Typography sx={{ fontSize: '0.88rem', color: T.text, lineHeight: 1.65 }}>
              ...
              <Box sx={{ px: 3, py: 2, borderTop: `1px solid ${T.divider}`, bgcolor: '#f9f9f9', display: 'flex', justifyContent: 'flex-end', gap: 1 }}>
                <AccentButton onClick={() => setDeleteConfirmId(null)} variant="outlined" sx={{ fontSize: '0.8rem', borderColor: T.accentBorder, color: T.muted, '&:hover': { transform: 'none' } }}>Cancel</AccentButton>
                <AccentButton onClick={() => handleDelete(deleteConfirmId)} variant="contained" disabled={loading}
